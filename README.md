...
| | | |
|---|---|---|
| **Compiler** | 12 passes, 3 complete, **9 carried with a named residue**, 0 partial, 0 open | 248 diagnostics · `gabbro paesse` |
...
| **Pass register** | **77 statements over 12 passes — 70 measured, **2 ARGUED**, 5 conjectured, 0 PROVED.** 45 of 248 diagnostics still owe one, and that number is a **ratchet**: it may fall, not rise. *A written statement is not a proved one — the third column is the whole rest* | `gabbro paesse --je-satz` · `./instrumente/pruefe-saetze.py` |
| **Guardians** | 27, and ~~44 of 44~~ ~~45 of 45~~ ~~47 of 47~~ **49 of 49 instruments carry all five requirements** — four read statically (deadline · two-way speech test · red on abort · **pinned locale**), the fifth (**work quantity beside the verdict**, W17) measured only by `--lauf`, held by `./instrumente/pruefe-waechter.py`. *The locale demand joined on 2026-08-25: under `de_DE.UTF-8` the linker says `Mehrfachdefinition von`, and `pruefe-emission.sh` reported an error that did not exist.* *The static half reads SOURCE; `--lauf` runs the light ones under a deadline* | **294 of 294 mutations caught** *(run 2026-08-24, `ki-pc-fisch-101`)* |
...
| **Corpus** | 57 clean examples, 331 poison files, 243 tests *(run 2026-08-31, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the last three are `beispiele/55`–`57`, one per quantifier domain that no program had ever written | `cargo test` |
