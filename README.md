...
| | | |
|---|---|---|
| **Compiler** | 12 passes, 3 complete, **9 carried with a named residue**, 0 partial, 0 open | 253 diagnostics · `gabbro paesse` |
...
| **Pass register** | **81 statements over 12 passes — 74 measured, **2 ARGUED**, 5 conjectured, 0 PROVED.** 45 of 253 diagnostics still owe one, and that number is a **ratchet**: it may fall, not rise. *A written statement is not a proved one — the third column is the whole rest* | `gabbro paesse --je-satz` · `./instrumente/pruefe-saetze.py` |
...
| **Corpus** | 57 clean examples, 341 poison files, 256 tests *(run 2026-08-31, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the last three are `beispiele/55`–`57`, one per quantifier domain that no program had ever written | `cargo test` |
