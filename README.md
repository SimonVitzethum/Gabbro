...
| **Corpus** | 63 clean examples, 383 poison files, 362 tests *(run 2026-09-01, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the newest is `beispiele/63-druckt.gab`, the first Gabbro program that PRINTS: `extern fn putchar(c : i32) -> i32`, emitted, compiled under `-Wall -Wextra -Werror` and RUN -- until 2026-09-01 `N041` refused every `extern fn` on a name C owns, and `putchar`, `puts` and `printf` are all three such names | `cargo test` |
