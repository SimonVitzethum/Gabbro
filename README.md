...
| **Corpus** | 60 clean examples, 348 poison files, 283 tests *(run 2026-08-31, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the newest are `beispiele/59` and `/60`, the counterprobes to `H102` and `A005`: an entry that hardware throws, and an assumption that names its machine | `cargo test` |
