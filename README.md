...
| | | |
|---|---|---|
| **Compiler** | 12 passes, 3 complete, **9 carried with a named residue**, 0 partial, 0 open | 266 diagnostics · `gabbro paesse` |
...
| **Pass register** | **86 statements over 12 passes — 74 measured, **2 ARGUED**, 5 conjectured, 0 PROVED.** 50 of 266 diagnostics still owe one, and that number is a **ratchet**: it may fall, not rise. *A written statement is not a proved one — the third column is the whole rest* | `gabbro paesse --je-satz` · `./instrumente/pruefe-saetze.py` |
...
| **Corpus** | 63 clean examples, 394 poison files, 373 tests *(run 2026-09-01, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the newest is `beispiele/63-druckt.gab`, the first Gabbro program that PRINTS: `extern fn putchar(c : i32) -> i32`, emitted, compiled under `-Wall -Wextra -Werror` and RUN -- until 2026-09-01 `N041` refused every `extern fn` on a name C owns, and `putchar`, `puts` and `printf` are all three such names | `cargo test` |
