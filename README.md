...
| **Guardians** | 43, and **69 of 72 instruments carry all five requirements** — deadline, two-way speech test, red on abort, pinned locale, and work quantity beside the verdict. *42 → 43 and 68/71 → 69/72 on 2026-09-15: the new one is `pruefe-genlean.py`, which re-runs `gabbro lean-g` / `gabbro obligations --g` on every COMMITTED generated Lean file and compares it byte for byte — the loop between `lean_g.rs` and the files the goal theorem is applied in, the same idea `pruefe-ctext.py` (added the same day) has on the emitted C* | `./instrumente/abnahme.py` |
