...
| **Guardians** | 42, and **68 of 71 instruments carry all five requirements** — deadline, two-way speech test, red on abort, pinned locale, and work quantity beside the verdict. *41 → 42 and 65/68 → 68/71 on 2026-09-15: the new one is `pruefe-ctext.py`, which re-emits every pinned C text and compares it byte for byte with the Lean pin (`CTEXT-PIN`) — the loop between `emit.rs` and the A2 proof. The other two came from lanes before it and stood unbooked; the figure is the measured one, not this lane's share* | `./instrumente/abnahme.py` |
