...
| | | |
|---|---|---|
| **Compiler** | 12 passes, 3 complete, **9 carried with a named residue**, 0 partial, 0 open | 245 diagnostics · `gabbro paesse` |
...
| **Pass register** | **75 statements over 12 passes — 68 measured, **2 ARGUED**, 5 conjectured, 0 PROVED.** 45 of 245 diagnostics still owe one, and that number is a **ratchet**: it may fall, not rise. *A written statement is not a proved one — the third column is the whole rest* | `gabbro paesse --je-satz` · `./instrumente/pruefe-saetze.py` |
...
| **Corpus** | 57 clean examples, 323 poison files, 242 tests *(run 2026-08-31, locally: `free -g` said 31 GB total, 13 available, 20 cores)* — the last three are `beispiele/gift/417`–`419`, one per class of the chain edge that no pass had ever read | `cargo test` |
...
| **Usability** | **5.8 % of the teaching corpus and 12.8 % of REAL code may fall** — 1050 and 109 clause sites, split derivable / redundant / load-bearing. The calibration travels with the tool (`--tafel`, per rule a may-fall AND a reason), because an uncalibrated usability number makes `effects` and `costs` the cheapest thing to drop | `gabbro zeremonie` · `./instrumente/zaehle-zeremonie.py` |
