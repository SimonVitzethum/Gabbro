...
| | | |
|---|---|---|
| **Compiler** | 12 passes, 3 complete, **9 carried with a named residue**, 0 partial, 0 open | 263 diagnostics · `gabbro paesse` |
...
| **Pass register** | **86 statements over 12 passes — 74 measured, **2 ARGUED**, 5 conjectured, 0 PROVED.** 48 of 263 diagnostics still owe one, and that number is a **ratchet**: it may fall, not rise. *A written statement is not a proved one — the third column is the whole rest* | `gabbro paesse --je-satz` · `./instrumente/pruefe-saetze.py` |
| **Guardians** | 29, and ~~44 of 44~~ ~~45 of 45~~ ~~47 of 47~~ ~~49 of 49~~ ~~50 of 50~~ ~~52 of 52~~ ~~53 of 53~~ ~~54 of 54~~ **55 of 55 instruments carry all five requirements** — four read statically (deadline · two-way speech test · red on abort · **pinned locale**), the fifth (**work quantity beside the verdict**, W17) measured only by `--lauf`, held by `./instrumente/pruefe-waechter.py`. *The locale demand joined on 2026-08-25: under `de_DE.UTF-8` the linker says `Mehrfachdefinition von`, and `pruefe-emission.sh` reported an error that did not exist.* *The static half reads SOURCE; `--lauf` runs the light ones under a deadline* | **294 of 294 mutations caught** *(run 2026-08-24, `ki-pc-fisch-101`)* |
...
| **Corpus** | 63 clean examples, 390 poison files, 368 tests *(run 2026-09-01, locally -- the `free -g` beside it stands in the commit, not in this cell: a memory figure in a CORPUS row is a number the guard reads as corpus data)* — the newest is `beispiele/63-druckt.gab`, the first Gabbro program that PRINTS: `extern fn putchar(c : i32) -> i32`, emitted, compiled under `-Wall -Wextra -Werror` and RUN -- until 2026-09-01 `N041` refused every `extern fn` on a name C owns, and `putchar`, `puts` and `printf` are all three such names | `cargo test` |
