...
- [ ] **`guarded` und `covered` sind VERSCHIEDENE Stärken, und die Trennung wird verlockend
      ...
      **Seit dem 2026-08-20 stehen beide Zahlen im Register und nicht mehr nur im Bericht:**
      ~~169~~ ~~170~~ ~~25~~ ~~171~~ **173 besetzte Zellen** stehen daneben, **~~26~~ 25 nur im Gift** (2026-09-13, lane 152: `atomic × written` and `tagged × return (body)` turn poison-only, `atomic × read` turns covered; 2026-09-14, lane 167, measured `74 blind · 173 covered · 25 poison-only · 12 no cell`: `tagged × return (body)` turns covered -- `beispiele/120` constructs and returns -- and `tagged × static` turns covered from BLIND -- `beispiele/121` initialises at file scope) — und `gabbro blindstellen`
...
Konstrukt — und **die Fehler sitzen an den Kombinationen**: ~~79~~ 78 blinde Zellen von 285. Jedes echte
...
- [ ] **Was eine Doppelvergabe RÜCKWIRKEND kostet, und das ist teurer als der Fund**
      *(gemessen 2026-08-21)*. Die Giftproben prüfen auf **Kennungen**. Eine doppelt
      ...
      ```
      ~~68~~ ~~70~~ ~~71~~ ~~73~~ ~~84~~ 88 Proben zeigen auf eine Kennung mit unaehnlichen Vergabestellen (von 641
      Giftproben; 2026-09-14, lane 167, recomputed to actuals: 86 on the unchanged tree are wave drift, 2 are mine -- `gift/938` and `gift/944` on the six-faced `N280`, each with its own face-probe beside it)
      ```
