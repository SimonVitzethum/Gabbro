...
theorem gift1 (s : State) (f : Int) (hf : s.local' "f" = .int f) :
    ∃ s', finalState (exec raeumen_body s) = some s' ∧ falsch_geraeumt f s' := by
...
theorem gift2 (s : State) (f : Int) (hf : s.local' "f" = .int f)
    (hoffen : s.world (.slot "Faecher" f "belegt") = .bool true) :
    ∃ s', finalState (exec raeumen_body s) = some s' ∧ falscher_rahmen f s s' := by
...
theorem gift3 (s : State) (f : Int) (hf : s.local' "f" = .int f) :
    ∃ s', finalState (exec raeumen_body s) = some s' ∧ erfundenes_feld f s' := by
