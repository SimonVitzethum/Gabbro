/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/proben/probe-zeugnis-injektiv-a.gab  total 0  goals 0  refused 0
...
namespace GabbroDuty.DutyProbeZeugnisInjektivA
...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Ring.plaetze" _ "elem" => some .int
  | .slot "Warteschlange" _ "a" => some .int
  | .slot "Warteschlange" _ "naechst" => some .opt
  | .field "Ring" "kopf" => some .int
  | _ => none
...
theorem eine_meets : eine_meets_statement := by
  ...
  gabbro_try 300 (gabbro_simp [eine_body, eine_pre, eine_post, wellFormed, nie_pre, nie_requires, nie_post, nie_writes, Frame_read _ _ _ fr_nie, eine_loop_1_inv, hall])
  trace_state
  gabbro_try 300 (all_goals (try gabbro_cases 4 [eine_body, eine_pre, eine_post, wellFormed, nie_pre, nie_requires, nie_post, nie_writes, Frame_read _ _ _ fr_nie, eine_loop_1_inv, hall]))
  gabbro_calls shapeOf [eine_body, eine_pre, eine_post, wellFormed, nie_pre, nie_requires, nie_post, nie_writes, Frame_read _ _ _ fr_nie, eine_loop_1_inv, hall]
  trace_state
  all_goals sorry
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoop ρ "eine#1" eine_loop_1_body "t"
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
  ...
  have l_eine_loop_1 : LoopRule ρ "eine#1" wellFormed eine_loop_1_inv :=
    looprule_of_body ρ "eine#1" wellFormed eine_loop_1_inv eine_loop_1_body "t" rl_eine_loop_1
      (fun t k hw hi => d_eine_loop_1 ρ t k hw hi)
...
end GabbroDuty.DutyProbeZeugnisInjektivA
