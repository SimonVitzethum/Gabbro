...
def kostenprobe_body : List Stmt :=
  [(.bindName "summe" (.lit (.int 0))), (.loop "kostenprobe#1" (.bin .and (.hasShape "summe" (.intIn 0 65535)) (.bin .le (.name "summe") (.lit (.int 65535)))) [(.ite (.place "Tafel" (.name "s") "belegt") [(.bindName "summe" (.bin .add (.name "summe") (.lit (.int 1))))] [])]), (.ret (some (.name "summe")))]
...
def kostenprobe_loop_1_inv : Expr :=
  (.bin .and (.hasShape "summe" (.intIn 0 65535)) (.bin .le (.name "summe") (.lit (.int 65535))))

def kostenprobe_loop_1_body : List Stmt :=
  [(.ite (.place "Tafel" (.name "s") "belegt") [(.bindName "summe" (.bin .add (.name "summe") (.lit (.int 1))))] [])]
...
theorem kostenprobe_loop_1_keeps : kostenprobe_loop_1_keeps_statement := by
  ...
  obtain ⟨n_Tafel_belegt_s, h_Tafel_belegt_s⟩ := WF_bool shapeOf t.world (.slot "Tafel" k "belegt") hwf rfl
  have hall := hinv
  gabbro_simp_at hall [kostenprobe_loop_1_inv, e_summe, h_Tafel_belegt_s]
  gabbro_auto [kostenprobe_loop_1_body, kostenprobe_loop_1_inv, wellFormed, e_summe, h_Tafel_belegt_s, hall] using shapeOf
...
def kostenprobe_meets_statement : Prop :=
  ∀ (ρ : Env) (s : State)
    ...
    (l_kostenprobe_loop_1 : LoopRule ρ "kostenprobe#1" wellFormed kostenprobe_loop_1_inv),
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoopIn ρ "kostenprobe#1" kostenprobe_loop_1_body "s" 0 16
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    ...
    ∧ LoopRule ρ "kostenprobe#1" wellFormed kostenprobe_loop_1_inv
  ...
  have l_kostenprobe_loop_1 : LoopRule ρ "kostenprobe#1" wellFormed kostenprobe_loop_1_inv :=
    looprule_of_body_in ρ "kostenprobe#1" wellFormed kostenprobe_loop_1_inv kostenprobe_loop_1_body "s" 0 16 rl_kostenprobe_loop_1
      (fun t k hlo hhi hw hi => d_kostenprobe_loop_1 ρ t k hlo hhi hw hi)
