/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/proben/probe-elems.gab  total 0  goals 0  refused 0
...
def als_u64_body : List Stmt :=
  [(.bindName "n" (.lit (.int 0))), (.bindName "#returned" (.lit (.bool false))), (.bindName "#ret" (.lit .absent)), (.loop "als_u64#1" (.bin .and (.hasShape "n" (.intIn 0 8192)) (.bin .and (.hasShape "#returned" .bool) (.bin .or (.bin .and (.name "#returned") (.hasShape "#ret" (.intIn 0 18446744073709551615))) (.bin .and (.un .not (.name "#returned")) (.lit (.bool true)))))) [(.ite (.bin .ne (.name "w") (.lit (.int 16045690984833335023))) [(.bindName "#ret" (.name "n")), (.bindName "#returned" (.lit (.bool true))), .leave] []), (.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))]), (.ite (.name "#returned") [(.ret (some (.name "#ret")))] []), (.ret (some (.name "n")))]
...
def als_u64_loop_1_inv : Expr :=
  (.bin .and (.hasShape "n" (.intIn 0 8192)) (.bin .and (.hasShape "#returned" .bool) (.bin .or (.bin .and (.name "#returned") (.hasShape "#ret" (.intIn 0 18446744073709551615))) (.bin .and (.un .not (.name "#returned")) (.lit (.bool true))))))

def als_u64_loop_1_body : List Stmt :=
  [(.ite (.bin .ne (.name "w") (.lit (.int 16045690984833335023))) [(.bindName "#ret" (.name "n")), (.bindName "#returned" (.lit (.bool true))), .leave] []), (.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))]
...
theorem als_u64_loop_1_keeps : als_u64_loop_1_keeps_statement := by
  ...
  have hall := hinv
  gabbro_simp_at hall [als_u64_loop_1_inv, e_n, e__returned]
  gabbro_auto [als_u64_loop_1_body, als_u64_loop_1_inv, wellFormed, e_n, e__returned, hall] using shapeOf
...
def als_u64_meets_statement : Prop :=
  ∀ (ρ : Env) (s : State)
    ...
    (l_als_u64_loop_1 : LoopRule ρ "als_u64#1" wellFormed als_u64_loop_1_inv),
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoopIn ρ "als_u64#1" als_u64_loop_1_body "w" 0 18446744073709551616

theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    (d_als_u64_loop_1 : als_u64_loop_1_keeps_statement)
    (d_als_u64 : als_u64_meets_statement) :
    LoopRule ρ "als_u64#1" wellFormed als_u64_loop_1_inv
    ∧ Contract ρ "als_u64" als_u64_requires als_u64_post := by
  obtain ⟨r_als_u64, fr_als_u64, rl_als_u64_loop_1⟩ := hp
  have l_als_u64_loop_1 : LoopRule ρ "als_u64#1" wellFormed als_u64_loop_1_inv :=
    looprule_of_body_in ρ "als_u64#1" wellFormed als_u64_loop_1_inv als_u64_loop_1_body "w" 0 18446744073709551616 rl_als_u64_loop_1
      (fun t k hlo hhi hw hi => d_als_u64_loop_1 ρ t k hlo hhi hw hi)
