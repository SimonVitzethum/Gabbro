/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/proben/probe-neun-domaenen.gab  total 11  goals 7  refused 4
        @assumed 2  of the refused are ASSUMPTIONS -- hardware, foreign code -- and 2 are refused forms
...
/-
  ...
  quantified-threads (1): refused -- `threads` -- no declaration names the thread set (every other domain hangs on one); write `slots of <the thread table>`, or the language needs a `threads over T`
    duty_11  N  d8_threads :: ensures #1

  quantified-mappings (1): refused -- `mappings of <walk>` -- the mappings of a page-table walk are hardware; state it as an `invariant` OF the `walk`, which is ASSUMED by name
...
/-
  ...
  duty_9  N  d6_fields :: ensures #1  --  carried by `d6_fields_meets`
  duty_10  N  d9_mappings :: ensures #1  --  refused (quantified-mappings)
  duty_11  N  d8_threads :: ensures #1  --  refused (quantified-threads)
-/
...
def d6_fields_post (s s' : State) (r : Option Value) : Prop :=
  wellFormed s'
  ∧ -- ensures #1
  (eval { world := s'.world, local' := s.local' } (.bin .eq (.place "Knoten" (.lit (.int 0)) "marke") (.lit (.int 0))) = some (.bool true))
...
def d8_threads_post (s s' : State) (r : Option Value) : Prop :=
  wellFormed s'
  ∧ -- ensures #1: NOT SAID (quantified-threads) -- a promise fewer makes a caller's goal harder, never wrong
  True
...
def d9_mappings_post (s s' : State) (r : Option Value) : Prop :=
  wellFormed s'
  ∧ -- ensures #1: NOT SAID (quantified-mappings) -- a promise fewer makes a caller's goal harder, never wrong
  True
