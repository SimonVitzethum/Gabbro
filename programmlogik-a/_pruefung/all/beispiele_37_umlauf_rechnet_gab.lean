...
-- REFUSED  ring_quadrieren  (record-value): a field of a record VALUE bound in the body (`let c = f(x); c.len`) -- this model's places are named by a record TYPE; pass the record through a `ptr` parameter
