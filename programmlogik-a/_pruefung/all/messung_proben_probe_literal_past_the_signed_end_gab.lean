/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/proben/probe-literal-past-the-signed-end.gab  total 0  goals 0  refused 0
...
namespace GabbroDuty.DutyProbeLiteralPastTheSignedEnd
...
def g_body : List Stmt :=
  [(.ret (some (.lit (.int 18446744073709551615))))]

/-- The precondition: the declared shapes and the `requires`. -/
def g_pre : Expr :=
  (.lit (.bool true))

def g_writes : List String := []
...
theorem g_meets : g_meets_statement := by
  ...
  gabbro_auto [g_body, g_pre, g_post, wellFormed, hall] using shapeOf
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
  ...
  have c_g : Contract ρ "g" g_requires g_post :=
    contract_of_duty ρ "g" g_body g_requires g_post r_g
      (fun t ht => d_g ρ t ht.1 ht.2)
  exact c_g

end GabbroDuty.DutyProbeLiteralPastTheSignedEnd