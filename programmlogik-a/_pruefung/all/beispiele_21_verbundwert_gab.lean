...
/-
  ...
  record-value (1): refused -- a field of a record VALUE bound in the body (`let c = f(x); c.len`) -- this model's places are named by a record TYPE; pass the record through a `ptr` parameter
    duty_1  V  laenge_von :: fertig requires #1
      (inherited: this clause HAS a term; the body or the `requires` of `laenge_von` has none)
...
/-
  duty_1  V  laenge_von :: fertig requires #1  --  refused (record-value)
-/
...
-- REFUSED  laenge_von  (record-value): a field of a record VALUE bound in the body (`let c = f(x); c.len`) -- this model's places are named by a record TYPE; pass the record through a `ptr` parameter
--   Its contract stands below all the same, so that a caller's theorem can name it;
--   what is missing is the theorem that discharges it.
