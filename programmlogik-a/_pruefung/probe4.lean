...
def shapeOf : Typing := fun p =>
  ...
  | .slot "Warteschlange" _ "frist" => some .int
  | .slot "Warteschlange" _ "naechst" => some .opt
  | .field "Manifest" "saetze" => some .int
  | .field "Manifest" "laenge" => some .int
  | _ => none
...
def faellige_wecken_body : List Stmt :=
  [(.loop "faellige_wecken#1" (.bin .and (.hasShape "jetzt" .int) (.bin .and (.bin .le (.lit (.int 1)) (.name "jetzt")) (.bin .le (.name "jetzt") (.lit (.int 1000000))))) [(.ite (.bin .le (.place "Warteschlange" (.name "eintrag") "frist") (.name "jetzt")) [(.assign "Warteschlange" (.name "eintrag") "aktiv" (.lit (.bool true)))] [])])]

/-- The precondition: the declared shapes and the `requires`. -/
def faellige_wecken_pre : Expr :=
  (.bin .and (.bin .and (.hasShape "jetzt" .int) (.bin .and (.bin .le (.lit (.int 1)) (.name "jetzt")) (.bin .le (.name "jetzt") (.lit (.int 1000000))))) (.lit (.bool true)))
...
theorem auf_quittung_warten_loop_1_keeps : auf_quittung_warten_loop_1_keeps_statement := by
  ...
  gabbro_auto [auf_quittung_warten_loop_1_body, auf_quittung_warten_loop_1_inv, wellFormed] using shapeOf
...
theorem auf_quittung_warten_meets : auf_quittung_warten_meets_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  gabbro_auto [auf_quittung_warten_body, auf_quittung_warten_pre, auf_quittung_warten_post, wellFormed, auf_quittung_warten_loop_1_inv] using shapeOf
...
theorem dienstschleife_loop_2_keeps : dienstschleife_loop_2_keeps_statement := by
  ...
  gabbro_auto [dienstschleife_loop_2_body, dienstschleife_loop_2_inv, wellFormed, h_Warteschlange_aktiv_e] using shapeOf
...
theorem dienstschleife_loop_1_keeps : dienstschleife_loop_1_keeps_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  gabbro_simp [dienstschleife_loop_1_body, dienstschleife_loop_1_inv, wellFormed, abschaltung_angefordert_pre, abschaltung_angefordert_requires, abschaltung_angefordert_post, abschaltung_angefordert_writes, Frame_read _ _ _ fr_abschaltung_angefordert, dienstschleife_loop_2_inv]
  clear c_abschaltung_angefordert fr_abschaltung_angefordert
  have hl := l_dienstschleife_loop_2 { world := t.world, local' := bindLocal t.local' "#pass" (.int k) } ?_ ?_
  trace_state
  all_goals sorry
  gabbro_calls shapeOf [dienstschleife_loop_1_body, dienstschleife_loop_1_inv, wellFormed, abschaltung_angefordert_pre, abschaltung_angefordert_requires, abschaltung_angefordert_post, abschaltung_angefordert_writes, Frame_read _ _ _ fr_abschaltung_angefordert, dienstschleife_loop_2_inv]
  trace_state
  all_goals sorry
...
def faellige_wecken_loop_1_inv : Expr :=
  (.bin .and (.hasShape "jetzt" .int) (.bin .and (.bin .le (.lit (.int 1)) (.name "jetzt")) (.bin .le (.name "jetzt") (.lit (.int 1000000)))))
...
theorem faellige_wecken_loop_1_keeps : faellige_wecken_loop_1_keeps_statement := by
  ...
  intro ρ t k hwf hinv
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_jetzt, e_jetzt, lo_jetzt, hi_jetzt⟩ := shape_int_in t "jetzt" _ _ hinv
  obtain ⟨n_Warteschlange_frist_eintrag, h_Warteschlange_frist_eintrag⟩ := WF_int shapeOf t.world (.slot "Warteschlange" k "frist") hwf rfl
  gabbro_auto [faellige_wecken_loop_1_body, faellige_wecken_loop_1_inv, wellFormed, e_jetzt, h_Warteschlange_frist_eintrag] using shapeOf
...
theorem faellige_wecken_meets : faellige_wecken_meets_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_jetzt, e_jetzt, lo_jetzt, hi_jetzt⟩ := shape_int_in s "jetzt" _ _ (and_left _ _ _ hpre)
  gabbro_auto [faellige_wecken_body, faellige_wecken_pre, faellige_wecken_post, wellFormed, faellige_wecken_loop_1_inv, e_jetzt] using shapeOf
...
theorem manifest_pruefen_loop_1_keeps : manifest_pruefen_loop_1_keeps_statement := by
  ...
  intro ρ t k hwf hinv
  simp only [wellFormed] at hwf ⊢
  gabbro_auto [manifest_pruefen_loop_1_body, manifest_pruefen_loop_1_inv, wellFormed] using shapeOf
...
theorem manifest_pruefen_meets : manifest_pruefen_meets_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  gabbro_auto [manifest_pruefen_body, manifest_pruefen_pre, manifest_pruefen_post, wellFormed, manifest_pruefen_loop_1_inv] using shapeOf
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoop ρ "dienstschleife#2" dienstschleife_loop_2_body "e"
  ...
  ∧ RunsLoop ρ "faellige_wecken#1" faellige_wecken_loop_1_body "eintrag"
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
  ...
  have l_dienstschleife_loop_2 : LoopRule ρ "dienstschleife#2" wellFormed dienstschleife_loop_2_inv :=
    looprule_of_body ρ "dienstschleife#2" wellFormed dienstschleife_loop_2_inv dienstschleife_loop_2_body "e" rl_dienstschleife_loop_2
      (fun t k hw hi => d_dienstschleife_loop_2 ρ t k hw hi)
  ...
  have l_faellige_wecken_loop_1 : LoopRule ρ "faellige_wecken#1" wellFormed faellige_wecken_loop_1_inv :=
    looprule_of_body ρ "faellige_wecken#1" wellFormed faellige_wecken_loop_1_inv faellige_wecken_loop_1_body "eintrag" rl_faellige_wecken_loop_1
      (fun t k hw hi => d_faellige_wecken_loop_1 ρ t k hw hi)
