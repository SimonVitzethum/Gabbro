...
def aufloesen_body : List Stmt :=
  [(.locked "KAPPEN" [(.call "fremd" ["z"] [(.name "z")] (.lit (.bool true))), (.retCall "liest" ["k", "i"] [(.name "k"), (.name "i")] (.bin .and (.bin .and (.hasShape "i" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "i")) (.bin .le (.name "i") (.lit (.int 63))))) (.lit (.bool true))))]), (.ret (some (.lit (.int 0))))]

/-- The precondition: the declared shapes and the `requires`. -/
def aufloesen_pre : Expr :=
  (.bin .and (.hasShape "i" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "i")) (.bin .le (.name "i") (.lit (.int 63)))))
...
/-- The precondition: the declared shapes and the `requires`. -/
def liest_pre : Expr :=
  (.bin .and (.bin .and (.hasShape "i" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "i")) (.bin .le (.name "i") (.lit (.int 63))))) (.lit (.bool true)))
...
theorem fremd_meets : fremd_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  obtain ⟨n_Zaehlwerk_wert, h_Zaehlwerk_wert⟩ := WF_int shapeOf s.world (.field "Zaehlwerk" "wert") hwf rfl
  have hall := hpre
