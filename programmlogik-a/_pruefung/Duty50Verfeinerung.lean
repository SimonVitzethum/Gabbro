...
def freigeben_pre : Expr :=
  (.bin .and (.hasShape "p" .int) (.bin .and (.lit (.bool true)) (.place "Buch" (.name "p") "belegt")))
...
theorem freigeben_meets : freigeben_meets_statement := by
  ...
  obtain ⟨w_p, e_p⟩ := shape_int s "p" (and_left _ _ _ hpre)
  obtain ⟨n_Buch_belegt_p, h_Buch_belegt_p⟩ := WF_bool shapeOf s.world (.slot "Buch" w_p "belegt") hwf rfl
  gabbro_auto [freigeben_body, freigeben_pre, freigeben_post, wellFormed, e_p, h_Buch_belegt_p] using shapeOf
