...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Endpunkte" _ "wartet" => some (.intIn 0 4294967295)
  | .slot "Faeden" _ "gruende" => some (.intIn 0 4294967295)
...
def einreihen_pre : Expr :=
  (.bin .and (.hasShape "e" (.intIn 0 63)) (.bin .and (.hasShape "t" (.intIn 0 255)) (.forallSlots "e" 64 (.bin .gt (.place "Faeden" (.place "Endpunkte" (.name "e") "wartet") "gruende") (.lit (.int 0))))))
...
theorem einreihen_meets : einreihen_meets_statement := by
  ...
  obtain ⟨w_e, e_e, lo_e, hi_e⟩ := shape_intIn s "e" _ _ (and_left _ _ _ hpre)
  obtain ⟨w_t, e_t, lo_t, hi_t⟩ := shape_intIn s "t" _ _ (and_left _ _ _ (and_right _ _ _ hpre))
  have hall := hpre
  gabbro_simp_at hall [einreihen_pre, e_e, e_t]
  gabbro_auto [einreihen_body, einreihen_pre, einreihen_post, wellFormed, einreihen_inv_wartende_haben_grund, e_e, e_t, hall] using shapeOf
