...
def aktive_zaehlen_body : List Stmt :=
  [(.bindName "n" (.lit (.int 0))), (.loop "aktive_zaehlen#1" (.bin .and (.hasShape "n" (.intIn 0 16)) (.bin .le (.name "n") (.lit (.int 16)))) [(.ite (.place "Werte" (.name "i") "aktiv") [(.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))] [])]), (.ret (some (.name "n")))]
...
def aktive_zaehlen_loop_1_inv : Expr :=
  (.bin .and (.hasShape "n" (.intIn 0 16)) (.bin .le (.name "n") (.lit (.int 16))))

def aktive_zaehlen_loop_1_body : List Stmt :=
  [(.ite (.place "Werte" (.name "i") "aktiv") [(.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))] [])]
...
theorem aktive_zaehlen_loop_1_keeps : aktive_zaehlen_loop_1_keeps_statement := by
  ...
  obtain ⟨n_Werte_aktiv_i, h_Werte_aktiv_i⟩ := WF_bool shapeOf t.world (.slot "Werte" k "aktiv") hwf rfl
  have hall := hinv
  gabbro_simp_at hall [aktive_zaehlen_loop_1_inv, e_n, h_Werte_aktiv_i]
  gabbro_auto [aktive_zaehlen_loop_1_body, aktive_zaehlen_loop_1_inv, wellFormed, e_n, h_Werte_aktiv_i, hall] using shapeOf
...
def aktive_zaehlen_meets_statement : Prop :=
  ∀ (ρ : Env) (s : State)
    ...
    (l_aktive_zaehlen_loop_1 : LoopRule ρ "aktive_zaehlen#1" wellFormed aktive_zaehlen_loop_1_inv),
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoopIn ρ "aktive_zaehlen#1" aktive_zaehlen_loop_1_body "i" 0 16
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    ...
    ∧ LoopRule ρ "aktive_zaehlen#1" wellFormed aktive_zaehlen_loop_1_inv
  ...
  have l_aktive_zaehlen_loop_1 : LoopRule ρ "aktive_zaehlen#1" wellFormed aktive_zaehlen_loop_1_inv :=
    looprule_of_body_in ρ "aktive_zaehlen#1" wellFormed aktive_zaehlen_loop_1_inv aktive_zaehlen_loop_1_body "i" 0 16 rl_aktive_zaehlen_loop_1
      (fun t k hlo hhi hw hi => d_aktive_zaehlen_loop_1 ρ t k hlo hhi hw hi)
