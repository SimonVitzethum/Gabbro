...
# **Der Lean-Riegel steht am PUSH, nicht am Commit** (Ordner, 2026-09-15).
#
# Er stand einen Abend lang hier und war in beide Richtungen gemessen (ein eingepflanztes
# `1 = 2` wurde abgefangen, der gruene Baum durchgelassen). Verschoben, weil der Commit die
# falsche Stelle ist: er kostet bei jeder Dokumentzeile einen Bau, und er schuetzt niemanden
# -- ein roter Commit auf einem Zweig schadet nur dem Zweig. **Der Push ist der Moment, in
# dem etwas andere erreicht**, und dort steht der Riegel jetzt:
# `.claude/muse-arbeit/pruef-push.sh` verlangt `lean=0` neben `ct=0`, `em=0`, `failed=0` und
# unterscheidet dabei ROT von NICHT GEMESSEN.
#
# *Was hier bleibt, ist die Lehre, aus der er entstand:* zweimal an einem Tag war ein Merge
# einzeln gruen und im Zusammentreffen rot, und beide Male hat es NUR der Bau gezeigt.
