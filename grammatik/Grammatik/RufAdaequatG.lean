...
inductive EndG {V : Vertrag D} (A : D.Lock → Prop) :
    {Γ : Ctx} → {Λ : List (Res D)} → Endblock D V false Γ Λ → Prop where
...
theorem BlockG.regLiesElse_inv {Λ Λ' : List (Res D)} {r : D.Reg}
    ...
    mr = true ∧ BlockG A true rest ∧ ∃ hl : l = false, EndG A (hl ▸ sonst) := by
  cases h with
  | regLiesElse _ _ _ _ _ hs hr => exact ⟨rfl, hr, rfl, hs⟩
...
theorem BlockG.narrow_inv {Λ Λ' : List (Res D)} {lo hi : Int}
    ...
    mr = true ∧ BlockG A true rest ∧ ∃ hl : l = false, EndG A (hl ▸ sonst) := by
  cases h with
  | narrow _ _ _ _ _ hs hr => exact ⟨rfl, hr, rfl, hs⟩
...
theorem BlockG.pruefung_inv {Λ Λ' : List (Res D)} {c : Expr D Γ Λ .bool}
    ...
    mr = true ∧ BlockG A true rest ∧ ∃ hl : l = false, EndG A (hl ▸ sonst) := by
  cases h with
  | pruefung _ _ _ hs hr => exact ⟨rfl, hr, rfl, hs⟩
...
theorem BlockG.gleitNarrow_inv {Λ Λ' : List (Res D)} {l1 h1 : Int × Int}
    ...
    mr = true ∧ BlockG A true rest ∧ ∃ hl : l = false, EndG A (hl ▸ sonst) := by
  cases h with
  | gleitNarrow _ _ _ _ _ hs hr => exact ⟨rfl, hr, rfl, hs⟩
...
theorem EndG.cons_inv {Γ : Ctx} {Λ Λ' : List (Res D)} {s : Stmt D V false Γ Λ Λ'}
    {rest : Endblock D V false Γ Λ'} (h : EndG A (.cons s rest)) :
    StmtG A true s ∧ EndG A rest := by
  cases h with
  | cons _ _ hs hr => exact ⟨hs, hr⟩

theorem EndG.bind_inv {Γ : Ctx} {Λ : List (Res D)} {τ : Ty} {e : Expr D Γ Λ τ}
    {rest : Endblock D V false (τ :: Γ) Λ} (h : EndG A (.bind e rest)) : EndG A rest := by
  cases h with
  | bind _ _ hr => exact hr
