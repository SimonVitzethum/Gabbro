/-
  ...
  for that call (`rueck_ohne_nachbedingung_iff`). Both directions are proved
...
theorem rueck_ohne_nachbedingung_iff (P : Programm D) (O : Orakel D)
    (passes : Nat)
    (fuel : Nat) (f : D.Fn) (sigma : World D) (rho : Env D (D.params f))
    (sigma1 : World D) (v : ErgVal D (D.erg f))
    (sinv : World D)
...
/-! ## CUTS
  - `rueck_ohne_nachbedingung_iff`: the proved half is that an `.ok` outcome
...
#print axioms Gabbro.Grammatik.rueck_ohne_nachbedingung_iff
