/-
  ...
  * §4 Refusals: two declared starts writing one unguarded table
    (`ak1_zwei_schreiber`), and a lock-floor violation
    (`ak2_boden_falsch`); in both, every OTHER component holds, so the named
    one is what bites.
-/
...
theorem akzeptiert_mehrfaden (P : Programm D) (O : Orakel D) (passes : Nat) (Q : AxEns D)
    (S : SperrInv D) (fs : List D.Fn) (ls : List D.Lock) (ws : List D.Fn) (sp : Speicher D)
    (init : Faden → Σ f : D.Fn, Env D (D.params f)) (e0 : Ereignis D)
    (hvoll : ∀ g : D.Fn, g ∈ fs) (hls : ∀ L : D.Lock, L ∈ ls)
    (hA : Akzeptiert P S fs ls ws = true) (hZ : StartZulaessig P S fs ws sp init)
  ...
  obtain ⟨hFrag, hAbg, hW, hFuss, hSO, hStart, hSs, hex, -, -, -⟩ := Akzeptiert_ok hvoll hls hA hZ
  exact ziel_ort_mehrfaden_ende P O passes Q S fs sp init e0 (kVon P fs init) hO hRL hQ hlok
...
theorem akzeptiert_verklemmung {P : Programm D} {O : Orakel D} {passes : Nat} {S : SperrInv D}
    {fs : List D.Fn} {ls : List D.Lock} {ws : List D.Fn} {sp : Speicher D}
    {init : Faden → Σ f : D.Fn, Env D (D.params f)}
    (hvoll : ∀ g : D.Fn, g ∈ fs) (hls : ∀ L : D.Lock, L ∈ ls)
    (hA : Akzeptiert P S fs ls ws = true) (hZ : StartZulaessig P S fs ws sp init) (hO : GutO O)
    {M : RufMaschineG D} (hr : RufErreichbarG P O passes (RufStartG P sp init) M)
    (hW : ∀ t, ¬ FertigG M t → WartetG M t) : ∀ t, FertigG M t := by
  obtain ⟨-, -, -, -, -, -, -, -, hSt, hLeer, -⟩ := Akzeptiert_ok hvoll hls hA hZ
  exact keine_verklemmungG hO hSt sp init hLeer ls hls hr hW

/-- **`rennfrei_g_voll` from `Akzeptiert`.** -/
theorem akzeptiert_rennfrei {P : Programm D} {O : Orakel D} {passes : Nat} {S : SperrInv D}
    ...
    (hjn : j < n) (hfg : ts i ≠ ts j) (c : D.Tab ⊕ D.Glob) (L : D.Lock) (hB : Bewacht c L)
    (hzi : ZugriffG (ms i) (ms (i + 1)) (ts i) c)
    (hzj : ZugriffG (ms j) (ms (j + 1)) (ts j) c) : GeordnetG ms ts L i j := by
  obtain ⟨-, -, -, -, -, -, -, hex, -, -, -⟩ := Akzeptiert_ok hvoll hls hA hZ
  exact rennfrei_g_voll P O passes sp init hO hex ms ts n hl i j hij hjn hfg c L hB hzi hzj

/-- **`frame_schritte_beschraenkt` from `Akzeptiert`**: every frame of every
    function, at call depth `fs.length + 1`. Needs no start. -/
theorem akzeptiert_zeit {P : Programm D} {S : SperrInv D} {fs : List D.Fn} {ls : List D.Lock}
    {ws : List D.Fn} (hvoll : ∀ g : D.Fn, g ∈ fs) (hls : ∀ L : D.Lock, L ∈ ls)
    (hA : Akzeptiert P S fs ls ws = true) (O : Orakel D) (passes : Nat) (f : Faden) (g : D.Fn)
    ...
    segZaehle run f ≤ kostenTief P passes (fs.length + 1) g :=
  frame_schritte_beschraenkt P O passes f g fs.length ((akzeptiertP_of hvoll hls hA).zeit g) hE
    run hAk
...
/-- **`Akzeptiert` holds on the two-thread fixture**, its declared starts
    being `hauptA` and `hauptB`. -/
theorem mP_akzeptiert : Akzeptiert mP mSI mFs [()] [mHauptA, mHauptB] = true := by decide

/-- `mInit` as a start table: thread 0 `hauptA`, thread 1 `hauptB`, every
    other thread `ruhe`. -/
def mTafel : StartTafel mD := ⟨[⟨mHauptA, .nil⟩, ⟨mHauptB, .nil⟩], ⟨mRuhe, .nil⟩⟩

theorem mTafel_start : startB mP mSI mFs [()] [mHauptA, mHauptB] mTafel mSp = true := by decide
...
theorem mP_startZulaessig : StartZulaessig mP mSI mFs [mHauptA, mHauptB] mSp mInit := by
  rw [← mTafel_init]
  exact startB_ok mLocks_voll mTafel_start
...
theorem mP_ziel_aus_akzeptiert : ∀ M : RufMaschineG mD,
  ...
  akzeptiert_mehrfaden mP mO 0 (axWahr mD) mSI mFs [()] [mHauptA, mHauptB] mSp mInit mE0
    mFs_voll mLocks_voll mP_akzeptiert mP_startZulaessig mSI_ok.2 axEnsLokal_wahr mP_koerper
    mP_inv mO_gut mO_lokal (axVertragO_wahr mO)

theorem mP_verklemmung_aus_akzeptiert : ∀ M : RufMaschineG mD,
    RufErreichbarG mP mO 0 (RufStartG mP mSp mInit) M →
      (∀ t, ¬ FertigG M t → WartetG M t) → ∀ t, FertigG M t :=
  fun _ hr hW => akzeptiert_verklemmung mFs_voll mLocks_voll mP_akzeptiert mP_startZulaessig
    mO_gut hr hW
...
theorem mTafel_doppelt_falsch :
    startB mP mSI mFs [()] [mHauptA, mHauptB]
...
theorem gP_akzeptiert : Akzeptiert x104P x104S x104Fs x104Ls [] = true := by decide

/-- With its two functions as starts the export is refused, and only by
    the start component: both hold `M` by signature. -/
theorem gP_als_starts :
    Akzeptiert x104P x104S x104Fs x104Ls x104Fs = false ∧ wurzelnB x104Fs = false ∧
    (programmImFragmentG x104P x104Fs && abgAlleB x104P x104Fs &&
      ...
      zeitB x104P x104Fs) = true := by
  decide

/-- **No start is admitted for the export**: both functions hold `M` by
    signature, so neither is idle, and with no declared start thread `0`
    has nothing to run. `GabbroZiel` says nothing about the export until it
    carries a driver and an idle function (Export104 CUT 2). -/
theorem gP_start_leer (sp : Speicher x104D)
  ...
  intro h
  have hr : ∀ f : x104D.Fn, ruheB x104P x104Fs f = false := by
    intro f; cases f <;> decide
  rcases h.passt.wurzel 0 with h0 | h0
  · exact absurd h0 List.not_mem_nil
  · rw [hr] at h0; cases h0
...
/-- **An unguarded shared write is refused**: `a` and `b` are both declared
    starts and both write the unguarded table `t`, so `t` is in `a`'s
    footprint while another start writes it. Every other component holds:
    the footprint check alone refuses. -/
theorem ak1_zwei_schreiber :
    Akzeptiert akP1 akS akFs [()] [akA, akB] = false ∧
    fussWB akP1 akS akFs [akA, akB] = false ∧
    (programmImFragmentG akP1 akFs && abgAlleB akP1 akFs && stufenB akP1 akFs &&
      sperrOrteB akS [()] && wurzelnB [akA, akB] && zeitB akP1 akFs) = true := by
  decide
...
theorem ak1_ein_schreiber :
    Akzeptiert akP1 akS akFs [()] [akA] = true ∧
    startB akP1 akS akFs [()] [akA] ⟨[⟨akA, .nil⟩], ⟨akRuhe, .nil⟩⟩ akSp = true ∧
    startB akP1 akS akFs [()] [akA] ⟨[⟨akA, .nil⟩, ⟨akB, .nil⟩], ⟨akRuhe, .nil⟩⟩ akSp = false ∧
    startB akP1 akS akFs [()] [akA] ⟨[⟨akA, .nil⟩, ⟨akA, .nil⟩], ⟨akRuhe, .nil⟩⟩ akSp = false := by
  decide
