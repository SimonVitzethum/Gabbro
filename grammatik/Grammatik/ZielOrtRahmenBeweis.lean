...
def StapelR : (Σ f : D.Fn, Env D (D.params f) × World D) → List (RufRahmenG D) → Prop
  | _, [] => True
  | G, F :: rest => WarteR P O passes F G ∧ StapelR (RufSchluesselG F) rest

def FadenR (z : RufFadenG D) (W : World D) : Prop :=
  KopfR P O passes z.kopf W ∧ StapelR P O passes (RufSchluesselG z.kopf) z.stapel

def ZielInvR (M : RufMaschineG D) : Prop :=
  (∀ t, FadenR P O passes (M.faeden t) (M.weltVon t)) ∧ ∀ t, LogOk P (M.faeden t).log
...
theorem kopfR_okG {F : RufRahmenG D} {W : World D} (h : KopfR P O passes F W)
...
/-- **A head-local step without a new record** keeps the replay: the
    ...
    refines the old one. -/
theorem fadenR_lokalQ {z : RufFadenG D} {W W' : World D} (hF : FadenR P O passes z W)
    {l : Bool} {Γ : Ctx} {Λ : List (Res D)} {ρ : Env D Γ}
...
/-- `fadenR_lokalQ` for a step whose frame semantics holds for every
    sequential oracle. -/
theorem fadenR_lokal {z : RufFadenG D} {W W' : World D} (hF : FadenR P O passes z W)
    {l : Bool} {Γ : Ctx} {Λ : List (Res D)} {ρ : Env D Γ}
    {r : GRest D (vertragVon D z.kopf.f) l Γ Λ} (hr : z.kopf.rest = ⟨l, Γ, Λ, ρ, r⟩)
    {l' : Bool} {Γ' : Ctx} {Λ' : List (Res D)} (ρ' : Env D Γ')
    (r' : GRest D (vertragVon D z.kopf.f) l' Γ' Λ') (spur' : List (Ereignis D))
...
theorem fadenR_speicher {z : RufFadenG D} {W W' : World D} (hF : FadenR P O passes z W)
    (hw : ∀ c ∈ fussOrteG P z.kopf.f, TraegerGleich W'.speicher W.speicher c) :
    FadenR P O passes z W' := by
  obtain ⟨⟨H, HA, σ, hreq, hf, hv, hk, hfa, hra, hka, hg, hok, heq⟩, hS⟩ := hF
...
/-- **The return leg.** -/
theorem popR_ens (hO : GutO O) (hRL : RegLokal O) (hK : ∀ f, KoerperGutR P passes f) {G : RufRahmenG D}
    {W : World D} (hG : KopfR P O passes G W) {lr : Bool} {Γ : Ctx} {Λ : List (Res D)}
...
/-- **The call leg.** -/
theorem pushR_req (hO : GutO O) (hRL : RegLokal O) (hK : ∀ f, KoerperGutR P passes f) {F : RufRahmenG D}
    {H : List (EintragV D)} {HA : List (AxEintrag D)} {σ : World D}
...
theorem popR_kopf (e0 : Ereignis D) {F : RufRahmenG D}
    ...
    (hW : WarteR P O passes F G) (s1 : World D) (mk : World D → RufAusgang G.1)
    ...
    KopfR P O passes ⟨F.f, F.rho, F.s0, ⟨l, Γ, Λ, ρ', r'⟩⟩ W' := by
  obtain ⟨H, HA, κ, hreq, hf, hv, hk, hfa, hra, hka, hok, hreqκ, ⟨hglκ, hsub, hfussκ⟩, hcont⟩ := hW
  ...
  refine ⟨H ++ [⟨G.1, κ, G.2.1, mk (rahmenWelt e0 G.1 κ s1)⟩], HA, rahmenWelt e0 G.1 κ s1, hreq,
    funkV_append hf hk _ (Nat.le_refl _), ⟨?_, ?_⟩, ?_, hfa, hra,
...
theorem pushR_ok (hO : GutO O) (hRL : RegLokal O) (hK : ∀ f, KoerperGutR P passes f)
    (hFrag : ∀ f, (P.rumpf f).gOk (kandP P (fussOrteG P f)) (regP (fussOrteG P f)) = true)
    {z : RufFadenG D} {W : World D} (hF : FadenR P O passes z W)
    {l : Bool} {Γ : Ctx} {Λ : List (Res D)} {ρ : Env D Γ}
    {r : GRest D (vertragVon D z.kopf.f) l Γ Λ} (hr : z.kopf.rest = ⟨l, Γ, Λ, ρ, r⟩)
  ...
  have hreqκ := pushR_req hO hRL hK hreq hf hv hfa hra r heq' g (σ.lese Λ args.orte)
...
theorem pushR_gen (hO : GutO O) (hRL : RegLokal O) (hK : ∀ f, KoerperGutR P passes f)
    (hFrag : ∀ f, (P.rumpf f).gOk (kandP P (fussOrteG P f)) (regP (fussOrteG P f)) = true)
    {z : RufFadenG D} {W : World D} (hF : FadenR P O passes z W)
    {l : Bool} {Γ : Ctx} {Λ : List (Res D)} {ρ : Env D Γ}
    {r : GRest D (vertragVon D z.kopf.f) l Γ Λ} (hr : z.kopf.rest = ⟨l, Γ, Λ, ρ, r⟩)
  ...
  have hreqκ := pushR_req hO hRL hK hreq hf hv hfa hra r heq' g (σ.lese Λ os) (rhoF (σ.lese Λ os))
...
/-- **An axiom step keeps the replay.** The answer of the machine's oracle
    ...
    position. -/
theorem fadenR_ax (e0 : Ereignis D) {z : RufFadenG D} {W : World D} (hF : FadenR P O passes z W)
    {l : Bool} {Γ : Ctx} {Λ : List (Res D)} {ρ : Env D Γ}
    {r : GRest D (vertragVon D z.kopf.f) l Γ Λ} (hr : z.kopf.rest = ⟨l, Γ, Λ, ρ, r⟩)
    {l' : Bool} {Γ' : Ctx} {Λ' : List (Res D)} (ρ' : Env D Γ')
    (r' : GRest D (vertragVon D z.kopf.f) l' Γ' Λ') (spur' : List (Ereignis D))
...
theorem fadenR_blatt (e0 : Ereignis D) (hO : GutO O) {z : RufFadenG D} {W : World D}
    (hF : FadenR P O passes z W) {l : Bool} {Γ : Ctx} {Λ Λ' : List (Res D)} {ρ : Env D Γ}
    {r : GRest D (vertragVon D z.kopf.f) l Γ Λ} (hr : z.kopf.rest = ⟨l, Γ, Λ, ρ, r⟩)
