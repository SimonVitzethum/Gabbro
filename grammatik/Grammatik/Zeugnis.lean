/-
  ...
  CUTS (shrunk, not faked -- each is a constructor this file does NOT cover)
    CUT-1 signed division and remainder (`sdiv`/`srem`, the second theorem).
    CUT-2 bitwise and shifts (`band`/`bor`/`bxor`/`shl`/`shr`, width proofs).
    CUT-3 floats, options, sums, grounds, quantifiers, `reaches`.
...
/-- The range table, recomputed on the Lean side: M104 for the shapes,
    M102 (`0 ≤ l1`, `1 ≤ l2`) for `/` and `%`. `none` means the print has
    no range at all -- a forged side condition lands here, not in a diagnostic.
