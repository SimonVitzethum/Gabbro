/-
  ...
  `Qk -> Qu`, `Ek ⊆ Eu` and the same register map, the oracle premise for
  that `Ax` holds in `GutO`-style per-axiom shape (frame + decoded
  contract) -- the syscall needs no named assumption.
-/
...
/-- The oracle premise for ONE axiom, `GutO`-style: the frame of the
    declared user effects, unchanged locks and trace, plus the decoded
...
def PaarGut (D : Deklaration) (a : D.Ax) (O : Orakel D)
  ...
  ∀ σ ρ, Rahmen u.Eu u.EuG σ (O.wirkt a σ ρ).1 ∧
    (O.wirkt a σ ρ).1.haelt = σ.haelt ∧
    (O.wirkt a σ ρ).1.spur = σ.spur ∧
    (u.Pu σ ρ → u.Qu σ (O.wirkt a σ ρ).1
      (antwortInt (dekodiere u.tab u.lo u.hi (O.wirkt a σ ρ).2)) ρ)

/-- The pairing theorem (PLAN-SYSCALL.md section 2, item 4): register maps
    agree, `Pu -> Pk`, `Qk -> Qu` after decoding, `Ek ⊆ Eu`, and the
    kernel entry's contract holds for its implementation `K` -- then the
    oracle premise holds for that `Ax` in per-axiom `GutO` shape, so the
    syscall needs no named assumption. Every premise is load-bearing:
    `hnum`/`habi` transport the kernel behaviour to the user dispatch
    identity, `hreq` discharges the kernel precondition, `hens` the user
    postcondition, `hsub`/`hsubG` widen the frame, `hKeff`/`hKcon` supply
    the kernel frame and postcondition, `hO` ties the oracle to `K`. -/
theorem syscall_paarung (D : Deklaration) (a : D.Ax) (O : Orakel D)
    ...
    (hKeff : ∀ σ ρ σ' roh, K k.nummer k.abi σ ρ σ' roh →
      Rahmen k.Ek k.EkG σ σ' ∧ σ'.haelt = σ.haelt ∧ σ'.spur = σ.spur)
  ...
  obtain ⟨hfr, hha, hsp⟩ := hKeff σ ρ σ' roh hKbeh
  rw [hOeq]
  dsimp only
  refine ⟨hfr.weiter hsub hsubG, hha, hsp, fun hPu => ?_⟩
...
/-- The kernel dispatch entry for number 1: requires the descriptor slot
    ...
    bumped, or EBADF (`-9`) leaving the world alone; writes the table. -/
def swK : KernelEintrag swD (swD.aparams ()) where
  ...
  Qk := fun σ σ' roh ρ =>
    (0 ≤ roh ∧ roh ≤ swLen ρ ∧
      σ' = σ.storeSlot () (swFd ρ) () ⟨1, by decide, by decide⟩) ∨
    (roh = -9 ∧ σ' = σ)
  Ek := fun _ => true
  EkG := fun g => nomatch g
...
/-- The oracle answering through the kernel: bump the offset slot,
    answer count `0` (always `≤ len`). Locks and trace are untouched. -/
def swO : Orakel swD where
  wirkt := fun _ σ ρ =>
    (σ.storeSlot () (swFd ρ) () ⟨1, by decide, by decide⟩, 0)
...
/-- The implementation keeps the kernel frame, locks, and trace. -/
theorem swEff : ∀ σ ρ σ' roh, swKbeh swK.nummer swK.abi σ ρ σ' roh →
    Rahmen swK.Ek swK.EkG σ σ' ∧ σ'.haelt = σ.haelt ∧ σ'.spur = σ.spur :=
  fun _ _ _ _ h => ⟨h.2.2.2.1, h.2.2.2.2.1, h.2.2.2.2.2⟩
...
/-- ZEUGE: all eight pairing premises hold jointly for the toy `write`
...
theorem syscall_paarung_zeuge :
  ...
  ⟨syscall_paarung swD () swO swK swU swKbeh swNum swAbi swReq swEns
      swSub swSubG swEff swCon swO_trifft,
    rfl,
    swWelt0,
    swWelt0.storeSlot () 0 ()
...
/- CUTS: what is not proved.
   - The kernel implementation `K` is a hypothesis (`hKeff`/`hKcon`),
     ...
     proving the dispatch entry's contract over its runs.
   - `syscall_paarung` covers the contract/frame pairing only. The
