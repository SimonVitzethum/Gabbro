/-
  ...
  REVIEWER NOTES (merge into master, 2026-09-12):
  * The line below once said "one `sorry` remains"; that is stale. Checked at merge:
    all six theorems depend only on [propext, Classical.choice, Quot.sound].
  * Scope: `kette_zwei_aus_lauf` covers LOCK-ONLY programs (`hlock_prog`: every atom is
    `take`/`rel`). Such programs never change memory, so the frame closes by `rfl`.
  * `kette_aus_deckung` / `deckung_strikt_schwaecher` do not advance the general case:
    `KettenSpurDeckung` is `exists J, hJw /\ hJsf` at every prefix (rule 4a), and the
    strictness certificate only shows that a DIFFERENT chain breaks the equations.

  What is proved (no `admit`, no `axiom`, no `sorry`):
