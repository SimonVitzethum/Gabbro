-- GENERATED by `gabbro obligations --g beispiele/130-derived-contract-pure.gab` (lean-g export plus the stated obligation) -- do not edit.
...
namespace G130_derived_contract_pure_oblig
...
def gSig_add : Signatur GTab Empty GLock Empty where
  params := [(.int 0 1000), (.int 0 1000)]
  erg := some ((.int 0 2000))
  gruende := 0
  haelt := []
  boden := none
...
abbrev gD : Deklaration where
  ...
  Fn := GFn
  sig := fun | .add => 0
  sigNr := fun | 0 => gSig_add | _ => gSig_add
...
abbrev gCtx_add : Ctx := [(.int 0 1000), (.int 0 1000)]
def g_add : gD.Fn := GFn.add

abbrev gL_add : List (Res gD) := []


def gBody_add : Endblock gD (vertragVon gD g_add) false gCtx_add gL_add :=
  (.ret (.wert (.add (.var .hier) (.var (.dort .hier)))) (List.Perm.refl _))
...
end G130_derived_contract_pure_oblig
