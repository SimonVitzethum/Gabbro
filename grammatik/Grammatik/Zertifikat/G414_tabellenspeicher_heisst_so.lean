-- GENERATED by `gabbro obligations --g beispiele/gift/414-tabellenspeicher-heisst-so.gab` (lean-g export plus the stated obligation) -- do not edit.
...
namespace G414_tabellenspeicher_heisst_so_oblig
...
def gSig_beruehren : Signatur GTab GGlob GLock Empty where
  params := [.index 8]
  erg := some (.bool)
  gruende := 0
  haelt := []
...
abbrev gD : Deklaration where
  ...
  count := fun | .Kappe => 8
  Feld := fun | .Kappe => GKappeFeld
  decFeld := fun | .Kappe => inferInstance
  typ := fun | .Kappe, .benutzt => (.bool)
  erlaubt := fun _ _ _ _ => false
  tabNr := fun | 0 => some GTab.Kappe | _ => none
  Glob := GGlob
  ...
  braucht := fun | .Kappe => []
  gbraucht := fun | .Kappe_speicher => []
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .beruehren => 0
  sigNr := fun | 0 => gSig_beruehren | _ => gSig_beruehren
...
abbrev gCtx_beruehren : Ctx := [.index 8]
def g_beruehren : gD.Fn := GFn.beruehren

abbrev gL_beruehren : List (Res gD) := []

theorem gDarf_beruehren_Kappe : darf gD GTab.Kappe gL_beruehren := by unfold darf; decide

def gBody_beruehren : Endblock gD (vertragVon gD g_beruehren) false gCtx_beruehren gL_beruehren :=
  (.ret (.wert (Expr.slot (D := gD) GTab.Kappe GKappeFeld.benutzt ((.var .hier)) gDarf_beruehren_Kappe)) (List.Perm.refl _))
...
def gP : Programm gD where
  ...
  ensures
    | .beruehren => .wahr
  rumpf
    | .beruehren => gBody_beruehren

def gFs : List gD.Fn := [g_beruehren]

def gLs : List gD.Lock := []

def gCs : List (gD.Tab ⊕ gD.Glob) := [(.inl GTab.Kappe), (.inr GGlob.Kappe_speicher)]
...
def gSp0 : Speicher gD :=
  ⟨(fun t _ f => match t, f with | .Kappe, .benutzt => false), (fun | .Kappe_speicher => ⟨0, by decide, by decide⟩)⟩
...
end G414_tabellenspeicher_heisst_so_oblig
