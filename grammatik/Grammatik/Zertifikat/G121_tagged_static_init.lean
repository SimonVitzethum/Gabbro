-- GENERATED by `gabbro obligations --g beispiele/121-tagged-static-init.gab` (lean-g export plus the stated obligation) -- do not edit.
...
def gSig_lies : Signatur GTab GGlob GLock Empty where
  params := []
...
abbrev gD : Deklaration where
  ...
  gbraucht := fun | .ANFANG => [] | .LEERE => []
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .lies => 0
  sigNr := fun | 0 => gSig_lies | _ => gSig_lies
...
abbrev gCtx_lies : Ctx := []
def g_lies : gD.Fn := GFn.lies

abbrev gL_lies : List (Res gD) := []

theorem gGDarf_lies_ANFANG : gdarf gD GGlob.ANFANG gL_lies := by unfold gdarf; decide

def gBody_lies : Endblock gD (vertragVon gD g_lies) false gCtx_lies gL_lies :=
  (.cons (.onTag (Expr.glob (D := gD) GGlob.ANFANG gGDarf_lies_ANFANG) (.cons (.cons (.ret (.wert ((.weiter (by decide) (by decide) (((.lit 0)) : Expr gD gCtx_lies gL_lies (.int 0 0))) : Expr gD gCtx_lies gL_lies (.int 0 4294967295))) (List.Perm.refl _)) .nil) (.cons (.cons (.ret (.wert (.var .hier)) (List.Perm.refl _)) .nil) .nil))) (.ret (.wert ((.weiter (by decide) (by decide) (((.lit 0)) : Expr gD gCtx_lies gL_lies (.int 0 0))) : Expr gD gCtx_lies gL_lies (.int 0 4294967295))) (List.Perm.refl _)))
...
def gFs : List gD.Fn := [g_lies]

def gLs : List gD.Lock := []

def gCs : List (gD.Tab ⊕ gD.Glob) := [(.inr GGlob.ANFANG), (.inr GGlob.LEERE)]
...
def gSp0 : Speicher gD :=
  ⟨(fun t => nomatch t), (fun | .ANFANG => ⟨⟨1, by decide⟩, ⟨5, by decide, by decide⟩⟩ | .LEERE => ⟨⟨0, by decide⟩, ()⟩)⟩
...
end G121_tagged_static_init_oblig
