-- GENERATED by `gabbro obligations --g beispiele/gift/701-rein-ohne-klausel.gab` (lean-g export plus the stated obligation) -- do not edit.
...
-- where exportable).
--
-- fn rein_ohne_klausel (holds; writes)
...
namespace G701_rein_ohne_klausel_oblig
...
def gSig_rein_ohne_klausel : Signatur GTab Empty GLock Empty where
...
abbrev gD : Deklaration where
  ...
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .rein_ohne_klausel => 0
  sigNr := fun | 0 => gSig_rein_ohne_klausel | _ => gSig_rein_ohne_klausel
...
abbrev gCtx_rein_ohne_klausel : Ctx := [(.int 0 4294967295)]
def g_rein_ohne_klausel : gD.Fn := GFn.rein_ohne_klausel

abbrev gL_rein_ohne_klausel : List (Res gD) := []


def gBody_rein_ohne_klausel : Endblock gD (vertragVon gD g_rein_ohne_klausel) false gCtx_rein_ohne_klausel gL_rein_ohne_klausel :=
  (.ret (.wert (.var .hier)) (List.Perm.refl _))

def gP : Programm gD where
  invariante := fun i => nomatch i
  requires
    | .rein_ohne_klausel => .wahr
  ensures
    | .rein_ohne_klausel => .wahr
  rumpf
    | .rein_ohne_klausel => gBody_rein_ohne_klausel

def gFs : List gD.Fn := [g_rein_ohne_klausel]
...
end G701_rein_ohne_klausel_oblig
