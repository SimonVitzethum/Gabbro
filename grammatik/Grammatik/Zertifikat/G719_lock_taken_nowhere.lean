-- GENERATED by `gabbro obligations --g beispiele/gift/719-lock-taken-nowhere.gab` (lean-g export plus the stated obligation) -- do not edit.
...
-- where exportable).
--
-- table 0: T (count 8), x : 0..4294967295
-- lock 0: L (rank 0) guards T
-- fn tut_nichts (holds; writes)
...
namespace G719_lock_taken_nowhere_oblig
...
def gSig_tut_nichts : Signatur GTab Empty GLock Empty where
  params := []
...
abbrev gD : Deklaration where
  ...
  braucht := fun | .T => [.inl GLock.L]
  gbraucht := fun e => nomatch e
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .tut_nichts => 0
  sigNr := fun | 0 => gSig_tut_nichts | _ => gSig_tut_nichts
...
abbrev gCtx_tut_nichts : Ctx := []
def g_tut_nichts : gD.Fn := GFn.tut_nichts

abbrev gL_tut_nichts : List (Res gD) := []


def gBody_tut_nichts : Endblock gD (vertragVon gD g_tut_nichts) false gCtx_tut_nichts gL_tut_nichts :=
  (.ret (.wert ((.weiter (by decide) (by decide) (((.lit 1)) : Expr gD gCtx_tut_nichts gL_tut_nichts (.int 1 1))) : Expr gD gCtx_tut_nichts gL_tut_nichts (.int 0 4294967295))) (List.Perm.refl _))
...
def gP : Programm gD where
  ...
  ensures
    | .tut_nichts => .wahr
  rumpf
    | .tut_nichts => gBody_tut_nichts

def gFs : List gD.Fn := [g_tut_nichts]
...
def gS : SperrInv gD where
  orte := fun | .L => [.inl GTab.T]
  inv := fun | .L => fun _ => true
...
def gSp0 : Speicher gD :=
  ⟨(fun t _ f => match t, f with | .T, .x => ⟨0, by decide, by decide⟩), (fun g => nomatch g)⟩
...
end G719_lock_taken_nowhere_oblig
