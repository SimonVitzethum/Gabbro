-- GENERATED by `gabbro obligations --g beispiele/gift/219-unaeres-minus.gab` (lean-g export plus the stated obligation) -- do not edit.
...
def gSig_spiegeln : Signatur GTab Empty GLock Empty where
  params := [(.int (-100) 100)]
  erg := some ((.int (-2147483648) 2147483647))
  gruende := 0
  haelt := []
  boden := none
...
abbrev gD : Deklaration where
  ...
  Fn := GFn
  sig := fun | .spiegeln => 0
  sigNr := fun | 0 => gSig_spiegeln | _ => gSig_spiegeln
...
abbrev gCtx_spiegeln : Ctx := [(.int (-100) 100)]
def g_spiegeln : gD.Fn := GFn.spiegeln

abbrev gL_spiegeln : List (Res gD) := []


def gBody_spiegeln : Endblock gD (vertragVon gD g_spiegeln) false gCtx_spiegeln gL_spiegeln :=
  (.ret (.wert ((.weiter (by decide) (by decide) (.neg (.var .hier))) : Expr gD gCtx_spiegeln gL_spiegeln (.int (-2147483648) 2147483647))) (List.Perm.refl _))
...
def gP : Programm gD where
  ...
  ensures
    | .spiegeln => .wahr
  rumpf
    | .spiegeln => gBody_spiegeln

def gFs : List gD.Fn := [g_spiegeln]
