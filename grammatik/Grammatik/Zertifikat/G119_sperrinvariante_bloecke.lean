-- GENERATED by `gabbro obligations --g beispiele/119-sperrinvariante-bloecke.gab` (lean-g export plus the stated obligation) -- do not edit.
...
-- lock 0: K (rank 0) guards A
-- fn auffuellen (holds; writes A)
...
namespace G119_sperrinvariante_bloecke_oblig
...
def gSig_auffuellen : Signatur GTab Empty GLock Empty where
  params := [.ptr 0 true]
  erg := none
  gruende := 0
  haelt := []
...
abbrev gD : Deklaration where
  ...
  count := fun | .A => 2
  Feld := fun | .A => GAFeld
  decFeld := fun | .A => inferInstance
  typ := fun | .A, .x => ((.int 0 100))
  erlaubt := fun _ _ _ _ => false
  tabNr := fun | 0 => some GTab.A | _ => none
  Glob := Empty
  ...
  braucht := fun | .A => [.inl GLock.K]
  gbraucht := fun e => nomatch e
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .auffuellen => 0
  sigNr := fun | 0 => gSig_auffuellen | _ => gSig_auffuellen
...
abbrev gCtx_auffuellen : Ctx := [.ptr 0 true]
def g_auffuellen : gD.Fn := GFn.auffuellen

abbrev gL_auffuellen : List (Res gD) := []
abbrev gL_auffuellen_in_K : List (Res gD) := [Res.held (D := gD) GLock.K]

theorem gDarf_auffuellen_in_K_A : darf gD GTab.A gL_auffuellen_in_K := by unfold darf; decide

def gBody_auffuellen : Endblock gD (vertragVon gD g_auffuellen) false gCtx_auffuellen gL_auffuellen :=
  (.cons (.locks GLock.K (fun M => by cases M <;> decide) (.cons (.assignDurch (.var .hier) GTab.A rfl GAFeld.x (((.weiter (by decide) (by decide) (.lit 0) : Expr gD gCtx_auffuellen gL_auffuellen_in_K (.index (gD.count GTab.A))))) ((.weiter (by decide) (by decide) (((.lit 40)) : Expr gD gCtx_auffuellen gL_auffuellen_in_K (.int 40 40))) : Expr gD gCtx_auffuellen gL_auffuellen_in_K (.int 0 100)) (by decide) gDarf_auffuellen_in_K_A) .nil)) (.ret .keine (List.Perm.refl _)))
...
def gP : Programm gD where
  ...
  ensures
    | .auffuellen => .wahr
  rumpf
    | .auffuellen => gBody_auffuellen

def gFs : List gD.Fn := [g_auffuellen]

def gLs : List gD.Lock := [GLock.K]

def gCs : List (gD.Tab ⊕ gD.Glob) := [(.inl GTab.A)]
...
def gS : SperrInv gD where
  orte := fun | .K => [.inl GTab.A]
  inv := fun | .K => fun s => (decide (((s.slots GTab.A 0 GAFeld.x).n) ≤ (100 : Int)))
...
def gSp0 : Speicher gD :=
  ⟨(fun t _ f => match t, f with | .A, .x => ⟨0, by decide, by decide⟩), (fun g => nomatch g)⟩
