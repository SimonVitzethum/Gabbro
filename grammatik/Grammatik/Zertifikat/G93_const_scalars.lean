-- GENERATED by `gabbro obligations --g beispiele/93-const-scalars.gab` (lean-g export plus the stated obligation) -- do not edit.
...
-- the `reads` effects; `costs`; the `const fn` declarations
-- (comptime: folded into the constants that call them, as the
-- emitter's `#define`s are; a run-time call of one is refused); the `by unvisited`/`by consuming`
...
namespace G93_const_scalars_oblig
...
def gSig_f : Signatur GTab Empty GLock Empty where
  params := []
...
abbrev gL_f : List (Res gD) := []


def gBody_f : Endblock gD (vertragVon gD g_f) false gCtx_f gL_f :=
  (.ret (.wert ((.weiter (by decide) (by decide) (.sub (.add (((.lit 42)) : Expr gD gCtx_f gL_f (.int 42 42)) (((.lit 100)) : Expr gD gCtx_f gL_f (.int 100 100))) (((.lit 100)) : Expr gD gCtx_f gL_f (.int 100 100)))) : Expr gD gCtx_f gL_f (.int 0 4294967295))) (List.Perm.refl _))
