-- GENERATED by `gabbro obligations --g beispiele/16-by-ops-am-feld.gab` (lean-g export plus the stated obligation) -- do not edit.
...
-- where exportable).
--
-- table 0: Objekte (count 8), benutzt : bool, zaehler : 0..65535
-- fn stand (holds; writes)
-- fn belegen (holds; writes Objekte)
...
namespace G16_by_ops_am_feld_oblig
...
def gSig_stand : Signatur GTab Empty GLock Empty where
...
def gSig_belegen : Signatur GTab Empty GLock Empty where
...
abbrev gD : Deklaration where
  ...
  count := fun | .Objekte => 8
  Feld := fun | .Objekte => GObjekteFeld
  decFeld := fun | .Objekte => inferInstance
  typ := fun | .Objekte, .benutzt => (.bool) | .Objekte, .zaehler => ((.int 0 65535))
  erlaubt := fun _ _ _ _ => false
  tabNr := fun | 0 => some GTab.Objekte | _ => none
  Glob := Empty
  decGlob := inferInstance
  gtyp := fun e => nomatch e
  nutzlast := fun e => nomatch e
  atomar := fun e => nomatch e
  geteilt := fun | .Objekte => false
  ...
  eigner := fun _ => []
  Fn := GFn
  sig := fun | .stand => 0 | .belegen => 1
  sigNr := fun | 0 => gSig_stand | 1 => gSig_belegen | _ => gSig_belegen
...
abbrev gCtx_stand : Ctx := [.ptr 0 false, .index 8]
def g_stand : gD.Fn := GFn.stand
abbrev gCtx_belegen : Ctx := [.ptr 0 true, .index 8]
def g_belegen : gD.Fn := GFn.belegen

abbrev gL_stand : List (Res gD) := []
abbrev gL_belegen : List (Res gD) := []

theorem gDarf_belegen_Objekte : darf gD GTab.Objekte gL_belegen := by unfold darf; decide
theorem gDarf_stand_Objekte : darf gD GTab.Objekte gL_stand := by unfold darf; decide

def gBody_stand : Endblock gD (vertragVon gD g_stand) false gCtx_stand gL_stand :=
  (.ret (.wert (Expr.durch (D := gD) (.var .hier) GTab.Objekte rfl GObjekteFeld.zaehler ((.var (.dort .hier))) gDarf_stand_Objekte)) (List.Perm.refl _))

def gBody_belegen : Endblock gD (vertragVon gD g_belegen) false gCtx_belegen gL_belegen :=
  (.cons (.assignDurch (.var .hier) GTab.Objekte rfl GObjekteFeld.benutzt ((.var (.dort .hier))) .wahr (by decide) gDarf_belegen_Objekte) (.ret .keine (List.Perm.refl _)))
...
def gP : Programm gD where
  ...
  rumpf
    | .stand => gBody_stand
    | .belegen => gBody_belegen

def gFs : List gD.Fn := [g_stand, g_belegen]

def gLs : List gD.Lock := []

def gCs : List (gD.Tab ⊕ gD.Glob) := [(.inl GTab.Objekte)]
...
def gSp0 : Speicher gD :=
  ⟨(fun t _ f => match t, f with | .Objekte, .benutzt => false | .Objekte, .zaehler => ⟨0, by decide, by decide⟩), (fun g => nomatch g)⟩
