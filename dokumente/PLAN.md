...
| **A8** | **Every construct has its C lowering written down**, not claimed | per rule | ~~18 claims open~~ — **BERICHTIGT 2026-08-28, and the count was recomputed row by row: 12 discharged (10 of them driven through), 2 open, 4 REFUTED.** The eighteen are the rows of `SPRACHE.md`:2621–2638, hand-counted once on 2026-08-14 and never recounted; no instrument counts them, and `pruefe-zahlen.py` cannot even flag the cell as unguarded (it is not bold). The four refuted: **5** (`linear T;` emits a byte — the table's row is missing the word `ghost`, and `SPRACHE.md`:750 says so four pages earlier), **8** (`grep -c printf emit.rs` → `0`; `measures` reaches no emitted line), **12** (`breaking` claims `none` and the generator REFUSES — 53 files against `MARKE_EMIT=52`, and the one is `beispiele/53-zwei-orte.gab`), **18** (claims `#if`, built is a filter in front of the generator). Still open: **3** (`chain` + generation stamp — zero lines of code anywhere) and **6** (`Duty` is emitted nowhere). *A refuted claim costs more than an open one, and this cell counted twelve discharged, two open and four errors as eighteen equal units.* **And three of the four refutations are refutations by the GENERATOR, not by C.** `messung/ABSENKUNG.md` §1.2, §1.6, `K5` |
...
16 unbewiesen, 4 davon lebend~~ ~~21 Einträge, 11 unbewiesen, 2 davon lebend~~
**21 Einträge, 11 unbewiesen, 3 davon lebend** *(nachgemessen 2026-09-04 mit `cargo run -q
--bin gabbro -- schablonen`: BERICHTIGT, nicht bewegt -- `gruppe.sperrabdruck` stand
ENTWORFEN mit dem Grund einer Nachbarzeile, siehe die `L`-Zeile oben)*. Die 24
Notationslücken zu schließen
...
>
> **Heute steht dort 97** *(2026-09-13, `cargo run -q --bin gabbro -- annahmen beispiele/*.gab`;
> die Statustafel unten führt die Zahl mit Befehl)* — davon 30 erzeugte `reglokal.*`-Zeilen
> (Lane 140: die `RegLokal`-Annahme pro Register mit ihren Trägern) — das Tor ist also nicht knapp erreicht,
...
> **The bar for that word did not exist until 2026-09-04, and the line above is why it had
> to.** As long as *"unfalsifiable with a reason"* is a judgement made while the work runs, it
> is the cheapest exit from every assumption whose probe is expensive — and `A = 19` against
> 44 is reachable by twenty-five such judgements. *Then the gate was not passed, it was
> emptied.*
>
> [`dokumente/UNFALSIFIZIERBAR.md`](UNFALSIFIZIERBAR.md) writes the criteria down **before** a
> single row was classified: two criteria (`U1`, `U2`), four rejection rules (`R1`–`R4`), each
> with a test and with what fails it, and a counted register with two ratchets
> (`./instrumente/pruefe-unfalsifizierbar.py`), on the model of `dokumente/AUSNAHMEN.md`.
>
> **And this paragraph's own prediction does not survive its own bar.** A probe for
> `mmu_schreibt_nur_a_und_d` would **not** have to stop the MMU: a refutation needs an entry
> to be FOUND changed, not the MMU to be WATCHED writing — snapshot, run, snapshot, compare.
> `U1` fails, and of the six assumptions booked unfalsifiable today exactly **one** survives.
> *A criterion that refuses its own worked example is one whose test could have gone the other
> way.* The four buckets over all 44 stand in `messung/AXIOMSCHICHT.md` §8.
...
```
H = 1        über dem Fragmentkorpus, mit ./instrumente/zaehle-pflichten.py neu abgeleitet
...
| | Ziel | heute | |
|---|---|---|---|
| `H` | 0 | **1** | **AM 2026-09-04 KURZ 0 UND AM SELBEN TAG ZURUECKGENOMMEN** — das schaerfere Kriterium las die FEHLERZAHL und nie den GRUND der Absage, also senkte eine neue Regel `H`; `messung/AUDIT-K100-2026-09-04.md`. | ~~4 verankert + 6 Absenkungen~~ ~~0 verankert + 5 Absenkungen~~ ~~0 verankert + 5 Absenkungen~~ ~~0 verankert + 4 Absenkungen~~ ~~0 verankert + 3 Absenkungen~~ ~~0 verankert + 2 Absenkungen~~ **0 verankert + 1 Absenkung** (2026-09-03: F9, F1 und F5 durchgestochen) (2026-08-31: F6 durchgestochen, am selben Tag an `N043` gefallen und am selben Tag geheilt; **BERICHTIGT 2026-08-31, und zwar von der Messung und nicht von einem Leser:** `zaehle-pflichten.py` las die Absenkungsspalte am QUELLTEXT von `pruefe-emission.sh` ab — an der blossen Anwesenheit einer `lauf`-Zeile, nicht daran, ob der Lauf haelt. `F06` steht seit `N043` (`measures eich`, ein Traeger, den es nicht gibt) und emittiert nicht mehr; der Waechter war deswegen zu Recht ROT, und `H` sagte weiter 4. *Dieselbe Familie wie `W25`, eine Stufe weiter: dort trug eine richtige Zahl eine ungemessene BESCHRIFTUNG, hier trug eine Zahl eine ungemessene VORAUSSETZUNG.* **`H` ist keine Ratsche, sondern eine Messung** — sie steigt, weil eine Einloesung weggefallen ist.); Start war 31. *F2 am 2026-08-25 an der Ausfuehrung gemessen — ohne ein Konstrukt und ohne eine Schablone: `L` steht unveraendert bei 1.* **Und der Schritt von 11 auf 10 am selben Tag ist KEINE Arbeit, sondern eine Berichtigung:** «B9» stand als Luecke, obwohl der Vertrag am `fn(…)`-Typ seit dem 2026-08-21 Pflicht ist (`N035`/`N036`/`N037`) — *ein falscher Eintrag entfernt, keine Pflicht geschlossen*. **Und der Schritt von 1 auf 0 am 2026-09-04 ist wieder eine BERICHTIGUNG und keine Arbeit** — `K100.1`s Methode, auf die Absenkungsspalte angewandt, so wie diese Datei es selbst verlangt (*„Die Absenkungsspalte zählt bis heute «Gabbro kann das nicht» und «dieser Text ist kein Programm» in einer Zahl."*). Das Kriterium steht **vor** der Anwendung in `zaehle-pflichten.py::absenkungsklasse` und ist mechanisch: eine Absenkungspflicht ist Klempnerei genau dann, wenn `gabbro pruefe` die Datei **annimmt** und nur der Erzeuger fehlt — dann schliesst ein Erzeugerzweig sie, und genau so sind F1, F5 und F9 am 2026-09-03 geschlossen worden. `F03` nimmt der Prüfer **nicht** an: 18 Fehler, und **19 von 19 Riegeln sitzen auf einer Zeile, die byteidentisch im eingefrorenen Ausschnitt steht** (`FRAGMENTE.md`:568-718, textuell nachgezählt). Kein Erzeugerzweig erreicht die Datei, also löst keine Absenkung die Pflicht ein — es ist eine **Notationslücke**, «B10», und sie steht GEZÄHLT in derselben Ausgabe. *Die Sprechprobe fährt bei jedem Lauf eine sauber prüfende, nicht abgesenkte Datei durch den Einordner und verlangt, dass sie `H` HEBT* — ein Maßstab, der nur subtrahieren kann, ist keiner |
| `L` | ≤ 4 | **3** | getragen und unbewiesen; daneben aber **6 Prämissen ohne Pass**. *Der Anstieg 1 → 2 am 2026-08-28 ist der PREIS von Zuschnitt (c):* `table.ops.erhaltung` ging von ENTWORFEN auf GETRAGEN, weil `emit.rs::ops` `insert`/`remove` jetzt ausliefert und `relabel` mit `umhaengen_faellt` absagt. **Genau diese Bewegung soll die Zahl sichtbar machen** — eine Klempnereipflicht wurde nicht erledigt, sondern in die Erzeugerfläche verschoben. **Und 2 → 3 am 2026-09-04 ist eine BERICHTIGUNG, keine Bewegung im Erzeuger:** `gruppe.sperrabdruck` stand als ENTWORFEN, mit einem von `gruppe.ops` kopierten Grund ("es gibt keine Gruppen-`ops`"), der auf ihr eigenes Konstrukt — die `locks`-Anweisung, seit jeher durch `emit.rs::StmtArt::Sperrt` erzeugt und in `zeugnis.rs` als `Traegt::Schablone` gebucht — nicht zutrifft; siehe `messung/K100-VERDICT-2026-09-04.md` §2 und die Buchung selbst in `schablonen.rs`. **Und `≤ 4` ist seit demselben Tag ein PROGRAMMTOR und keine Beschriftung mehr:** zuvor stand hier nur `assert_eq!(lebend_ungedeckt(), 2)` in `paesse.rs` — eine Gleichheit auf den Tageswert, kein Test der Schranke, und die Zeichenkette `L <= 4` kam im ganzen Baum nur als Kommentar vor (`messung/AUDIT-K100-2026-09-04.md` §2.3). Jetzt wertet `schablonen.rs::l_gerissen()`/`L_GRENZE` die Schranke aus, geprüft in `paesse.rs::das_zweite_tor_haelt_l_unter_seiner_schranke` in beide Richtungen |
| `A` | ~~19~~ **retired** | **97** | jede mit Sonde oder mit ausgeschriebenem Grund. **And since 2026-09-04 what this row MEASURES stands beside it, and it is not the trust surface:** the 45 are 41 distinct SENTENCES under 45 names, six of them GENERATED by the checker and declared in no `.gab` at all — `A` grows with the corpus, and a target a new example file can miss is pointed the wrong way. *19 was never a bar over the trust surface: it is 14 plus the five rebookings of `K100.2`, a prediction of a delta, and it came true on 2026-08-17.* **`A = 19` IS RETIRED as of 2026-09-04, and this row is a census from here on.** It was never a bar over the trust surface: 19 = 14 plus the five rebookings of `K100.2`, a prediction of a delta, and it came true on 2026-08-17. *A target a new example file can miss is pointed the wrong way* — under it the cheapest way to pass is to write fewer programs, and the second cheapest is to write `unfalsifiable` twenty-five times. **What replaces it is a SHARE and not a count**, so that a new example file lifts no gate and may not dilute the quota either: `A_p` = **19** of 52 (falsifiable assumptions whose probe stands as a PROGRAM, over falsifiable assumptions -- `beispiele/71` dates one function, so one generated `frist_` assumption dilutes it; each dated function costs share, and that is the price of dates, not a flaw in the share — a ratchet that **may not fall** and a **floor of `1/8` which the tree FAILED by a factor of 4.75 until 2026-09-04 and MEETS since**, at 0.35 against 0.125 (0.1538 at 6 of 39; 0.2326 at 10 of 43). [`dokumente/SONDENDECKUNG.md`](SONDENDECKUNG.md) derives the floor from the class of probes that need nothing but a C compiler and priced reaching it at **four userland probes**; **all four were written on 2026-09-04** — `sonde_mxcsr_rne`, `sonde_keine_ueberbreite`, `sonde_tsc`, `sonde_rdtscp`, each with a positive control that was MADE to fall — and `./instrumente/pruefe-sondendeckung.py` is green. *A bound the tree already meets is a decoration, not a gate — so the next raise costs a bench with ring 0 or a device, because class `P4` is now empty of open work.* **And the same guardian holds the reverse connection**, which is the same build type backwards: **1** program under `sonden/` that no `falsifier` names — may fall, not rise. Beside it, under the bar of [`dokumente/UNFALSIFIZIERBAR.md`](UNFALSIFIZIERBAR.md), stands `A_u` = 2 of 45 (assumptions no probe can ever refute — **may fall, not rise**, `./instrumente/pruefe-unfalsifizierbar.py`). **Two ratchets left a third state unwatched, and it is the one the tree is actually in: falsifiable, probe missing — 37 of 38 when the share was defined, 33 of 39 since.** `A_u` does not fall when nobody writes a probe and `A_p` as a bare count only rises when somebody does — the share is what watches the gap between them. The four buckets over all 45 are in `messung/AXIOMSCHICHT.md` §8 |
| zweiter Korpus | vorhanden | **«K3», acht Fragmente — 0 von 8** | ~~«K2», fünf Fragmente; fremde Autorenlinie; drei sind ganze Module, alle drei prüfen sauber und senken ab~~ **CORRECTED 2026-09-04, and BOTH halves of the old cell were false.** (a) *"three whole modules, all three check clean and lower"* — the checker said **1 clean unit out of 4 blocks** over the «K2» range on the day the row was read (`./target/debug/gabbro fragmente dokumente/FRAGMENTE.md`; `messung/AUDIT-K100-2026-09-04.md` §4). (b) **«K2» is not a second corpus at all**, and that is readable off the commit graph: `git log --date=format:'%H:%M' --since=2026-08-18 --until=2026-08-19` puts the cut at **21:53** (`a805a4a`, *"zwei Fragmente geschnitten -- EIN Konstrukt blockiert beide"*) and the construct both fragments failed on into grammar and checker at **22:50 the same evening** (`b5f5fdb`, `kw.rs`+`parse.rs`+`ast.rs`+`beispiele/31-rcu.gab` in one commit) — *fifty-seven minutes, and the corpus became a fixture.* Its COUNTING half stands and re-derives byte for byte (234 / 578 / 2669 / 2034). **What replaces it is «K3»**: eight functions from `linux/lib/*.c` (7.2.0-rc7, `db2ddb87`), picked by a rule stated before the cut, over a denominator fixed before the candidates were enumerated, and **frozen in commit `61a1b35` before a single one had been run** ([`../messung/K3-AUSWAHL.md`](../messung/K3-AUSWAHL.md)). Measured in the commit after it, and nothing under `crates/` moved afterwards: **0 of 8 transcribe without a wall, 2 of 8 parse, 0 of 8 check, 0 of 8 lower, 0 of 8 compile** — against 9 of 10 over the corpus we chose ([`../messung/K3-BEFUND.md`](../messung/K3-BEFUND.md)) |
...
## NL.4 und NL.4 — die Prämissen und die Absenkungen
...
| | |
|---|---|
| **`H = 4` ist vollständig die Absenkung** | `./instrumente/zaehle-pflichten.py --haengend`: verankert **0**, Absenkung **5**. Das erste Tor von `K100` steht damit auf einem einzigen Posten |
| **Die Bewegung aus `Table_Absenkung.thy`:36 ist NICHT die häufigste** | über alle ~~vierzehn~~ **fünfzehn** Theorien ausgezählt (die fünfzehnte, `Table_Zaehlung`, trägt zu keiner Gestalt bei — nachgezählt 2026-08-30): **4** Delegationen an C, **21** an den Erzeuger, 7 an die Maschine, 9 an den Prüfer, 5 an eine Nachbarschablone. **Die Gabel aus §7 entscheidet vier davon; die Brücke steht auf jedem Weg und ist der fünffach größere Posten** |
...
| **`A8` nachgerechnet: nicht 18 offen, sondern 12 eingelöst / 2 offen / 4 WIDERLEGT** | Zeile für Zeile am Erzeuger gehalten. **Drei der vier Widerlegungen sind Widerlegungen durch den ERZEUGER, nicht durch C** — dieselbe Richtung wie die Zeile darüber |
