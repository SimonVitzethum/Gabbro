...
| `A` | 19 | **44** | jede mit Sonde oder mit ausgeschriebenem Grund. **And since 2026-09-04 what this row MEASURES stands beside it, and it is not the trust surface:** the 44 are 40 distinct SENTENCES under 44 names, four of them GENERATED by the checker and declared in no `.gab` at all — `A` grows with the corpus, and a target a new example file can miss is pointed the wrong way. *19 was never a bar over the trust surface: it is 14 plus the five rebookings of `K100.2`, a prediction of a delta, and it came true on 2026-08-17.* **Under the bar of [`dokumente/UNFALSIFIZIERBAR.md`](UNFALSIFIZIERBAR.md) it splits into two figures that are both ratchets and point in opposite directions:** `A_u` = 2 of 44 (assumptions no probe can ever refute — **may fall, not rise**, `./instrumente/pruefe-unfalsifizierbar.py`) and `A_p` = 2 of 44 (assumptions whose probe stands as a PROGRAM — may rise). The four buckets over all 44 are in `messung/AXIOMSCHICHT.md` §8; **39 of 44 are falsifiable and unprobed, and that is the more expensive place to stand, not the cheaper one** |
