...
| **falsifiable, probe stands as a program** | **18 of 51** | this file — a ratchet UP |
...
| **`P4`** | **nothing — it runs in userland on the bench this folder already has** | **18** |
...
| 47 | **frist_zaehle_eingehalten** | `sonde_zaehle` | `P4` | **PROGRAM** |
...
**51 rows, 18 of them with a PROGRAM, 18 of them `P4`** — and one probe name carries two rows
(`sonde_vtd_srtp`, rows 8 and 29), which is why 51 assumptions stand under 50 names.
...
**62 distinct probe names stand in the whole non-poison corpus, and 47 of them are in the
register above** — the register's other two, `sonde_mxcsr_rne` and `sonde_keine_ueberbreite`,
are GENERATED by the checker and written in no `.gab` at all, which is why it carries 49.
