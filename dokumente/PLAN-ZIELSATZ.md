...
| Gap | Status | Route |
|---|---|---|
| **Linking and cross-unit ABI**: the chain proves one translation unit refines its P. The linked program is another step: symbol resolution, calling convention, record layout, alignment, and whether the ABI promises of two units actually fit. | Covered empirically (abi, the binder, the three-unit library chain), NOT as a theorem | A linking theorem: if each unit's certificate checks and the units' ABI interfaces match (decidable: names, signatures, layouts from the `_Static_assert` pins), the linked C refines the composition of the Ps. Needs a notion of composition over G (separate programs sharing declarations) and a C-level linking step. |
