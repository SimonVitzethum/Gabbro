...
(* The clone-child path (K-1): the block that runs on the handed stack of a `syscall`
   ...
   behind a DOMINATING call of a gate claiming a stack, one call per region (`N450`,
   per region since fix lane F3). The child is its own thread: never started inside
   `locks`/`observes`/`breaking` or under `requires Held` (`N456`), and every carrier
...
* **buffers: `requires x <= lenof(p)`** (fix lane F5, review G04 F3) — a
  ...
  the caller's own parameter under the caller's own clause. A kernel that
...
* **`stack r` + `child { … }`** — the checked clone handoff (lane O-1, K-1).
  ...
  through past the block (`N449`) and runs behind a call of a gate claiming
  a stack that dominates it, one call per region (`N450`). It is a thread of
  its own (fix lane F3): no `child` inside `locks`/`observes`/`breaking` or
  under `requires Held` (`N456`), and every carrier its path or its callees
  touch that anyone writes is guarded, atomic or per-core (`N457`). The gate's number, registers and error map stay user-made in
  ...
  twin, `gift/1107`-`1113`, `1115`-`1117` and `1139`-`1147` the refusals (`1114` the silent spill pin, refused only by `C185`).
  **The checker's reading assumes the lowering enters the child by jump at the
  region** (the statements between the gate call and the region are parent code;
  SATZKARTE §39) -- a lowering that lifts `C185` keeps that or re-checks the gap.
