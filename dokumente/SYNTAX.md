...
| vocabulary words | 221 | **232 table words + 4 Sonderformen** measured (`pruefe-wortschatz.py`: 232 EBNF terminals against 232 table words, both readings) — new words since the second version: `owner` («SG-9»), `deadline` («SG-22»), `concurrent` («SG-23»), `syscall` + `abi` + `number` + `errors` + `kernel` («SS-1», §12.1, checked since lane S5, emission refused as `C001` until S6), `library` + `payload` («E2», §7.1), `arena` + `capacity` + `alloc` + `reset` («E4», §9.1) |
