...
|---|---|---|
| defined EBNF rules | 132 | **170** measured (`pruefe-syntax.sh` EBNF branch: 170 defined, 0 open, 0 unreachable from `program`) — new since the second version: `endblock`, `endstmt`, `matcharm`, `stateassign`, `advstmt`, `countexpr`, `concurrentdecl` («SG-23»), `libcall`, `libregion` (lane E1); `syscalldecl`, `errmap`, `nonzero`, `uint` («SS-1», §12.1); `profiledecl`, `requiresprofile`, `profileentry` («E6», §12.2); lane 88 widened the operator arms inside the same three expression rules (`<<%`, `+%`, `-%`, `+%|` saturating, `*%`); nothing removed |
| used but never defined | 0 | **0** (measured same run) |
| vocabulary words | 221 | **233 table words + 4 Sonderformen** measured (`pruefe-wortschatz.py`: 233 EBNF terminals against 233 table words, both readings) — new words since the second version: `owner` («SG-9»), `deadline` («SG-22»), `concurrent` («SG-23»), `syscall` + `abi` + `number` + `errors` + `kernel` («SS-1», §12.1, checked since lane S5, emission refused as `C001` until S6), `library` + `payload` («E2», §7.1), `profile` + `rounding` + `fp_contract` + `memory_model` + `interrupt_routing` («E6», §12.2) |
...
item       = [ buildgate ]
             ...
             | bootdecl | syscalldecl | profiledecl | requiresprofile ) ;
