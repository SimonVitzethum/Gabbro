...
| 886 | it ends because **the device** completes or faults | L | ~~the human~~ — **REBOOKED 2026-09-03: this is an ASSUMPTION, and the tree already books it as one.** `messung/fragmente/F04.gab`:33 declares `assume device_completes_or_faults` with `falsifier sonde_geraet_meldet_zurueck`, and it *has* to: `progress` names WHO ends the loop (`SYNTAX.md`:1074), and `schleifen.rs`:221 refuses a `progress` whose name is no declared assumption (`S003`) or is unfalsifiable (`S004`). **The two channels of the binary already disagree with this row's old fourth column:** `gabbro annahmen messung/fragmente/F04.gab` lists it as `A1`; `gabbro pflichten` on the same file emits **one** obligation and it is `reg QUEUE_SIZE requires` — *no progress row at all.* So the double booking was never in the emitted manifest, only here. Discharged by `S003`/`S004` + the declared falsifier, exactly like the `S001`/`S002` row four lines above. *The borderline note the row carried stays true and is now the reason rather than the verdict: not "over a finite set" but "because the device makes progress" — and an agent the program does not contain cannot be a proof obligation of it.* `messung/GABBROV-V2.md` §1 |
...
| 981 | it makes progress because a client calls or the endpoint is revoked | L | ~~the human~~ — **REBOOKED 2026-09-03, same class and same witness as 886.** `messung/fragmente/F05.gab`:87 declares `assume client_calls_or_endpoint_revoked` with `falsifier sonde_endpunkt_meldet_sich`; `gabbro annahmen` lists it as `A1`. Discharged by `S003`/`S004` + the declared falsifier. **The referent is the world** — a client that will call — and no predicate over the program's states says it. *Unlike 886 the falsifier is not hardware but the rest of the system, which makes the assumption cheaper to falsify and no more provable.* `messung/GABBROV-V2.md` §1 |
...
| 1656 | it ends because **a token is consumed** | L | ~~the human~~ — **REBOOKED 2026-09-03, and this one is the instructive member of the three.** `messung/fragmente/F10.gab`:62 declares `assume token_verbraucht` with `falsifier sonde_dtb_endlich`, and `F10.gab`:60 writes down why: *"`progress` nennt eine Annahme mit Falsifikator; ohne `assume` steht der Name in keinem Manifest."* `gabbro annahmen` lists it as `A1`; `gabbro pflichten` emits two obligations for this fragment and neither comes from the `progress`. Discharged by `S003`/`S004` + the declared falsifier. **But the note this row already carried is the difference that matters** — *the algorithm's progress measure, not the machine's finiteness*: the referent of `token_verbraucht` is the PROGRAM's own state, not the world, so unlike 886 and 981 this assumption is one a prover could in principle discharge instead of merely declare. It stands as a Lean `Prop` today (`programmlogik/gabbrov/V1.lean`:632) where the other two do not. *The grammar forces all three to be assumptions; only two of them are assumptions about something the program cannot reach.* `messung/GABBROV-V2.md` §1 |
...
### Offen — **`H = 1`** *(~~10~~ ~~9~~ ~~5~~ ~~4~~ ~~5~~ ~~4~~ ~~3~~ ~~2~~ ~~1~~ ~~2~~ — 2026-08-25 zweimal, 2026-08-26 durch F4s Absenkung; **2026-08-28 von 9 auf 5**: «B18» ×2 und «B26» durch BAU, «B27» durch BERICHTIGUNG — und damit ist keine einzige verankerte Zeile mehr offen; **2026-08-31 von 5 auf 4 durch F6s Durchstich**, `lauf "fragment6"` in `pruefe-emission.sh` — **und am selben Tag zurück auf 5**, weil derselbe Durchstich an `N043` gefallen ist und der Zähler ihn bis dahin nicht gelesen hat — **und zurück auf 4**, nachdem `eich` deklariert ist und der Durchstich wieder HÄLT. *Beide Bewegungen sind gemessen: `zaehle-pflichten.py --haengend` fährt `pruefe-emission.sh --absenkung` und liest je Fragment ein Urteil, keine Zeile.* — **2026-09-03 von 4 auf 3 durch F9s Durchstich**, `lauf "fragment9"` in `pruefe-emission.sh` — und die Einloesung ist zur Haelfte eine Erzeugerreparatur (`walk … levels EBENEN` las keinen `const`-Namen) und zur Haelfte eine Korpusreparatur (`device … at normal` war die falsche Form, und `costs <= 4096 ops` ueber `traverse … over mappings of` eine Zusage, die `SPRACHE.md` §5.4 seit Stufe 3 fuer untragbar erklaert). — **2026-09-03 von 3 auf 2 durch F1s Durchstich**, `lauf "fragment1"` -- und dort waren ALLE DREI Absagen richtig und der Ausschnitt falsch: zwei `M140` an einem Widerspruch, den `FRAGMENTE.md` mit sich selbst hat (`Dma(DmaRef)` gegen `d : DmaObj`, vierzig Zeilen auseinander), und ein `N029` als unbezahlte Rechnung. *Dahinter lagen zwei Erzeugerloecher, die niemand sehen konnte:* eine `impl fn` mit `or R` und ohne Ergebnis hatte gar kein C -- der Ruf wurde abgewiesen, und die Erfolgsrueckgabe schrieb niemand. — **2026-09-03 von 2 auf 1 durch F5s Durchstich** -- und dort waren wieder alle Absagen richtig: DREI Namen des eingefrorenen Ausschnitts gehoeren C (`exit`, `signal`, `recv`), und **keiner davon ist an `cc` zu bemerken**, weil die erzeugte Einheit keinen dieser Koepfe einbindet. Dazu fuenf gerufene Ruempfe, die der Kopf der Datei als deklariert ausgab und die nirgends standen -- vier davon ohne jede Absage. *Der Ertrag ist ein Prueferbefund:* `N046` verglich die Absenkung der Signatur und las nie `fehler`, obwohl ein `or R` die C-Signatur aendert.)*, abgelesen mit `./instrumente/zaehle-pflichten.py --haengend` — und die Zahlenspalte ist am 2026-08-20 GESTRICHEN

— **2026-09-10 von 1 auf 2: F1s Durchstich fällt an `M147`** (`obj`-Snapshots nach `unlink`/`release_slot`; lane-43 schließt ein echtes Loch per `frische_alle_toeten` auf leerer `pfade`-Menge, die Kosten sind ehrlich hier: `messung/V4-INDEX-PRAEZISION.md`; die verordnete Nachlese träfe einen freigegebenen Slot und wäre falsch). — **und am selben Tag zurück auf 1:** die Absenkungs-Verdrahtung (zentral) maß Instanz- statt Klassengröße (Wenn mit 41, Match mit 39 Statements gegen Schranke 18) und fiel auf F1/F5; variable Stelligkeit hat keine konstante Schranke — zurückgebaut, Modul und Tests bleiben als spezifizierte Stufe. — **und am selben Tag zurück auf 1:** die Absenkungs-Verdrahtung (zentral) maß Instanz- statt Klassengröße (Wenn mit 41, Match mit 39 Statements gegen Schranke 18) und fiel auf F1/F5; variable Stelligkeit hat keine konstante Schranke — zurückgebaut, Modul und Tests bleiben als spezifizierte Stufe. — **und am selben Tag zurück auf 1:** lane-60 lehrt die Absage Gebrauchsspuren (`veraltet` trägt Quellträger; Index in disjunkten Träger entschuldigt, Rest feuert fail-closed); F01 ohne `M147`, Durchstich hält, `H` misst wieder 1.
