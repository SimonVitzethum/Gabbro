...
> **State carries 2026-09-11** (additive; the bill table above and its cells are untouched and left for the central integrator run). S2 proved 2026-09-10: U1/U2 discharged (`programmlogik/Gabbro/KompositionBeweis.lean`: `umgebung_ok_of_runs`, `schleifen_ok_of_runsloop`), so the first line above reads with named premises as theorems. Cut G3 narrowed 2026-09-10: shared-side preservation from checked lock discipline (`InterferenzAllgemein.lean`: `schuldnerHaelt_gilt`, `fremdDisziplin_gilt`, `fremdErhalt_disjunkt_aus_Disziplin`, `invErhalt_aus_Kontext`). S3 threaded 2026-09-10: fault outcome through parallel execution (`Grammatik/Fehler.lean`: `evalF`, `AusF.folge`, `schrittFolge`, `schleifeF`). General N-thread stability proved with its invariant corollary (`allgemeinStabil`, `allgemeinStabil_invariant`). Device windows chained (`Geraet.lean`: `dma_uebergabe`, `kette_ohne_wettlauf`, `kette_uebergabe`). Validated copy safe inside the model and wired to `World` (`Adressraum.lean`: `gepruefteKopie_ohneToctou`, `modellSequenz_ohneToctou`; exec-side range check stays missing as booked). Lowering measured: maximum 17 at `traverse over descendants of` with bound 18 (`messung/ABSENKUNG-MESSUNG.md` 2026-09-10; `ABSENKUNG-ZAEHLUNG.md`; `Ziel.lean` bound widened 8 to 18). Sondes at 17 of 50, all passing (`dokumente/SONDENDECKUNG.md`; 2026-09-11 remainder). H back to 1 by use-granularity precision (2026-09-10; `dokumente/PFLICHTEN.md` register). Numeric registers themselves (`Ziel.lean` constant, `SONDENDECKUNG.md` counts, `PFLICHTEN.md` H) are deliberately left for the central carries.
