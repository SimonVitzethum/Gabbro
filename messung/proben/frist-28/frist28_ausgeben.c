/* frist28_ausgeben -- the staged date probe for `ausgeben` (`36-asm.gab`:18).
 *
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `9`, class `P1`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of the single `outb %[wert], %[tor]` of the emit (`beispiele/36-asm.gab`:22-26) and reports the
 ...
 * ----------------------
 * Only the single privileged port write, guarded as above.
 *
...
int main(void)
{
    printf("probe frist28_ausgeben :: frist_ausgeben_eingehalten "
           "(36-asm.gab:18, staged)\n");
...
{
    printf("probe frist28_ausgeben :: frist_ausgeben_eingehalten (staged, UNBOUND) "
           "(36-asm.gab:18, staged deadline <= 10 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_ausgeben [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_ausgeben [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
