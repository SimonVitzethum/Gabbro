/* frist28_maske_setzen -- the staged date probe for `maske_setzen` (`65-port-space.gab`:89).
 *
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `17`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one IER byte store (`beispiele/65-port-space.gab`:93) and reports the
 ...
 *   N=20000, min 43, p50 43, p99 in {86, 86, 86, 129, 129} across five runs, max noisy (86-59555, scheduling tail); checksum 5087304 every run. C=192 booked.
 ...
 * ----------------------
 * Only the byte store. The backing is anonymous RAM, not the interrupt register; device latency is NOT measured. The read-back folds the stored byte into the checksum without timing the check.
 *
...
int main(void)
{
    printf("probe frist28_maske_setzen :: frist_maske_setzen_eingehalten "
           "(65-port-space.gab:89, staged)\n");
...
#define SEED 0x065D17U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint8_t c_IER;

static void arbeit(uint8_t m)
{
    c_IER = m;
}
...
{
    printf("probe frist28_maske_setzen :: frist_maske_setzen_eingehalten (staged, UNBOUND) "
           "(65-port-space.gab:89, staged deadline <= 20 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_maske_setzen [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_maske_setzen [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        uint8_t m = (uint8_t)naechste();
        arbeit(m);
        volatile uint8_t senke = c_IER;
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    u64 pruefsumme = 0;
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += c_IER + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: one shadow byte store (stands in for the IER\n");
    printf("      port write); read-back into the checksum, outside the bracket\n");
