/* frist28_bereit -- the staged date probe for `bereit` (`65-port-space.gab`:71).
 *
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `15`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one LSR byte load, THRE field extract (`>> 5 & 1`) and compare against one (`beispiele/65-port-space.gab`:75) and reports the
 ...
 *   N=20000, min 43, p50 43..86, p99 in {86, 86, 86, 86, 129} across five runs, max noisy (86-57663, scheduling tail); checksum 2556335 every run. C=192 booked.
 * Margin rationale: C sits about half again above the worst measured p99 (129 -> 192), clear of the bulk and of routine jitter. A broken path at 10x lands near 1300. C is a tripwire over THIS machine's distribution, not a conversion of the 2 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the extract-and-compare. The port read itself is replaced by a shadow load: without the serial line on the bench a real `inb` faults, and device latency is NOT measured -- the header says so. What is timed is the field logic the emit runs after the read.
 *
...
int main(void)
{
    printf("probe frist28_bereit :: frist_bereit_eingehalten "
           "(65-port-space.gab:71, staged)\n");
...
#define SEED 0x065B1DU

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint8_t c_LSR;

static uint32_t arbeit(void)
{
    return (uint32_t)(((c_LSR >> 5) & 1u) == 1u);
}
...
{
    printf("probe frist28_bereit :: frist_bereit_eingehalten (staged, UNBOUND) "
           "(65-port-space.gab:71, staged deadline <= 20 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_bereit [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_bereit [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    printf("      workload: shadow byte load + THRE extract-and-compare;\n");
    printf("      the port read is a shadow load (no serial line on the bench)\n");
