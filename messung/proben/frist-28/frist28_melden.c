/* frist28_melden -- the staged date probe for `melden` (`36-asm.gab`:47).
 *
 * STAGED, NOT BOUND. No `deadline` clause in `36-asm.gab` names this probe and no
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `10`, class `P1`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of the `ausgeben(112, w)` call of the body (`beispiele/36-asm.gab`:47) down to its single `outb` and reports the
 ...
 * ----------------------
 * Only the call path down to the single privileged port write (`tor 112`), guarded as above. The stimulus byte is the harness, outside the bracket.
 *
...
int main(void)
{
    printf("probe frist28_melden :: frist_melden_eingehalten "
           "(36-asm.gab:47, staged)\n");
...
#define BOOKED_MAX_CYCLES 8192ULL
...
#define SEED 0x036010U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static uint8_t meld_w;

static void arbeit(void)
{
    /* `ausgeben(112, w)` inlined to its single instruction: the call itself
     * carries no cycles worth naming beside the port write. */
    __asm__ __volatile__("outb %[wert], %[tor]"
                         :
                         : [wert] "a"(meld_w), [tor] "d"((uint16_t)112)
                         : "memory");
}
...
{
    printf("probe frist28_melden :: frist_melden_eingehalten (staged, UNBOUND) "
           "(36-asm.gab:47, staged deadline <= 40 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_melden [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_melden [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += meld_w + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: the call path down to its single outb, stimulus\n");
    printf("      byte outside the bracket; guarded -- 77 outside ring 0\n");
