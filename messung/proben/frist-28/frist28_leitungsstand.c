/* frist28_leitungsstand -- the staged date probe for `leitungsstand` (`65-port-space.gab`:79).
 *
 ...
 * `costs <= 2 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `16`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one LSR byte load and return (`beispiele/65-port-space.gab`:83) and reports the
 ...
 *   N=20000, min 43, p50 43..86, p99 in {86, 86, 86, 86, 86} across five calibration runs, max noisy (86-52589, scheduling tail); checksum 5072265 every run. REBOOKED 128 -> 192 on 2026-09-11: the verification run plus two of six re-runs measured p99 129 (frequency stepping 43/86/129, same stepping the gate probes show), refuting C=128. C=192 booked -- about half again above the worst measured p99 (129 -> 192), uniform with the five sibling probes.
 * Margin rationale: C sits about half again above the worst measured p99 over eleven runs (129 -> 192), clear of the bulk (p50 43..86) and of routine jitter. A broken path at 10x lands near 1300. C is a tripwire over THIS machine's distribution, not a conversion of the 2 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the byte load. The backing is anonymous RAM, not the serial line register; device latency is NOT measured.
 *
...
int main(void)
{
    printf("probe frist28_leitungsstand :: frist_leitungsstand_eingehalten "
           "(65-port-space.gab:79, staged)\n");
...
#define SEED 0x065C15U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint8_t c_LSR2;

static uint8_t arbeit(void)
{
    return c_LSR2;
}
...
{
    printf("probe frist28_leitungsstand :: frist_leitungsstand_eingehalten (staged, UNBOUND) "
           "(65-port-space.gab:79, staged deadline <= 20 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_leitungsstand [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_leitungsstand [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        c_LSR2 = (uint8_t)naechste();
        volatile uint8_t senke = arbeit();
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    u64 pruefsumme = 0;
    ...
    printf("      workload: one shadow byte load (stands in for the LSR\n");
    printf("      port read; no serial line on the bench -- see header)\n");
