/* frist28_empfang_freigeben -- the staged date probe for `empfang_freigeben` (`65-port-space.gab`:98).
 *
 ...
 * `costs <= 4 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `18`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one IER read-modify-write setting ERBFI (`beispiele/65-port-space.gab`:102) and reports the
 ...
 *   N=20000, min 43, p50 43, p99 in {86, 86, 86, 86, 129} across five runs, max noisy (645-62436, scheduling tail); checksum 2566416 every run. C=192 booked.
 * Margin rationale: C sits about half again above the worst measured p99 (129 -> 192), clear of the bulk and of routine jitter. A broken path at 10x lands near 1300. C is a tripwire over THIS machine's distribution, not a conversion of the 4 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the read-modify-write on the shadow byte. The backing is anonymous RAM, not the interrupt register; device latency is NOT measured.
 *
 ...
 *   $ ./frist28_empfang_freigeben
 *     -> exit 0, p99 within the booked tripwire (default run PASSES)
 *   $ ./frist28_empfang_freigeben --max-cycles 1
...
int main(void)
{
    printf("probe frist28_empfang_freigeben :: frist_empfang_freigeben_eingehalten "
           "(65-port-space.gab:98, staged)\n");
...
#define SEED 0x065E18U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint8_t c_IER2;

static void arbeit(void)
{
    c_IER2 = (uint8_t)(c_IER2 | 0x01u);
}
...
{
    printf("probe frist28_empfang_freigeben :: frist_empfang_freigeben_eingehalten (staged, UNBOUND) "
           "(65-port-space.gab:98, staged deadline <= 40 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_empfang_freigeben [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_empfang_freigeben [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += c_IER2 + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: read-modify-write setting bit 0 on a shadow\n");
    printf("      byte (stands in for the IER port access -- see header)\n");
