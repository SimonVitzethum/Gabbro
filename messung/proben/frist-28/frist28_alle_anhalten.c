/* frist28_alle_anhalten -- the staged date probe for `alle_anhalten` (`57-faedenhalt.gab`:123).
 *
 ...
 * `costs <= 600 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `27`, class `P3`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of the `128`-slot traverse setting `BEREIT` where `zustand == LAEUFT` (`beispiele/57-faedenhalt.gab`:138-144) and reports the
 ...
 *   N=20000, min 989..1075, p50 1462..1505, p99 in {1763, 1763, 1763, 1806, 2107} across five runs, max noisy (52589-90730, scheduling tail); checksum 3825843 every run. C=3200 booked.
 * Margin rationale: C sits about half again above the worst measured p99 (2107 -> 3200), clear of the bulk (p50 1462..1505) and of routine jitter. A broken traverse at 10x lands near 21000, an order of magnitude above C. C is a tripwire over THIS machine's distribution, not a conversion of the 600 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the traverse body. The lock acquire (`FAEDEN`) is EXCLUDED: the emitter writes the lock as a prototype the bench does not implement, and timing a local stub would be an analogy (forbidden by `sonden/README.md`) -- the same exclusion `sonde_zaehle` books for `TAKT`. A green run says nothing about lock cost.
 *
...
int main(void)
{
    printf("probe frist28_alle_anhalten :: frist_alle_anhalten_eingehalten "
           "(57-faedenhalt.gab:123, staged)\n");
...
{
    printf("probe frist28_alle_anhalten :: frist_alle_anhalten_eingehalten (staged, UNBOUND) "
           "(57-faedenhalt.gab:123, staged deadline <= 6000 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_alle_anhalten [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_alle_anhalten [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    printf("      workload: 128-slot traverse setting BEREIT where LAEUFT,\n");
    printf("      refilled per iteration from a fixed-seed xorshift;\n");
    printf("      lock FAEDEN EXCLUDED (see header)\n");
