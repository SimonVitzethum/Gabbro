/* frist28_warteschlange_scharf -- the staged date probe for `warteschlange_scharf` (`02-geraet.gab`:147).
 *
 ...
 * `costs <= 16 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `13`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of two u16 stores (`USED_IDX = 0; AVAIL_IDX = 0;`, `beispiele/02-geraet.gab`:151-152) and reports the
 ...
 *   N=20000, min 43, p50 43, p99 in {86, 86, 86, 86, 86} across five calibration runs, max noisy (301-53320, scheduling tail); checksum 2546416 every run. REBOOKED 128 -> 192 on 2026-09-11: the verification run measured p99 above 128 (frequency stepping 43/86/129 -- the same stepping that refuted C=128 on `leitungsstand` the same day). C=192 booked -- about half again above the worst measured p99, uniform with the sibling probes.
 * Margin rationale: C sits about half again above the worst measured p99 over six runs, clear of the rock-steady bulk (p50 43) and of routine jitter. A broken path at 10x lands near 1300. C is a tripwire over THIS machine's distribution, not a conversion of the 16 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the two stores. The backing is anonymous RAM, not dma memory, and device latency is NOT measured. The `queue_arm(p)` call is EXCLUDED: it is a foreign phase body with nothing emitted behind it, and timing a local stub would be an analogy (forbidden by `sonden/README.md`).
 *
 ...
 *   $ ./frist28_warteschlange_scharf
 *     -> exit 0, p99 within the booked tripwire (default run PASSES)
 *   $ ./frist28_warteschlange_scharf --max-cycles 1
...
int main(void)
{
    printf("probe frist28_warteschlange_scharf :: frist_warteschlange_scharf_eingehalten "
           "(02-geraet.gab:147, staged)\n");
...
#define SEED 0x028B22U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint16_t q_USED_IDX;
static volatile uint16_t q_AVAIL_IDX;
...
{
    printf("probe frist28_warteschlange_scharf :: frist_warteschlange_scharf_eingehalten (staged, UNBOUND) "
           "(02-geraet.gab:147, staged deadline <= 160 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_warteschlange_scharf [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_warteschlange_scharf [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        arbeit();
        volatile uint16_t senke = (uint16_t)(q_USED_IDX + q_AVAIL_IDX);
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    u64 pruefsumme = 0;
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += q_USED_IDX + q_AVAIL_IDX + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: two u16 stores into shadow queue words, queue_arm\n");
    printf("      call EXCLUDED (foreign body, nothing emitted -- see header)\n");
