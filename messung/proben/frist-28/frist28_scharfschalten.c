/* frist28_scharfschalten -- the staged date probe for `scharfschalten` (`02-geraet.gab`:55).
 *
 * STAGED, NOT BOUND. No `deadline` clause in `02-geraet.gab` names this probe and no
 ...
 * `costs <= 1032 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `12`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of RTA u64 store, one GSTS.RTPS poll check, one GCMD store -- the `scharfschalten` body shape (`beispiele/02-geraet.gab`:55-72) with the bounded retry kept as a bounded loop and reports the
 ...
 *   N=20000, min 43, p50 43..86, p99 in {86, 86, 86, 86, 129} across five runs, max noisy (19651-61748, scheduling tail -- which is why the verdict reads p99 and not max); checksum 64602295757552 every run. C=192 booked.
 * Margin rationale: C sits about half again above the worst measured p99 (129 -> 192), clear of the bulk (p50 43..86) and of routine jitter. A broken path at 10x lands near 1300, an order of magnitude above C. C is a tripwire over THIS machine's distribution, not a conversion of the 1032 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the register shapes. The backing is anonymous RAM, not VT-d MMIO: no device stands on the bench, and device latency is NOT measured -- the header says so. The `on_exceeded geraet_haengt` arm is not timed (it never fires here: the shadow reports ready). The `requires aligned` premise is kept by masking the stimulus.
 *
 ...
 *   $ ./frist28_scharfschalten
 *     -> exit 0, p99 within the booked tripwire (default run PASSES)
 *   $ ./frist28_scharfschalten --max-cycles 1
...
int main(void)
{
    printf("probe frist28_scharfschalten :: frist_scharfschalten_eingehalten "
           "(02-geraet.gab:55, staged)\n");
...
#define BOOKED_MAX_CYCLES 192ULL
...
#define SEED 0x028A17U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint32_t dev_RTA;
static volatile uint32_t dev_GCMD;
static volatile uint32_t dev_GSTS;

static uint32_t arbeit(uint32_t wurzel)
{
    dev_RTA = wurzel & ~0xFFFu;
    uint32_t n = 0;
    while ((((dev_GSTS >> 30) & 1u) == 0) && n < 1024) {
        n++;
    }
    dev_GCMD |= (1u << 30);
    return n;
...
{
    printf("probe frist28_scharfschalten :: frist_scharfschalten_eingehalten (staged, UNBOUND) "
           "(02-geraet.gab:55, staged deadline <= 10320 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_scharfschalten [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_scharfschalten [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        uint32_t wurzel = (naechste() << 12);
        uint32_t n = arbeit(wurzel);
        volatile uint32_t senke = n + dev_RTA + dev_GCMD;
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    dev_GSTS = (1u << 30);
    ...
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += (u64)n + dev_RTA + dev_GCMD + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: RTA store + one GSTS poll check + GCMD store over\n");
    printf("      shadow registers (anonymous RAM, NOT device MMIO); stimulus\n");
    printf("      word outside the bracket; device latency EXCLUDED (see header)\n");
