/* frist28_used_lesen -- the staged date probe for `used_lesen` (`02-geraet.gab`:159).
 *
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `14`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one u16 load and return (`return q.USED_IDX;`, `beispiele/02-geraet.gab`:163) and reports the
 ...
 *   N=20000, min 43, p50 43..86, p99 in {86, 86, 86, 129, 129} across five runs, max noisy (129-47988, scheduling tail); checksum 661972504 every run. C=192 booked.
 ...
 * ----------------------
 * Only the load. The backing is anonymous RAM, not dma memory; device latency is NOT measured. The stimulus refill is the harness, outside the bracket.
 *
 ...
 *   $ ./frist28_used_lesen --max-cycles 1
...
int main(void)
{
    printf("probe frist28_used_lesen :: frist_used_lesen_eingehalten "
           "(02-geraet.gab:159, staged)\n");
...
#define SEED 0x028C41U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint16_t qr_USED_IDX;

static uint16_t arbeit(void)
{
    return qr_USED_IDX;
}
...
{
    printf("probe frist28_used_lesen :: frist_used_lesen_eingehalten (staged, UNBOUND) "
           "(02-geraet.gab:159, staged deadline <= 40 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_used_lesen [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_used_lesen [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        qr_USED_IDX = (uint16_t)naechste();
        volatile uint16_t senke = arbeit();
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    u64 pruefsumme = 0;
    ...
    printf("      workload: one u16 load from a shadow queue word, refilled\n");
