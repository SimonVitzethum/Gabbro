/* frist28_sende -- the staged date probe for `sende` (`65-port-space.gab`:108).
 *
 * STAGED, NOT BOUND. No `deadline` clause in `65-port-space.gab` names this probe and no
 ...
 * `costs <= 1026 ops` -- the headroom rule of `messung/SONDEN-ROWS-72.md`,
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `19`, class `P2`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of one bounded poll (up to `1024` checks) plus one THR byte store (`beispiele/65-port-space.gab`:112-118) and reports the
 ...
 *   N=20000, min 43, p50 43, p99 in {86, 86, 86, 129, 129} across five runs, max noisy (258-68542, scheduling tail); checksum 5095507 every run. C=192 booked.
 * Margin rationale: C sits about half again above the worst measured p99 (129 -> 192), clear of the bulk and of routine jitter. The shadow reports ready so the poll exits on the first check; a spinning device is outside the timed shape. C is a tripwire over THIS machine's distribution, not a conversion of the 1026 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the poll control plus the store. The port reads/writes are replaced by shadow accesses: without the serial line on the bench real port IO faults, and device latency is NOT measured. The shadow reports ready, so the loop exits on the first check; a spinning device is explicitly outside the timed shape. The `on_exceeded uart_haengt` arm is not timed.
 *
...
int main(void)
{
    printf("probe frist28_sende :: frist_sende_eingehalten "
           "(65-port-space.gab:108, staged)\n");
...
#define SEED 0x065E19U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static volatile uint8_t s_LSR;
static volatile uint8_t s_THR;

static uint32_t arbeit(uint8_t b)
{
    uint32_t n = 0;
    while ((((s_LSR >> 5) & 1u) == 0) && n < 1024) {
        n++;
    }
    s_THR = b;
    return n;
...
{
    printf("probe frist28_sende :: frist_sende_eingehalten (staged, UNBOUND) "
           "(65-port-space.gab:108, staged deadline <= 10260 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_sende [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_sende [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        uint8_t b = (uint8_t)naechste();
        uint32_t n = arbeit(b);
        volatile uint32_t senke = n + s_THR;
        (void)senke;    }
    /* ---- The measured window. ---- */
    rng_state = SEED;
    s_LSR = (uint8_t)(1u << 5);
    ...
    for (long i = 0; i < iters; i++) {
        ...
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        pruefsumme += (u64)n + s_THR + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: bounded poll (up to 1024 checks, shadow ready)\n");
    printf("      plus one shadow byte store; port IO EXCLUDED (see header)\n");
