/* frist28_seite_vergessen -- the staged date probe for `seite_vergessen` (`67-befehlsebene.gab`:29).
 *
 ...
 *
 * FRIST-SONDEN-NOTIZ.md row `7`, class `P1`.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of the single `invlpg (%[adr])` of the emit (`beispiele/67-befehlsebene.gab`:33-37) and reports the
 ...
 * ----------------------
 * Only the single privileged instruction, guarded: outside ring zero it faults and the probe reports `77`, measuring nothing. On a ring-zero bench the same binary times the instruction as emitted.
 *
 ...
 *   $ ./frist28_seite_vergessen
 *     -> exit 0, p99 within the booked tripwire (default run PASSES)
 *   $ ./frist28_seite_vergessen --max-cycles 1
...
int main(void)
{
    printf("probe frist28_seite_vergessen :: frist_seite_vergessen_eingehalten "
           "(67-befehlsebene.gab:29, staged)\n");
...
#define SEED 0x067007U

/* ---- The workload: the staged body shape, same `-O2`. ---- */

static uint64_t seite_adr;

static void arbeit(void)
{
    __asm__ __volatile__("invlpg (%[adr])"
                         :
                         : [adr] "r"(seite_adr)
                         : "memory");
}
...
{
    printf("probe frist28_seite_vergessen :: frist_seite_vergessen_eingehalten (staged, UNBOUND) "
           "(67-befehlsebene.gab:29, staged deadline <= 10 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: frist28_seite_vergessen [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: frist28_seite_vergessen [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    rng_state = SEED;
    seite_adr = (uint64_t)(uintptr_t)&seite_adr;
    ...
    printf("      workload: the single invlpg instruction as emitted;\n");
