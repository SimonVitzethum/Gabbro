...
| `probe-marke-an-retry-und-traverse.gab` | senkt `next` an einer `retry`-Marke ab? | **nein** -- `0 errors` beim Pruefer, `C001` beim Erzeuger. der EINZIGE `schleifen.push` der Datei steht im `forever`-Schreiber. Der Korpus verdeckt es: alle `leave`/`next` von `beispiele/41` nennen `forever`-Marken |
| `probe-fehlerkanal-verbundwert.gab` | `let x = f() else (e)` mit einem VERBUND -- was schreibt der Erzeuger? | `W w;` und dann `w->a`. `0 errors` beim Pruefer, `cc`: *invalid type argument of '->'*. **Durch LESEN des Rumpfs gefunden, nicht durch ein Muster** |
