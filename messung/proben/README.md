...
| `probe-transport-warteschlange-aufsetzen.gab` | `queue_setup` gibt ein `Option<(Queue, u16)>` -- hat Gabbro die Form? | **nicht als `tagged`** (`O8`, closed lane 167 -- der Konstruktor existiert jetzt; der Fehlerkanal sagt trotzdem mehr), **wohl als Fehlerkanal** -- und der sagt mehr: die Abwesenheit traegt einen Namen |
...
| `probe-tagged-wird-gebaut.gab` | laesst sich ein `tagged type`-Wert BAUEN? | **bis 2026-09-14 nein** -- in keiner von vier Schreibungen; neun `tagged type`s im Korpus, alle nur zerlegt, keiner gebaut («B9» ein drittes Mal, `O8`). **Seit lane 167 ja**: `Case(payload)`, `Case()` und das blosse `Case` bauen -- ohne neues Wort, zur selben `marke`+`union`-Darstellung, die `match` liest (`beispiele/120`, `beispiele/121`; Absagen `N280`-`N284`, Gift `944`-`947`). Ungebaut bleibt nur die qualifizierte Schreibung `Aufsatz::Keine` (`M126`) -- Faelle haben keinen Typnamen vor sich, mit Absicht |
...
| `probe-besitz-zwei-typen.gab` | traegt die Eigentumsuebergabe Caprocks AUSSCHLUSS, oder nur seine Gestalt? | **beides ist schreibbar, und `beispiele/66` hat die schwaechere Form gewaehlt.** Der Geistermarker NEBEN dem Puffer haelt die Reihenfolge (`L104`, `L107`, `O003`) und laesst „armiert und trotzdem beschrieben" mit `0 errors` durch. Zwei `linear type` mit dem Schreibweg allein ueber der Treiberseite geben ihn zurueck: `22 items, 0 errors, 0 hints`, senkt ab, `cc` bei `-O0` und `-O2` still. **Ohne ein neues Wort.** Absagen: `beispiele/gift/671`, `672`, `673` ([`../FUENFTE-MARKE.md`](../FUENFTE-MARKE.md) §6, `OFFEN.md` `O10`) |
