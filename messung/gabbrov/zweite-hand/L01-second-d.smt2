...
; ===========================================================================
; CANDIDATE PREMISE (d) -- siblings agree on their parent:
;   forall u : c.slots[u].next_sibling == Some(n) => c.slots[n].parent == c.slots[u].parent
; Also <<B14>>: it resolves an `option index into` inside a `pred`.
; ===========================================================================
(assert (forall ((u Int)) (=> (and (In u) ((_ is present) (nxt0 u)))
  (= (par0 (idx (nxt0 u))) (par0 u)))))
(check-sat)
(get-model)
