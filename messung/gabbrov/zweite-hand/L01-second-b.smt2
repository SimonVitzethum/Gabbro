...
; ===========================================================================
; CANDIDATE PREMISE (b) -- FORWARD mutuality of the sibling chain:
;   forall u : c.slots[u].next_sibling == Some(n) => c.slots[n].prev_sibling == Some(u)
; This is L02, the first of the two "Gegenseitigkeits-Invarianten".
; The fragment CANNOT declare it: <<B14>> -- a `pred` has no way to RESOLVE an
; `option index into` (F01:37-39).
; ===========================================================================
(assert (forall ((u Int)) (=> (and (In u) ((_ is present) (nxt0 u)))
  (= (prv0 (idx (nxt0 u))) (present u)))))
(check-sat)
(get-model)
