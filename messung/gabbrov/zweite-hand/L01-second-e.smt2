...
; ===========================================================================
; (e) NOT a candidate premise -- a robustness check on the BASE refutation.
; z3's first model used a self-loop (a slot whose prev/next point at itself)
; and N=2.  Forbid all of that and see whether the counterexample survives:
; no self-pointers anywhere, the witness slot t distinct from s, N >= 3.
; ===========================================================================
(assert (>= N 3))
(assert (not (= t s)))
(assert (forall ((u Int)) (=> (In u)
  (and (not (= (par0 u) (present u)))
       (not (= (prv0 u) (present u)))
       (not (= (nxt0 u) (present u)))
       (not (= (fc0  u) (present u)))))))
(check-sat)
(get-model)
