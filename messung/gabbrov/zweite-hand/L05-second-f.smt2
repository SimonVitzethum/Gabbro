...
; SANITY CHECK: pin the argument to the ROOT slot.  `unlink` then clears
; parent[WURZEL], and no chain to WURZEL ever READS parent[WURZEL] (reachesIn
; stops at s = t).  So the VC must come back unsat.  If it did not, the
; encoding would be vacuously sat and the main result worthless.
(assert (= x ROOT))
(assert (not (cdtWf w6)))
