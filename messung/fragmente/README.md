...
5 von 10 prüfen sauber        (über den Ausschnitten: 5; am 2026-08-20 kurz 7;
...
5 von 10 senken ab            (über den Ausschnitten: 3)
5 von 10 sind DURCHGESTOCHEN  — F02, F04, F07, F08, F10
1 trägt einen Differenztest, dessen Lauf FÄLLT — F06 (`N043`, seit 2026-08-31)
```
...
| **F6** | zwei Konstanten, `IrqMarke`+`static irq`, zwei Kanäle, ein Tor | — *prüft sauber*; ~~Absenkung an «B12» `elems of`~~ *(«B12» ist seit 2026-08-20 entschieden)*. **2026-08-25: 4 × `C001` auf 2.** Was bleibt: drei Namen im `check`-Rumpf, die niemand deklariert — **korpusseitig**, und kein Pass sagt es (Befund 7) — und `lenof` außerhalb eines `format`, dessen einziger Korpusort genau dahinter liegt (Regel A). ~~**2026-08-31 DURCHGESTOCHEN** — `lauf "fragment6"`, achtzehn Zahlen, `-O0`/`-O2`/UBSan; `H` fällt von 5 auf 4~~ **UND AM SELBEN TAG WIEDER GEFALLEN:** `N043` sieht `measures eich.leer, eich.voll, eich.tiefe, eich.gelaufen` und findet keinen Träger `eich` — `gabbro emit` schreibt kein C mehr, der Durchstich ist WEG, `H` steht wieder bei 5. *Die `lauf`-Zeile steht unverändert da; bis heute hat der Zähler genau sie gelesen und nicht den Lauf.* |
