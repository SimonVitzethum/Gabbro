...
| **F6** | zwei Konstanten, `IrqMarke`+`static irq`, zwei Kanäle, ein Tor | — *prüft sauber*; ~~Absenkung an «B12» `elems of`~~ *(«B12» ist seit 2026-08-20 entschieden)*. **2026-08-25: 4 × `C001` auf 2.** Was bleibt: drei Namen im `check`-Rumpf, die niemand deklariert — **korpusseitig**, und kein Pass sagt es (Befund 7) — und `lenof` außerhalb eines `format`, dessen einziger Korpusort genau dahinter liegt (Regel A). ~~**2026-08-31 DURCHGESTOCHEN** — `lauf "fragment6"`, achtzehn Zahlen, `-O0`/`-O2`/UBSan; `H` fällt von 5 auf 4~~ **AM SELBEN TAG GEFALLEN UND GEHEILT:** `N043` sah `measures eich.leer, eich.voll, eich.tiefe, eich.gelaufen` und fand keinen Träger `eich` — `gabbro emit` schrieb kein C mehr, der Durchstich war WEG, `H` stand wieder bei 5. *Die `lauf`-Zeile stand unverändert da; bis dahin hat der Zähler genau sie gelesen und nicht den Lauf.* **Geheilt mit `type EichMarke` + `static eich`** — derselbe Zug wie `IrqMarke`/`irq` in derselben Datei, `H` = 4. **Und ein Befund bleibt stehen, den keine Regel sieht:** der `can_fail`-Rumpf meldet vier Größen und *schreibt keine davon* — `N021` sieht den umgekehrten Fall, `N022` eine einseitige Schwelle, eine Berichtszeile ohne Schreiber sieht niemand |
