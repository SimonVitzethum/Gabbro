...
| **15** | of those 21 are **repaired here** — each green on today's tree and red on a deliberately broken fixture |
| **+1** | a 22nd, found by one of the repairs, in `pruefe-waechter.py` itself — repaired too |

*After the repairs the middle number moves: **52 of 61** now check their fixture, and
**55 of 61** check it deeply enough.*
...
| `pruefe-luecken.py` | yes | anchor: thirteen literal `crates/gabbro-check/src/*.rs` source lines (`:37-84`), **plus two `NULLMUTATION` entries of the same kind, deliberately excluded from scoring** | the thirteen real anchors are checked by `_mp.ankerstand()` and the result is **wired into the exit code** (`:249`, `weg += 1`, and `:300` `if offen or weg: SystemExit(1)`). The two `NULLMUTATION` anchors take the `if not echt:` path (`:226-231`), which `continue`s **without calling `ankerstand()` and without touching `weg`** | `== LUECKEN: ALL PASS -- und alle Quellen byteidentisch zurueck ==` | **YES — and it is LIVE today, not hypothetical.** The second null anchor quotes `let ecken = [a.min * b.min, …]` from `multipliziere()`; that function was rewritten to a `checked_mul` loop on 2026-08-20 and **the literal is not in `typen.rs` any more — grepped, zero hits.** A dead null anchor replaces nothing, so the run behaves exactly as a live one would, and `ALL PASS` stands over a fixture that no longer exists. · **REPAIRED — the null anchors now take the same `ankerstand()` call and the same exit gate as the thirteen, and the dead anchor is renewed. Unrepaired over a planted dead anchor: `ALL PASS`, exit 0; repaired: `ANKER WEG`, exit 1.** |
...
| `pruefe-reichweite.py` | yes | name pattern `PAESSE` (`:44`), twelve hardcoded pass filenames; expected value `RUEMPFE` | pure text, speech-tested in ONE direction only; `PAESSE` is never checked against the real files — `quelle()` returns `""` for a missing one, in silence (`:68`) | `== N ungelesen, M von genau einem Pass gelesen ==` | **YES** — there is no mark on `ungelesen`/`duenn` at all, so a `pruefe-` tool can never redden on its own worst finding, and a pass renamed out of the list is invisible · **REPAIRED — `PAESSE` is now READ from `lib.rs::pruefe`. It had 12 of 15, and the three missing passes were producing **two false `DUENN` findings** (`Format`, `Boot`), both read by `bindung.rs`.** |
...
| `zaehle-karten.py` | yes | anchor: the regex mirroring the literal `HashMap<` field syntax of `umgebung.rs` (`:90`); expected values `MARKE_DIREKT=40`, `MARKE_UNQUALIFIZIERT=36` | pure text. The speech test checks the regex **only against an invented line** (`:117`); there is no floor on the real match count | `== KARTEN: N direkte Blicke, M unqualifiziert -- keine neue ==` | **YES** — a format change in `umgebung.rs` collapses `stellen` to empty, `0 <= MARKE`, and the tool reports a record-best green · **REPAIRED — a floor on the real harvest. Unrepaired with `: HashMap<` renamed in `umgebung.rs`: `0 direkte Blicke … keine neue`, exit 0; repaired: `ABBRUCH … es wurde NICHTS gemessen`, exit 2.** |
| `zaehle-lean.py` | yes | name pattern: `beispiele/*.gab` + `messung/*/*.gab` (`:177`); expected value: the closed `GRUENDE`/`ARTEN` tables | the tables are **live**: an unknown refusal reason aborts with `2`. The glob is **not guarded**: there is no `if not dateien` anywhere in the file | `== BODY CHANNEL: N obligations, M goals, K refused ==`, `return 0` | **YES** — with zero matching files the tool prints `0 obligations, 0 goals, 0 refused` and exits green, indistinguishable from a real all-clear. Its own sibling over the same corpus, `pruefe-lean-beweis.sh`, guards exactly this (`GOALS -eq 0` → exit 2). · **REPAIRED — the empty-population guard its own sibling already had. Unrepaired: `0 obligations, 0 goals, 0 refused`, exit 0; repaired: exit 2.** |
...
| `zaehle-wortschatz.py` | yes | expected values `MARKE_WOERTER=221`, `MARKE_OHNE_GRUND=208` (speech-tested); plus `ABSENKUNGSSAETZE = 1` and `ABSENKUNG_ADRESSE` (`:104-116`) — a hardcoded assertion about an external Isabelle theorem's pass/fail state | the ratchets: pure text, speech-tested. `ABSENKUNG_ADRESSE`: **not validated at all** — no `isabelle build`, no existence check on the `.thy`, no check the theorem still fails | `== Die Beweisschuld: N Absenkungssatz auf M Woerter ==`, with the address printed as a standing fact | **YES** — a claim about a proof, carried as a Python string constant. Fixed, renamed or moved, and the line goes silently wrong. · **REPAIRED — the theory file and both cited theorems are checked; a rename or a removal aborts with 2. The run states that whether the theorem still FALLS is not checked here.** |
...
## What was repaired, and what each repair was measured against

**15 of the 21 are closed.** Every one is green on today's tree AND red on a deliberately
broken fixture, and for most the *unrepaired* tool was run over the same planted defect so
the repair's value is a measurement rather than a claim. Sources touched by a probe were
restored byte-identically, checked each time.

| what was planted | the tool BEFORE | the tool AFTER |
|---|---|---|
| a dead `NULLMUTATION` anchor (`pruefe-luecken.py`) | `== LUECKEN: ALL PASS ==`, exit **0** | `-- ANKER WEG … (NULLMUTATION)`, exit **1** |
| `: HashMap<` renamed in `umgebung.rs` (`zaehle-karten.py`) | `0 direkte Blicke, 0 unqualifiziert -- keine neue`, exit **0** | `ABBRUCH: 0 Karten … es wurde NICHTS gemessen`, exit **2** |
| the corpus glob emptied (`zaehle-lean.py`) | `0 obligations, 0 goals, 0 refused`, exit **0** | `ABBRUCH`, exit **2** |
| the corpus glob emptied (`zaehle-p6.py`) | `0 Pflichten … 0 Dateien angesehen`, exit **0** | `ABBRUCH`, exit **2** |
| the gate forced open (`zaehle-bereichspflichten.py`) | the invalid `== N = 4 Fundstellen ==` printed, exit **0** | `ABBRUCH: die Zahl darueber waere keine Messung`, exit **2** |
| a real citation planted as an anchor (`pruefe-zitate.py`) | candidates 342 → **341**, exit **0** | `THE ANCHOR RULE NOW HIDES 1 CANDIDATE(S)`, exit **1** |
| the diagnostic bracket drifted (`zaehle-absagen.py`) | (untested — `KOPF` was outside the probe) | three probes fall by name, exit **2** |
| the cited theorem renamed (`zaehle-wortschatz.py`) | (unchecked — a printed constant) | `ABBRUCH: … diese Saetze nicht mehr`, exit **2** |
| the cited table removed (`zaehle-c-formen.py`) | (unchecked — the file never opened it) | `ABBRUCH: BEWEIS.md §1 nennt seine eigene Zahl nicht mehr`, exit **2** |
| a false POSIX equivalence (`miss-c-signaturen.py`) | (unchecked — outside the assert loop) | `KAPUTT: size_t != uint8_t (POSIX, LP64)` |
| a ninth unlisted descender / a stale booking (`pruefe-abstieg.py`) | (uncounted) | exit **1**, both directions |
| `lib.rs::pruefe` renamed / calling no pass (`pruefe-reichweite.py`) | (unread) | `ABBRUCH`, exit **2** |

### One repair produced a finding of its own, twice over

**`pruefe-reichweite.py` was printing two FALSE findings.** Its hand-written `PAESSE` had
12 of the 15 passes `lib.rs::pruefe` really runs. With `bindung`, `gatter` and `kontexte`
restored, `Format` and `Boot` stop being *read by only one pass* — `bindung.rs` names both.
The reach table went from `0 ungelesen, 2 duenn` to `0 ungelesen, 0 duenn`. *A guardian over
blind spots that had one, and its blindness came out as a finding about the tree.*

### And one repair uncovered a 22nd mismatch, in the guardian over guardians

Adding a printed sentence to `zaehle-wortschatz.py` that points a reader at
`isabelle build` turned `pruefe-waechter.py` red for `FRIST` — a deadline demanded for
something the tool never runs. `FUEHRT_AUS` matched the bare NAME anywhere.

**Its probe against exactly this was green for the wrong reason.** `pruefe-prosa.py` names
`cc` in prose and carries `timeout=5`, so `HAT_FRIST` matched whatever `FUEHRT_AUS` did;
the probe could not tell *prose was ignored* from *prose was flagged and the deadline
covered it anyway*. **A green two different mechanisms can produce measures neither** — the
class of this document, inside the tool that polices the class.

Both repaired: the rule now strips comments and quoted spans before looking for the name,
and a second probe executes nothing and carries no deadline (`FEHLT FRIST` under the old
pattern, `ok` under the new one).

***And the first attempt at that repair was wrong, and died of the counter-check rather
than of the colour.*** It demanded the call shape `GIT_AUFRUF` uses; measured over all 63
tools, four real call sites stopped matching (`cc "${CFLAGS[@]}"` in `pruefe-sonden.sh`,
`timeout "$FRIST" cargo build` in `pruefe-syntax.sh`, and two more). **All four carry a
deadline anyway, so no verdict would have moved.** The whole-population diff is what caught
it. Net effect of the rule that shipped: two tools that only ever NAMED a compiler stop
being counted as executing, and `pruefe-lean-beweis.sh` and `pruefe-lean-programm.sh` start
being counted — the old pattern did not know the word `lean` at all.

## What is left open, and why

| instrument | why not repaired here |
|---|---|
| `pruefe-uebersetzerfamilie.py` | another lane owns this file; the empty-population hole is reported, not touched |
| `mutiere-pruefer.py` | the 39 `annotation` mutations are scored at the generated TEXT while the property is about a **proof**. Closing it means running `isabelle`/`lean` from the mutation loop — a decision about what a 13-minute run may cost |
| `pruefe-todo.py` | `PLAN_ABWEICHUNG_GEBUCHT` needs a synthetic `PLAN.md` fixture, the shape every other check in that file already has |
| `pruefe-englisch.py` | `quellsprache()` needs invented sources under it; the ratchets it drives are load-bearing and the repair should not move them by accident |
| `zaehle-b3.py` | the classifier vocabulary would need labelled examples out of a foreign tree; the tool already downgrades itself to a lower bound |
| `pruefe-grammatiktafel.py` | **self-disclosed and dormant** — the `abgesagt` state rests on 0 words today, and the file says so at `:38-40`. Booked, not silent |

## Standing reds found on the way, and NOT caused here

Measured at `a2cd217` by running the tool from `git show HEAD:…` in place:

| instrument | state |
|---|---|
| `zaehle-karten.py` | exit **1** — `45` direct looks against mark `40`, `40` unqualified against `36` |
| `pruefe-zahlen.py` | exit **1** |
| `miss-c-signaturen.py --pruefe` | exit **1** |

*The first is worth a second look next to its repair:* the tool is red today, and a moved
subject would have turned that real finding into a green record best.
