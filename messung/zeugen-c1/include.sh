#!/usr/bin/env bash
# #include — C1 witness pair (used and on the never list).
#
# Gabbro fragment class: none of its own — every emitted unit carries the
# header block the emitter writes itself (emit.rs:675-678: stdint.h,
# stdbool.h, stdatomic.h, math.h). Census: 408 sites of preprocessor use
# other than `#if`.
# Expected C (the forbidden form under test): `#include <stdint.h>` and
# `#include <stdbool.h>` at the top of the unit, with the program using
# `uint32_t` and `bool` from those headers.
# Expected behaviour: stdout holds `42 1` at every optimisation level.
# Atomic-safe verdict: YES, vacuously — a compile-time directive emitting
