...
### 1.0 The whole answer in one screen

| | the runtime | | | the Caprock areas | |
|---:|---|---|---:|---|---|
| 1 | start a thread on a declared root | **A** | 9 | move a `state` field (`transition …;`) | **A** |
| 2 | an entry point and an exit code | D | 10 | advance a phase mark (`advances …;`) | **A** |
| 3 | a message — no strings, no varargs | **A** | 11 | stop a table walk at the first hit | B |
| 4 | a `bool` at module scope | **C** | 12 | a fn pointer to something that locks | B |
| 5 | `atomic_fetch_add` | **C** | 13 | type application (`Queue(T)`) | C |
| 6 | `+%` over a local | **C** | 14 | aggregation in a predicate | B |
| 7 | the idle root | D | 15 | `refcount -= 1` under a `requires` | D |
| 8 | the lock beside its own `lock L` | B | 16 | `option` of an ordinary type | **A** |
| | | | 17 | a record value that reaches the proof | B |
| | | | 18 | a multi-line assumption text | **A** |
| | | | 19 | the kernel-side entry stub | D |
| | | | 20 | a register wider than `u64` | *decided* |
| | | | 21 | one memory copy for every table | D |
| **P1** | **the runtime's lock — WRITABLE, linked, RUN** | | **P2–P4** | **«B17» ½, «B23», «B27» user side — closed** | |

**The two that surprised me**, because nothing in the tree says them: **rows 9 and 10** — two
statement forms stand in `SYNTAX.md` §8, one with a Lean constructor and a theorem, and the
Rust parser has no arm for either (§3 counts it: 2 of 16) — and **row 4**, a boolean static,
which checks clean and never becomes C.
...
| **2** | **An entry point and an exit code.** `main` in C. `entry`/`boot` exist and emit a prototype plus a `#define`, never a body (row 19). An `impl fn` lowers to `static`, so nothing in an emitted unit can be C's `main`. | **D** | `S05-treiber-in-gabbro.gab` | Accepted (0 errors). The exit code travels as an ordinary return value nobody reads; the driver that reads it is C. |
...
| # | What a writer cannot write | Class | Probe | What the tool said, verbatim |
|---|---|---|---|---|
| **9** | **Move a `state` field.** `transition T.slots[i].s : A -> B;` — `stateassign`, «SG-19», `SYNTAX.md`:866, in §8's table with `Stmt.uebergang` and the theorem `uebergang_erklaert`. **The Rust statement parser has no `Kw::Transition` arm** (`crates/gabbro-syntax/src/parse.rs`:3467-3630), so a `state` declaration is unusable: the only statement that could use it does not parse. F3's reply obligation. | **A** | `S08-zwei-orte-ein-uebergang.gab` | 6 shapes. Four spell the statement and all four are `[P017] assignment or call expected`, at whatever identifier follows the word: `transition E.slots[0].offen : oeffnen;` and `: 0 -> 1;` (identifier `Endpunkte`), `transition oeffnen;` (identifier `oeffnen`), `transition Antwortpflicht.oeffnen;` (identifier `Antwortpflicht`). Two spell it as a call: `Antwortpflicht.oeffnen();` → `[M129] is not a function pointer, so it cannot be called`, `oeffnen();` → `[H021] unknown to the graph`. |
...
| **14** | **Aggregation in a predicate.** `count k in slots of T : p` inside an `invariant` or a `spec fn`. F1's `refcount_matches` — the capability system's bookkeeping. | **B** | `S11-zaehlung-im-praedikat.gab` | In an `invariant` → `[D021] `k` in an `invariant` is not declared here`; in a `spec fn` body → `[D021] `k` in the body of a `spec fn` is not declared here`. The binder is not bound in either. **Three further spellings** of the cross-table form: `count(s in … : p)` and `count(s in … where p)` → `[P012] predicate expected, identifier `Objekte` found`, `count Kappen` → `[P001]`. **The same expression IS accepted in an `ensures` and in a body** (`beispiele/71`:33) — so the aggregation exists as a computation and not as a statement. The cross-table half is refused **by decision** (`SYNTAX.md`:554: *"over ONE table only … A cross-table count is a `group` invariant"*). |
