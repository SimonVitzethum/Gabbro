...
| `6506` | ~~`queue` -- «B10»: `traverse` yields no value and knows no `break`, so `by consuming` drains the WHOLE queue; that is a different program~~ **rewritten 2026-09-04** -- `` `queue` -- the domain names no ELEMENT SET: a queue is an ordinary record with exactly one array field, and nothing declares head, tail or count … THE RUN FORM IS NOT THE REASON: `by unvisited` reaches this line too `` | UNGEDECKT | messung/proben/probe-vier-zellen.gab (which writes `by unvisited` -- and got the `by consuming` text, which is how the defect was found) |
