...
pub fn export(source_name: &str, tree: &Programm) -> Result<String, Refusal> {
    ...
    text.push_str(&crate::freigabe::freigabe_abschnitt(tree, ""));
...
fn abschnitt() -> String {
    ...
    out.push_str("-- Carriers are tables or globals (lane 198 exports `static mut`\n");
    out.push_str("-- globals; a unit without one has `Glob := Empty`), and each arm\n");
    out.push_str("-- closes by cases over its constructors. `simp [gCs]`\n");
...
// ---------------------------------------------------------------------------------------
// O12: the release obligation, STATED in the obligation channel -- the analysis is
// `crate::freigabe` (shared with `gabbro counterexample` since fix lane F7).
