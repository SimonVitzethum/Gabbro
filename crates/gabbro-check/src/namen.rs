...
fn fnptr_traegt_seinen_vertrag(baum: &Programm, absagen: &mut Absagen) {
    crate::fuer_jedes_item_im_modul(baum, &mut |item, _modul| {
        crate::jeder_typausdruck_im_item(item, &mut |t| {
            ...
            if let Some(p) = f.requires.first() {
                absagen.schiebe(
                    Absage::fehler(
                        "N037",
                        p.span,
                        "a function pointer type carries no `requires`",
                    )
                    .mit_notiz(
                        "a precondition is checked at the CALL SITE against the caller's \
                         state, and the passes that do that are keyed by the callee's name",
                    ),
                );
            }
        });
