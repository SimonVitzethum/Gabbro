...
pub fn export(
    ...
    out.push_str(&freigabe_abschnitt(tree));
...
// ---------------------------------------------------------------------------------------
// O12: the release obligation, STATED beside the search.
//
...
// (`NutzerPflicht`) unproved. The per-function search above cannot see it
// either: it runs each body in isolation, while the release duty sits at the
// CALLER, between the callee's promise and the lock invariant. What follows
// states that gap per locked section -- in this output, never as a
// diagnostic: the checker verdict of the file is unchanged by this section.
//
...
//
// NOTE (duplication): the same analysis stands in `obligations_g.rs`. It is
...
/// Slot cells read by an expression (see the section note: no `old`).
...
/// Slot cells read by a contract predicate (see the section note).
...
/// unresolvable (never counted -- see the section note).
struct Abschnitt {
    sperre: String,
    sicher: Vec<String>,
    unsicher: Vec<String>,
...
fn freigabe_abschnitt(tree: &Programm) -> String {
    let mut out = String::new();
    ...
    out.push_str("-- while `RELEASE UNPROVED` names the exact gap. The per-function search\n");
    out.push_str("-- above runs each body in isolation; the release duty sits at the CALLER,\n");
    out.push_str("-- between the callee's promise and the lock invariant, and only this\n");
    out.push_str("-- section states it.\n");
    let mut fns = Vec::new();
