...
pub fn export(
    ...
    out.push_str(&crate::freigabe::freigabe_abschnitt(
        tree,
        "\n-- The per-function search above runs each body in isolation; the release\n-- duty sits at the CALLER, between the callee's promise and the lock\n-- invariant, and only this section states it.",
    ));
...
// ---------------------------------------------------------------------------------------
// O12: the release obligation, STATED beside the search -- the analysis is
// `crate::freigabe` (shared with `gabbro obligations --g` since fix lane F7). The
// per-function search above runs each body in isolation; the release duty sits at the
// CALLER, between the callee's promise and the lock invariant, and only that section
// states it.
