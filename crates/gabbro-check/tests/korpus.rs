...
const BENANNT: &[&str] = &[
    ...
    "N305",
    // Lane 194 (W1): `N310`-`N313` an axiom call/binding at an empty answer
    // (range, caseless reason, orphan `fn(...)`, uninhabited sum/record),
    // `N314` a register read at one. Named here so the corpus run counts
