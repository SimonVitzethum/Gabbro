...
//! * `M141` is an EQUALITY. Nothing converts at an indirect call, so `fn(u32)` at a `fn(u8)`
...
/// **`M141`, the loud half -- and both directions of the SAME crossing are in it.**
///
...
fn eine_andere_unterschrift_faellt() {
    ...
    for (was, slot, fun) in faelle {
        ...
        assert!(
            g.contains(&"M141".to_string()),
            "{was}: `{fun}` at `fn({slot})` must fall with `M141` -- got {g:?}"
        );
    }
}

/// **`M141`, the counter-direction.** A signature that agrees must stay silent -- including
...
///
/// `M128` holds the contract, `M141` the signature; the arity arm is `M128`'s alone and
/// `M141` returns before it, so an arity mismatch must give exactly one of them. *Without
/// this row the division of labour between the two codes is an intention, not a fact.*
#[test]
fn vertrag_und_unterschrift_sind_zwei() {
    // Arity: `M128`'s, and `M141` must not double-report it.
    ...
    assert!(
        !g.contains(&"M141".to_string()),
        "`M141` must not also report an arity mismatch -- got {g:?}"
    );
    ...
    assert!(
        !g.contains(&"M141".to_string()),
        "the signature agrees, so `M141` must stay out of it -- got {g:?}"
    );
}
