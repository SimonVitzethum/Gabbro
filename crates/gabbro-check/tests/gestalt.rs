...
//!
//! `M140` was written on 2026-09-02 out of a measurement: eighteen parameter kinds against
...
/// **Two crossings belong to older rules, and `M140` must not take them.**
///
...
fn was_anderen_regeln_gehoert_bleibt_ihnen() {
    ...
    assert_eq!(g, vec!["M135"], "the bool/number crossing is `M135`'s, not `M140`'s");
...
fn die_falsche_gestalt_faellt() {
    ...
    for (was, slot, arg) in faelle {
        let g = codes(&ruf(slot, arg));
        assert!(
            g.contains(&"M140".to_string()),
            "{was}: `{arg}` at `{slot}` must fall with `M140` -- got {g:?}"
        );
    }
}
