...
## `A = 19` is retired, and what replaces it is a SHARE with a floor the tree FAILS *(2026-09-04)*

**`K100`'s last open gate no longer measures what it was set to measure.** 19 = 14 plus the
five rebookings of `K100.2` — a prediction of a delta, and it came true on 2026-08-17. Since
then the measurand grew: 44 names over 40 distinct sentences, four of them generated by the
checker and declared in no `.gab`. *A target a new example file can miss is pointed the wrong
way* — the cheapest way to pass it is to write fewer programs.

### The proposal was two ratchets, and two ratchets are not enough

`A_u` (unfalsifiable, may fall not rise) and `A_p` (probes, may rise) **leave a third state
unwatched: falsifiable, probe missing.** That is **37 of 38** today. `A_u` does not fall when
nobody writes a probe, and `A_p` as a bare count only rises when somebody does. **The state
the tree is actually in had no guardian.**

> **So `A_p` is a PROPORTION.** Falsifiable assumptions whose named probe stands as a program
> in `sonden/`, over falsifiable assumptions — **1 of 38**. A new example file lifts no gate,
> and it may not dilute the quota either: *whoever adds an assumption writes its probe or
> lowers the proportion visibly.*

### And the floor, because a proportion-ratchet alone demands nothing

Booked at today's 1 of 38 a ratchet passes forever while nothing is written. **So the quota
carries a floor, and the floor is one this tree fails.**

| | |
|---|---|
| **the floor** | **`A_p ≥ 1/8`** — `./instrumente/pruefe-sondendeckung.py` |
| today | `1 / 38` = **0.026** — missed by a factor of **4.75** |
| where `1/8` comes from | `sonden/README.md` and `messung/RACE.md` §5.4 classify every named probe by what it would need. Five of the 38 rows are class `P4` — **nothing but a userland C program**; one of the five has one. So the work that is actually possible reaches `5 / 38` = **0.132**, and `1/8` = 0.125 is the largest unit fraction under it (`1/7` = 0.143 is already out of reach) |
| **what reaching it costs** | **four probes**: `sonde_mxcsr_rne`, `sonde_keine_ueberbreite`, `sonde_tsc`, `sonde_rdtscp` — each a userland C program under `sonden/` with a sensitivity arm, each one line in `SONDEN_MIT_PROGRAMM` (`crates/gabbro-check/src/manifest.rs`) |

*A bound the tree already meets is a decoration, not a gate.*
`./instrumente/pruefe-sondendeckung.py` is **RED by construction** and says so in its own
verdict: *"This is the debt, not a regression."* **And it is not a target** — the 29 rows in
`P1` (ring 0) and `P2` (a device) stay where they are; whether this project ever gets a bench
with ring 0 is a question about apparatus that no guardian can force.

### Both directions of the same broken connection

**`2 of 48` was a sum over two halves that do not overlap.** Exactly **one** name has a
program (`stilllegung_boot_ende_ist_unerreichbar` → `sonde_boot_unerreichbar.c`), and the
second program, `sonde_release_sichtbarkeit.c`, is named by **no `falsifier` line anywhere**.

> **A built program nobody references is the same build type as a name without a program,
> just reversed.** Both are held: the share watches the first, and a plain count — **1, may
> fall not rise** — watches the second, because a probe bound to no obligation is not a share
> of anything and the target is zero.
...
#   5 of 38 rows are class `P4`, 4 of them without a program
#   2 program(s) under `sonden/`, 1 of them named by no `falsifier`
#   THE FLOOR IS NOT MET                                              -- exit 1
```
...
equal `sonden/*.c`, or a string added to that constant would raise the quota inside the
checker **with no program written** — the same move as `unfalsifiable`, one layer down.

`pruefe-zahlen.py` holds six of the figures against the run (85 → **91** entries), and
`dokumente/PLAN.md`'s `K100` status table reads `~~19~~ **retired**`.
...
`messung/AXIOMSCHICHT.md` says it about its own count: *"Die Zahl misst hier die Grammatik,
nicht die Sorgfalt."* So the second branch was free, and `K100`'s open gate — `A = 19` against
**44** — is reachable by writing it twenty-five times. *Then the gate was not passed, it was
emptied.*
...
| | |
|---|---|
| `U1` | **the observation IS the violation** — no arrangement of the refuting observation avoids performing an instance of what the assumption denies. *Fails for everything a passive read can settle.* |
| `U2` | **the negation has no finite witness** — and no bound is choosable. *Fails for every sentence that already names one, and the corpus has that idiom in four places.* |

And the four that refuse: `R1` a **green-direction** reason (why no run could CONFIRM it says
nothing about refutation), `R2` a reason about the **bench** (that is return code **77**, which
`sonden/README.md` already defines), `R3` the **subject is not the environment** (a probe is
external by construction), `R4` **nothing observable rides on it** (then nothing rests on it
either — *the rule that stops the category being a refuge*).

### The result, and it goes against the plan's own worked example

**Of the six assumptions booked `unfalsifiable`, one survives.** `ipi_kommt_an`, under `U2` —
and on a better reason than the one it carries.

> **`PLAN.md` §`K100.2` predicted the category and named its seed in one sentence:** *"eine
> Sonde für „die MMU schreibt nur `A` und `D`" müsste die MMU anhalten."* **It would not.** A
> refutation needs the entry to be FOUND changed, not the MMU WATCHED writing — snapshot, run,
> snapshot, compare. `U1` fails; the residue is `R4`. *A criterion that refuses its own worked
> example is one whose test could have gone the other way.*

`x2apic_zweischritt` is refused by the corpus itself: `beispiele/60-annahme-mit-maschine.gab`
carries the **byte-identical sentence** under another name **with a probe**. And
`release_stellt_sichtbarkeit_her` is refused with its probe already built —
`sonden/sonde_release_sichtbarkeit.c`, positive control fell 3 079 times.

### The four buckets over all 44 — `messung/AXIOMSCHICHT.md` §8

| bucket | of 44 |
|---|---:|
| falsifiable, probe EXISTS as a program | ~~**2 of 44**~~ **1 of 44** *(corrected the next entry up: the two was a name with a program plus a program with no name)* |
| falsifiable, probe MISSING | **39 of 44** |
| unfalsifiable under a criterion | **2 of 44** |
| not an assumption of this layer (`R3`) | **1 of 44** |

*The bar removed rows in both directions:* `quelle_endet` carries a `falsifier` and is
unfalsifiable under `U2` — **and the change was deliberately NOT made**, because it would raise
the ratchet on the day the ratchet was written.

### Counted, not asserted

```
./instrumente/pruefe-unfalsifizierbar.py     # 8 clauses, 8 rows, 1 ADMITTED -- exit 0
```

On the model of `dokumente/AUSNAHMEN.md` (`W7`: not a second register over one thing). Three
teeth: **set equality against the corpus** (a row cannot be written without a clause, nor a
clause without a row), **two ratchets** (population 8, ADMITTED 1 — may fall, not rise), and
**every row names a rule the document defines**, with `ADMITTED` taking a `U` and `REFUSED` an
`R`. Seven speech tests, one per tooth plus the control; the empty table is a refusal.
Both figures are held against the command by `pruefe-zahlen.py` (83 → **85** entries).

### And what `A = 19` means under the bar

**It no longer measures what it was set to measure.** 19 is 14 plus the five rebookings of
`K100.2` — *a prediction of a delta, and it came true on 2026-08-17.* Since then its
measurand grew: the 44 are **40 distinct sentences** under 44 names, four of them GENERATED by
the checker and declared in no `.gab`. **A target a new example file can miss is pointed the
wrong way** — the cheapest way to pass it is to write fewer programs, and the second cheapest
is the bin this bar was written against. Under the bar it splits into `A_u` = 2 of 44
(ratchet **down**) and ~~`A_p` = 2 of 44 (ratchet **up**)~~ — **and the second half was replaced the same day**: a
count in either direction has the defect `A = 19` had, and two ratchets left the state the tree
is in unwatched. See the entry above.
...
| **0** | a third answer: no panic, no timeout, no unnamed exit, and all ~~**471**~~ **479** refusals carried their note | `messung/ERZEUGERSWEEP.md` §5 |
...
**~~71~~ ~~74~~ ~~76~~ ~~89~~ ~~91~~ ~~95~~ ~~96~~ ~~97~~ ~~103~~ ~~107~~ **109** clean examples, (2026-09-14, lane 184: +2 eigene (`130`/`131`)) ~~471~~ ~~479~~ ~~488~~ ~~498~~ ~~500~~ ~~501~~ ~~550~~ ~~569~~ ~~585~~ ~~587~~ ~~618~~ ~~623~~ ~~624~~ ~~640~~ ~~644~~ ~~656~~ ~~665~~ ~~672~~ **669** poison probes, (2026-09-14, lane 191: +1 eigene (`968`), −3 (`969`-`971`), Rest Drift) ~~451~~ ~~688~~ ~~693~~ ~~767~~ ~~773~~ ~~883~~ ~~891~~ ~~945~~ **966** tests · (2026-09-14, lane 191: `cargo-pruef` exit 0, 0 failing; Testfunktionen −1/+1, Rest Drift) ~~55~~ 37 translation units** —`cargo test` · `cargo run --bin gabbro -- pruefe beispiele/*.gab` · `./instrumente/pruefe-emission.sh`> **Measured 2026-08-30, and every one of the four was wrong.** It read ~~*25 clean> examples, 78 poison probes, 123 tests · 11 translation units*~~ — a line that had not> been touched while the corpus grew to four times its size.>
