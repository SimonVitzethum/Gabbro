/* sonde_zaehle -- the date probe for the slot write `zaehle`.
 *
 * THE OBLIGATION, as the checker declares it (`gabbro annahmen`):
 *
 *     frist_zaehle_eingehalten (assume): falsifier sonde_zaehle
 *
 * and in the source (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:33):
 *
 *     deadline <= 80 ops arch x86_64 falsifier sonde_zaehle
 *
 * on `zaehle`. This probe belongs to EXACTLY this one obligation (N024).
 ...
 * (`Takte_speicher.slots[i].stand = 1;` in the emit: the u64 store into the
 * 64-slot table, replicated here in the same shape and under the same `-O2`)
 * and reports the distribution (min/p50/p99/max over N iterations).
 ...
 *   N=20000, min 27..62, p50 32..65, p99 in {35,35,39,47,67} across five
 *   runs, max noisy (39-27026, scheduling tail -- which is why the verdict
 ...
 * (67 -> 104), clear of the rock-steady slow-mode bulk (p50 64..65) and of
 * routine jitter. Two core-frequency modes step run to run (fast p50 ~32,
 * slow p50 ~65, the same stepping the gate probes show); the margin is read
 * against the SLOW mode, not the fast one. A broken slot write at 10x lands
 * near 650, six times above C. C is a tripwire over THIS machine's
 * distribution, not a conversion of the 8 ops cost. The stored word is the
 * constant 1 the body writes, so the checksum is seed-independent by
 * construction; it still proves the N stores landed once each.
 *
 ...
 * check. The lock acquire/release (`TAKT_nimm`/`TAKT_gib`) is EXCLUDED: the
 * emitter writes `void TAKT_nimm(void);` and never defines it, the bench
 * provides no `TAKT` implementation, and timing a local stub would be an
 ...
 *   $ ./sonde_zaehle
 *     -> exit 0, p99 in 35..67 <= 104 (default run PASSES)
 *   $ ./sonde_zaehle --max-cycles 1
 *     -> exit 1, p99 in 35..67 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_zaehle :: frist_zaehle_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:33)\n");
...
#define SEED 0x2e59a1e1U

/* ---- The workload: the emitted slot write's shape, same `-O2`. ---- */

/* Mirrors `table Takte count 64 { slot { stand : u64 } }`
 * (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:22) and the body
 * `Takte.slots[i].stand = 1;` (:35). Locks are outside the bracket by
...
{
    printf("sonde sonde_zaehle :: frist_zaehle_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:33, deadline <= 80 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_zaehle [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_zaehle [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        ...
        zaehle(wo);
        volatile uint64_t senke = takte[wo];
        (void)senke;
    }
    ...
    for (long i = 0; i < iters; i++) {
        ...
        pruefsumme += takte[wo] + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: indexed u64 store into a 64-slot table, index per\n");
