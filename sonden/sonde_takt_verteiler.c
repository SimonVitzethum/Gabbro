/* sonde_takt_verteiler -- the date probe for `takt_verteiler`.
 *
 * THE OBLIGATION, as the checker declares it (`gabbro annahmen`):
 *
 *     frist_takt_verteiler_eingehalten (assume): falsifier sonde_takt_verteiler
 *
 * and in the source (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:41):
 *
 *     deadline <= 400 ops arch x86_64 falsifier sonde_takt_verteiler
 *
 * on `takt_verteiler`. This probe belongs to EXACTLY this one obligation
 * (N024). N rule, after the `beispiele/71` precedent (500 -> 5000): the
 * deadline is 10x the declared `costs` bound (`costs <= 40 ops`, so 400).
 * No pass holds the two numbers against each other (ops and cycles share no
 * conversion, D10); the factor is the writer's documented choice, not a
 * derivation.
 *
 * WHAT IT MEASURES, AND WHAT IT DOES NOT ASSERT
 * ---------------------------------------------
 * ops != cycles and the tree refuses the conversion (D10). So this probe MUST
 * NOT assert the ops deadline. It measures CYCLES of the emitted dispatch
 * body minus its lock: `takt_verteiler` emits `TAKT_nimm(); { zaehle(0); }
 * TAKT_gib();`, and the bracket below holds exactly the inner call the
 * dispatcher issues -- the u64 store at the literal index 0 into the 64-slot
 * table, replicated in the same shape and under the same `-O2` -- and
 * reports the distribution (min/p50/p99/max over N iterations).
 ...
 *   N=20000, min 26..61, p50 30..64, p99 in {33,33,67,67,67} across five
 *   runs, max noisy (73-18497, scheduling tail -- which is why the verdict
 *   reads p99 and not max); checksum 5108590 every run. C=104 booked.
 * Margin rationale: C sits about half again above the worst measured p99
 * (67 -> 104), clear of the slow-mode bulk (p50 64) and of routine jitter.
 * Two core-frequency modes step run to run (fast p50 ~30, slow p50 ~64, the
 * same stepping the gate probes show); the margin is read against the SLOW
 * mode, not the fast one. A broken dispatch at 10x lands near 650, six
 * times above C. C is a tripwire over THIS machine's distribution, not a
 * conversion of the 40 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the inner call at the constant index 0 -- the literal the emitter
 * passes, so the stimulus is constant by construction and the fixed seed
 * below salts the checksum instead of the index: the generator is still
 * reset before warmup and before the measured window, and the checksum over
 * the read-back plus the salt still stands beside the verdict, so a re-run
 * can check that it measured the same distribution. The lock
 * acquire/release (`TAKT_nimm`/`TAKT_gib`) is EXCLUDED: the emitter writes
 * `void TAKT_nimm(void);` and never defines it, the bench provides no `TAKT`
 * implementation, and timing a local stub would be an analogy (forbidden by
 * `sonden/README.md`). A green run therefore says nothing about lock cost;
 * it says the dispatched body holds its cycle budget here, this time.
 *
 * NOISE HONESTY
 * -------------
 ...
 * -----------------------
 * The probe is made to fall via `--max-cycles 1` (no bracketed call runs in
 ...
 *     -> exit 0, p99 in 33..67 <= 104 (default run PASSES)
 *   $ ./sonde_takt_verteiler --max-cycles 1
 *     -> exit 1, p99 in 33..67 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_takt_verteiler :: frist_takt_verteiler_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:41)\n");
...
#define WARMUP 1000L
/* Fixed seed: the stimulus is the constant 0 the emitter passes; the seed
 * salts the checksum instead, reset before warmup and before the window. */
#define SEED 0x2e59a1e2U

/* ---- The workload: the emitted dispatch body's shape, same `-O2`. ---- */

/* Mirrors `table Takte count 64 { slot { stand : u64 } }`
 * (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:22) and the inner
 * call `zaehle(0)` inside `takt_verteiler` (:44): the store
 * `Takte.slots[0].stand = 1;`. The lock stubs are outside the bracket by
 * design (see header): `TAKT_nimm`/`TAKT_gib` stand in the emit as bare
 * prototypes with no definition behind them. */
#define NSLOTS 64u

static uint64_t takte[NSLOTS];

static void zaehle(uint32_t i)
{
    takte[i] = 1;
}

/* ---- Fixed-seed PRNG: same run twice; salts the checksum, not the index. ---- */
...
{
    printf("sonde sonde_takt_verteiler :: frist_takt_verteiler_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:41, deadline <= 400 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_takt_verteiler [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_takt_verteiler [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        zaehle(0);
        volatile uint64_t senke = takte[0];
        (void)senke;
    }
    ...
    for (long i = 0; i < iters; i++) {
        /* The index is the constant 0 the emitter passes -- no stimulus
         * choice here. Only the dispatched call itself is timed. */
        u64 t0 = lies_tsc();
        zaehle(0);
        u64 t1 = lies_tsc();
        delta[i] = t1 - t0;
        /* Kept observable: the read-back proves the store landed, the salt
         * proves the re-run drew the same generator stream, and without the
         * sum the loop is dead code under -O2. Outside the bracket. */
        pruefsumme += takte[0] + (uint64_t)(naechste() & 0xffu) + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: dispatched u64 store at constant index 0 into a 64-slot\n");
    printf("      table (the inner call as issued); seeded salt into the checksum\n");
    printf("      (harness, outside bracket); read-back, also outside; locks\n");
    printf("      EXCLUDED (see header)\n");
    ...
    if (p99 > grenze) {
        printf("      REFUTED: p99 %llu exceeds --max-cycles %llu HERE, this time. A flaky\n",
                p99, grenze);
        printf("      red is a failed probe, not a failed date -- re-run first. The 40 ops\n");
