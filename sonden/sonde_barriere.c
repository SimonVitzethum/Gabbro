/* sonde_barriere -- the date probe for the fence `barriere`.
 *
 * THE OBLIGATION, as the checker declares it in the generated header
 * (`gabbro annahmen beispiele/60-annahme-mit-maschine.gab`):
 *
 *     frist_barriere_eingehalten (assume): falsifier sonde_barriere
 *
 * and in the source (`beispiele/60-annahme-mit-maschine.gab`:56):
 *
 *     deadline <= 10 ops arch x86_64 falsifier sonde_barriere
 *
 * on `barriere`. This probe belongs to EXACTLY this one obligation (N024).
 ...
 * (`barriere` in the emit: `__asm__ __volatile__("mfence\n" ::: "memory")`,
 ...
 *   N=20000, min 43 or 73, p50 48 or 75, p99 in {53,59,77,77,77} across five
 *   runs, max noisy (148-18591, scheduling tail -- which is why the verdict
 *   reads p99 and not max); checksum 42913250536966 every run. C=120 booked.
 ...
 * and none is used (FRIST-SONDEN-NOTIZ.md row 4, class `P0`).
 *
 * NOISE HONESTY
 * -------------
 ...
 *   $ ./sonde_barriere
 *     -> exit 0, p99 in 53..77 <= 120 (default run PASSES)
 *   $ ./sonde_barriere --max-cycles 1
 *     -> exit 1, p99 in 53..77 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_barriere :: frist_barriere_eingehalten "
           "(beispiele/60-annahme-mit-maschine.gab:56, deadline <= 10 ops arch x86_64)\n");
...
#define SEED 0xf00d0004U

/* ---- The workload: the emitted fence's shape, same `-O2`. ---- */

static void stoss_barriere(void)
{
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_barriere [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_barriere [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    printf("      workload: single `mfence` as emitted for `barriere`; pacing PRNG\n");
