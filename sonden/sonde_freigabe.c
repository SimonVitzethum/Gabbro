/* sonde_freigabe -- the date probe for the gate read `freigabe`.
 *
 * THE OBLIGATION (to be declared -- this probe stands AHEAD of it):
 *
 *     frist_freigabe_eingehalten (assume): falsifier sonde_freigabe
 *
 * for `freigabe` (`beispiele/06-annahmen.gab`:102):
 *
 *     impl fn freigabe()
 *         effects { reads tiefe_max }
 *         costs   <= 4 ops
 *
 ...
 * single load of the `tiefe_max` cell, replicated here in the same shape and
 * under the same `-O2`) and reports the distribution (min/p50/p99/max over N
 ...
 *   N=20000, min 29 or 57, p50 31 or 61, p99 in {36,39,43,63,63} across five
 *   runs, max noisy (91-67552, scheduling tail -- which is why the verdict
 *   reads p99 and not max); checksum 42906544037901 every run. C=96 booked.
 ...
 * memory. The cell is `u64` here (`Tiefe`), `u32` in `sonde_abnahme` -- the
 * only difference between the two probes, and the reason they are two
 * programs instead of one flag.
 *
 * NOISE HONESTY
 * -------------
 ...
 *   $ ./sonde_freigabe
 *     -> exit 0, p99 in 36..63 <= 96 (default run PASSES)
 *   $ ./sonde_freigabe --max-cycles 1
 *     -> exit 1, p99 in 36..63 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_freigabe :: frist_freigabe_eingehalten "
...
#define SEED 0xf9e1be06U

/* ---- The workload: the emitted gate read's shape, same `-O2`. ---- */

/* Mirrors `static mut tiefe_max : Tiefe` (`beispiele/06-annahmen.gab`:121,
 * `Tiefe = u64`). */
static uint64_t tiefe_max;

static uint64_t lies_freigabe(void)
{
    return tiefe_max;
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_freigabe [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_freigabe [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        tiefe_max = (uint64_t)naechste();
        volatile uint64_t senke = lies_freigabe();
        (void)senke;
    }
    ...
    printf("      workload: single global read of `tiefe_max` (u64), fresh value\n");
