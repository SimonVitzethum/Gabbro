/* sonde_byte_legen -- the date probe for the slot write `byte_legen`.
 *
 * THE OBLIGATION (to be declared -- this probe stands AHEAD of it):
 *
 *     frist_byte_legen_eingehalten (assume): falsifier sonde_byte_legen
 *
 * for `byte_legen` (`beispiele/25-entrust.gab`:47):
 *
 *     impl fn byte_legen(p : ptr<normal, rw> Puffer, wo : Bytelage, was : u8)
 ...
 * NOT assert the ops cost. It measures CYCLES of the emitted slot write (the
 * indexed byte store into the 4096-slot table, replicated here in the same
 * shape and under the same `-O2`) and reports the distribution (min/p50/p99/
 * max over N iterations). Falsification enters through `--max-cycles C`:
 * exit 1 iff p99 > C. The threshold C below is booked from a calibration run
 * on this machine plus a stated margin -- a measurement, not a derivation.
 *
 * BOOKED THRESHOLD (calibration 2026-09-10, workbench `Tux`, x86_64):
 *   N=20000, min 59..62, p50 65, p99 in {67,67,69,69,69} across five runs,
 *   max noisy (75-35237, scheduling tail -- which is why the verdict reads
 *   p99 and not max); checksum 5102281 every run. C=112 booked.
 * Margin rationale: C sits about half again above the worst measured p99
 * (69 -> 112), clear of the rock-steady bulk (p50 65 in all five runs) and
 * of routine jitter. One short run landed a whole bulk lower (N=5000,
 * p99 35 -- a fast core-frequency state, the same stepping the two gate
 * probes show); the margin is read against the SLOW mode, not the fast one.
 * A broken slot write at 10x lands near 700, six times above C. C is a
 * tripwire over THIS machine's distribution, not a conversion of the
 * 2 ops cost.
 *
 * WHAT IS TIMED, EXACTLY
 * ----------------------
 * Only the indexed byte store. Index and value come from the fixed-seed
 * generator (harness, outside the bracket -- fresh contents every iteration,
 * the same run twice), and the read-back that folds the stored byte into the
 * checksum runs AFTER the bracket: it proves the store landed without timing
 * the check. The table stays resident, which is the shape the emitter runs
 * in -- the guest-image writer fills a live buffer, not cold memory.
 *
 * NOISE HONESTY
 * -------------
 ...
 *   $ ./sonde_byte_legen
 *     -> exit 0, p99 in 67..69 <= 112 (default run PASSES)
 *   $ ./sonde_byte_legen --max-cycles 1
 *     -> exit 1, p99 in 67..69 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_byte_legen :: frist_byte_legen_eingehalten "
           "(UNBOUND -- beispiele/25 carries no deadline yet, see header)\n");
...
#define BOOKED_MAX_CYCLES 112ULL
...
#define SEED 0xb71e1925U

/* ---- The workload: the emitted slot write's shape, same `-O2`. ---- */

/* Mirrors `table Puffer count 4096 { slot { wert : u8 } }`
 * (`beispiele/25-entrust.gab`:43) and `NBYTES = 4096` (:40). */
#define NBYTES 4096u

static uint8_t puffer[NBYTES];

static void lege_byte(uint32_t wo, uint8_t was)
{
    puffer[wo] = was;
}
...
int main(int argc, char **argv)
{
    printf("sonde sonde_byte_legen :: frist_byte_legen_eingehalten "
           "(UNBOUND -- beispiele/25 carries no deadline yet, see header; "
           "slot write costs <= 2 ops arch none)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_byte_legen [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_byte_legen [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        uint32_t wo = naechste() % NBYTES;
        uint8_t was = (uint8_t)(naechste() & 0xffu);
        lege_byte(wo, was);
        volatile uint8_t senke = puffer[wo];
        (void)senke;
    }
    ...
    for (long i = 0; i < iters; i++) {
        /* Stimulus stays OUTSIDE the bracket: index and value are the
         * harness, not the date. Only the slot write itself is timed. */
        uint32_t wo = naechste() % NBYTES;
        uint8_t was = (uint8_t)(naechste() & 0xffu);
        u64 t0 = lies_tsc();
        lege_byte(wo, was);
        ...
        pruefsumme += puffer[wo] + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: indexed byte store into a 4096-slot table, index and\n");
    printf("      value per iteration from a fixed-seed xorshift (harness, outside\n");
    ...
    if (p99 > grenze) {
        printf("      REFUTED: p99 %llu exceeds --max-cycles %llu HERE, this time. A flaky\n",
                p99, grenze);
        printf("      red is a failed probe, not a failed date -- re-run first. The 2 ops\n");
