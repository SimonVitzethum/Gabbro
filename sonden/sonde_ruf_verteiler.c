/* sonde_ruf_verteiler -- the date probe for `ruf_verteiler`.
 *
 * THE OBLIGATION, as the checker declares it (`gabbro annahmen`):
 *
 *     frist_ruf_verteiler_eingehalten (assume): falsifier sonde_ruf_verteiler
 *
 * and in the source (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:89):
 *
 *     deadline <= 400 ops arch x86_64 falsifier sonde_ruf_verteiler
 *
 * on `ruf_verteiler`. This probe belongs to EXACTLY this one obligation
 ...
 * body minus its lock: `ruf_verteiler` emits `RING_nimm(); { bearbeite(0); }
 * RING_gib();`, and the bracket below holds exactly the inner call the
 * dispatcher issues -- the u64 store at the literal index 0 into the 16-slot
 ...
 *   N=20000, min 29..61, p50 32..64, p99 in {41,49,67,67,67} across five
 *   runs, max noisy (69-12473, scheduling tail -- which is why the verdict
 *   reads p99 and not max); checksum 5120748 every run. C=104 booked.
 ...
 * Two core-frequency modes step run to run (fast p50 ~32, slow p50 ~64, the
 ...
 * acquire/release (`RING_nimm`/`RING_gib`) is EXCLUDED: the emitter writes
 * `void RING_nimm(void);` and never defines it, the bench provides no `RING`
 ...
 *   $ ./sonde_ruf_verteiler
 *     -> exit 0, p99 in 41..67 <= 104 (default run PASSES)
 *   $ ./sonde_ruf_verteiler --max-cycles 1
 *     -> exit 1, p99 in 41..67 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_ruf_verteiler :: frist_ruf_verteiler_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:89)\n");
...
#define SEED 0x2e59a1e4U

/* ---- The workload: the emitted dispatch body's shape, same `-O2`. ---- */

/* Mirrors `table Auftraege count 16 { slot { stand : u64 } }`
 * (`beispiele/59-eintritt-nimmt-maskierte-sperre.gab`:71) and the inner
 * call `bearbeite(0)` inside `ruf_verteiler` (:92): the store
 * `Auftraege.slots[0].stand = 1;`. The lock stubs are outside the bracket
 * by design (see header): `RING_nimm`/`RING_gib` stand in the emit as bare
...
{
    printf("sonde sonde_ruf_verteiler :: frist_ruf_verteiler_eingehalten "
           "(beispiele/59-eintritt-nimmt-maskierte-sperre.gab:89, deadline <= 400 ops arch x86_64)\n");
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_ruf_verteiler [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_ruf_verteiler [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    for (long w = 0; w < WARMUP; w++) {
        bearbeite(0);
        volatile uint64_t senke = auftraege[0];
        (void)senke;
    }
    ...
    for (long i = 0; i < iters; i++) {
        ...
        pruefsumme += auftraege[0] + (uint64_t)(naechste() & 0xffu) + (uint64_t)(i & 0xffu);
    }
    ...
    printf("      workload: dispatched u64 store at constant index 0 into a 16-slot\n");
