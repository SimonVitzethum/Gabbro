/* sonde_schreib_schranke -- the date probe for the fence `schreib_schranke`.
 *
 * THE OBLIGATION, as the checker declares it in the generated header
 * (`gabbro annahmen beispiele/67-befehlsebene.gab`):
 *
 *     frist_schreib_schranke_eingehalten (assume): falsifier sonde_schreib_schranke
 *
 * and in the source (`beispiele/67-befehlsebene.gab`:43):
 *
 *     deadline <= 10 ops arch x86_64 falsifier sonde_schreib_schranke
 *
 * on `schreib_schranke`. This probe belongs to EXACTLY this one obligation (N024).
 ...
 * (`schreib_schranke` in the emit: `__asm__ __volatile__("sfence\n" ::: "memory")`,
 ...
 *   N=20000, min 29, 45 or 59, p50 31 or 61, p99 in {33,63,63,65,65} across
 *   five runs, max noisy (77-209993, scheduling tail -- which is why the
 *   verdict reads p99 and not max); checksum 42954178085256 every run.
 *   C=96 booked.
 ...
 * and none is used (FRIST-SONDEN-NOTIZ.md row 5, class `P0`).
 *
 * NOISE HONESTY
 * -------------
 ...
 *     -> exit 0, p99 in 33..65 <= 96 (default run PASSES)
 *   $ ./sonde_schreib_schranke --max-cycles 1
 *     -> exit 1, p99 in 33..65 > 1 (positive control FALLS)
 *
...
int main(void)
{
    printf("sonde sonde_schreib_schranke :: frist_schreib_schranke_eingehalten "
           "(beispiele/67-befehlsebene.gab:43, deadline <= 10 ops arch x86_64)\n");
...
#define SEED 0xf00d0005U

/* ---- The workload: the emitted fence's shape, same `-O2`. ---- */

static void stoss_schreib_schranke(void)
{
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--max-cycles") == 0 && i + 1 < argc) {
            ...
            if (dezimal(argv[++i], &v) != 0 || v < 1) {
                fprintf(stderr, "usage: sonde_schreib_schranke [--iterations N] [--max-cycles C]\n");
                return 1;
            }
            grenze = (u64)v;
            grenze_gesetzt = 1;
            ...
            printf("usage: sonde_schreib_schranke [--iterations N] [--max-cycles C] [N]\n");
            return 0;
        } else {
            long v = 0;
    ...
    printf("      workload: single `sfence` as emitted for `schreib_schranke`; pacing PRNG\n");
