...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Text.bytes" _ "elem" => some .int
  | .field "Text" "len" => some .int
  | _ => none
...
def anhaengen_pre : Expr :=
  (.bin .and (.hasShape "b" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "b")) (.bin .le (.name "b") (.lit (.int 255)))))
...
def byte_an_pre : Expr :=
  (.bin .and (.bin .and (.hasShape "i" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "i")) (.bin .le (.name "i") (.lit (.int 63))))) (.bin .lt (.name "i") (.fieldOf "Text" "len")))
...
theorem anhaengen_meets : anhaengen_meets_statement := by
  ...
  obtain ⟨w_b, e_b, lo_b, hi_b⟩ := shape_int_in s "b" _ _ hpre
  obtain ⟨n_Text_len, h_Text_len⟩ := WF_int shapeOf s.world (.field "Text" "len") hwf rfl
  gabbro_auto [anhaengen_body, anhaengen_pre, anhaengen_post, wellFormed, e_b, h_Text_len] using shapeOf
...
theorem byte_an_meets : byte_an_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_i, e_i, lo_i, hi_i⟩ := shape_int_in s "i" _ _ (and_left _ _ _ hpre)
  obtain ⟨n_Text.bytes_elem_i, h_Text.bytes_elem_i⟩ := WF_int shapeOf s.world (.slot "Text.bytes" w_i "elem") hwf rfl
  gabbro_auto [byte_an_body, byte_an_pre, byte_an_post, wellFormed, e_i, h_Text.bytes_elem_i] using shapeOf
...
theorem laenge_meets : laenge_meets_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  obtain ⟨n_Text_len, h_Text_len⟩ := WF_int shapeOf s.world (.field "Text" "len") hwf rfl
  gabbro_auto [laenge_body, laenge_pre, laenge_post, wellFormed, h_Text_len] using shapeOf
