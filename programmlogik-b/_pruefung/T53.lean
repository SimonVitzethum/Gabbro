...
def treffen_oeffnen_pre : Expr :=
  (.bin .and (.hasShape "kern" .int) (.bin .and (.hasShape "ruft" .int) (.bin .and (.hasShape "dient" .int) (.bin .and (.lit (.bool true)) (.bin .and (.place "Endpunkt" (.name "kern") "benutzt") (.forallSlots "e" 4 (.bin .eq (.bin .eq (.place "Endpunkt" (.name "e") "anrufer") (.lit .absent)) (.bin .eq (.place "Endpunkt" (.name "e") "antwortende") (.lit .absent)))))))))
...
theorem treffen_oeffnen_meets : treffen_oeffnen_meets_statement := by
  ...
  gabbro_simp_at hi_antwortpflicht_paarig [treffen_oeffnen_inv_antwortpflicht_paarig]
  obtain ⟨w_kern, e_kern⟩ := shape_int s "kern" (and_left _ _ _ hpre)
  obtain ⟨w_ruft, e_ruft⟩ := shape_int s "ruft" (and_left _ _ _ (and_right _ _ _ hpre))
  obtain ⟨w_dient, e_dient⟩ := shape_int s "dient" (and_left _ _ _ (and_right _ _ _ (and_right _ _ _ hpre)))
  rcases WF_opt shapeOf s.world (.slot "Endpunkt" w_kern "anrufer") hwf rfl with h_Endpunkt_anrufer_kern | ⟨n_Endpunkt_anrufer_kern, h_Endpunkt_anrufer_kern⟩ <;>
    ...
    <;> gabbro_auto? [treffen_oeffnen_body, treffen_oeffnen_pre, treffen_oeffnen_post, wellFormed, treffen_oeffnen_inv_antwortpflicht_paarig, e_kern, e_ruft, e_dient, h_Endpunkt_anrufer_kern, h_Endpunkt_antwortende_kern] using shapeOf
...
def treffen_schliessen_pre : Expr :=
  (.bin .and (.hasShape "kern" .int) (.bin .and (.lit (.bool true)) (.bin .and (.place "Endpunkt" (.name "kern") "benutzt") (.forallSlots "e" 4 (.bin .eq (.bin .eq (.place "Endpunkt" (.name "e") "anrufer") (.lit .absent)) (.bin .eq (.place "Endpunkt" (.name "e") "antwortende") (.lit .absent)))))))
...
theorem treffen_schliessen_meets : treffen_schliessen_meets_statement := by
  ...
  gabbro_simp_at hi_antwortpflicht_paarig [treffen_schliessen_inv_antwortpflicht_paarig]
  obtain ⟨w_kern, e_kern⟩ := shape_int s "kern" (and_left _ _ _ hpre)
  rcases WF_opt shapeOf s.world (.slot "Endpunkt" w_kern "anrufer") hwf rfl with h_Endpunkt_anrufer_kern | ⟨n_Endpunkt_anrufer_kern, h_Endpunkt_anrufer_kern⟩ <;>
    rcases WF_opt shapeOf s.world (.slot "Endpunkt" w_kern "antwortende") hwf rfl with h_Endpunkt_antwortende_kern | ⟨n_Endpunkt_antwortende_kern, h_Endpunkt_antwortende_kern⟩
    <;> gabbro_auto? [treffen_schliessen_body, treffen_schliessen_pre, treffen_schliessen_post, wellFormed, treffen_schliessen_inv_antwortpflicht_paarig, e_kern, h_Endpunkt_anrufer_kern, h_Endpunkt_antwortende_kern] using shapeOf
