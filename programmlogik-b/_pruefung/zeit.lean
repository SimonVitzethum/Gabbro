...
theorem release_slot_meets : release_slot_meets_statement := by
  ...
  gabbro_pipeline_b [release_slot_body, release_slot_pre, release_slot_post, wellFormed, release_slot_inv_wurzel_ohne_vorgaenger, e_s, h_CapSpace_used_s, hall] using shapeOf <;> sorry
...
theorem revoke_loop_1_keeps : revoke_loop_1_keeps_statement := by
  ...
  gabbro_pipeline_b [revoke_loop_1_body, revoke_loop_1_inv, wellFormed, delete_leaf_pre, delete_leaf_requires, delete_leaf_post, delete_leaf_writes, Frame_read _ _ _ fr_delete_leaf, e_s, e__returned, hall] using shapeOf <;> sorry
...
theorem revoke_meets : revoke_meets_statement := by
  ...
  gabbro_pipeline_b [revoke_body, revoke_pre, revoke_post, wellFormed, revoke_inv_cdt_wohlgeformt, revoke_inv_wurzel_ohne_vorgaenger, delete_leaf_pre, delete_leaf_requires, delete_leaf_post, delete_leaf_writes, Frame_read _ _ _ fr_delete_leaf, revoke_loop_1_inv, e_s, hall] using shapeOf <;> sorry
...
theorem unlink_meets : unlink_meets_statement := by
  ...
  rcases WF_opt shapeOf s.world (.slot "CapSpace" w_s "prev_sibling") hwf rfl with h_CapSpace_prev_sibling_s | ⟨n_CapSpace_prev_sibling_s, h_CapSpace_prev_sibling_s⟩ <;>
    ...
    <;> gabbro_pipeline_b [unlink_body, unlink_pre, unlink_post, wellFormed, unlink_inv_cdt_wohlgeformt, unlink_inv_wurzel_ohne_vorgaenger, e_s, h_CapSpace_prev_sibling_s, h_CapSpace_next_sibling_s, h_CapSpace_parent_s, hall] using shapeOf <;> sorry
