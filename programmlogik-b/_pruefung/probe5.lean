...
def fehler_melden_pre : Expr :=
  (.bin .and (.hasShape "n" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "n")) (.bin .le (.name "n") (.lit (.int 4294967295)))))
...
def melde_hoch_pre : Expr :=
  (.bin .and (.hasShape "t" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "t")) (.bin .le (.name "t") (.lit (.int 18446744073709551615)))))
...
theorem fehler_meets : fehler_meets_statement := by
  ...
  have hw := WF_int shapeOf s.world (.global "fehlerzahl") hwf rfl
  gabbro_shape shapeOf
  trace_state
  gabbro_auto? [fehler_body, fehler_pre, fehler_post, wellFormed] using shapeOf
...
theorem fehler_melden_meets : fehler_melden_meets_statement := by
  ...
  gabbro_auto [fehler_melden_body, fehler_melden_pre, fehler_melden_post, wellFormed, e_n] using shapeOf
...
theorem hoechster_meets : hoechster_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  gabbro_auto [hoechster_body, hoechster_pre, hoechster_post, wellFormed] using shapeOf
...
theorem melde_hoch_meets : melde_hoch_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_t, e_t, lo_t, hi_t⟩ := shape_int_in s "t" _ _ hpre
  gabbro_auto [melde_hoch_body, melde_hoch_pre, melde_hoch_post, wellFormed, e_t] using shapeOf
...
theorem melde_tief_meets : melde_tief_meets_statement := by
  ...
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_t, e_t, lo_t, hi_t⟩ := shape_int_in s "t" _ _ hpre
  gabbro_auto [melde_tief_body, melde_tief_pre, melde_tief_post, wellFormed, e_t] using shapeOf
...
theorem tiefster_meets : tiefster_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  gabbro_auto [tiefster_body, tiefster_pre, tiefster_post, wellFormed] using shapeOf
