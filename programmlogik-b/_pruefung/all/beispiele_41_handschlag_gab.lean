...
/-  NOT WIRED -- the duty is stated above; the step from it to the contract is not:
    ...
    A self-recursion is wired by induction over its `decreases`
    (`contract_of_duty_rec`), a cycle of routines over their shared measure
    (`contracts_of_duties_rec`); a caller of a REFUSED callee has no contract
    to compose with, and a recursive call inside a loop is not written. -/
