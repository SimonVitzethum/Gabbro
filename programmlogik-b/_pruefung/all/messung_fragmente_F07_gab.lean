/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/fragmente/F07.gab  total 0  goals 0  refused 0
...
def autoritaet_melden_writes : List String := ["p"]
...
def cap_tabellen_writes : List String := ["p", "caps"]
...
def ipc_tabellen_writes : List String := ["p", "eps"]
...
def mmu_an_writes : List String := ["p", "mmu"]
...
def hochlauf_writes : List String := ["p", "mmu", "caps", "eps", "faeden"]
...
def Program (ρ : Env) : Prop :=
  Runs ρ "hochlauf" hochlauf_body
  ∧ Frame ρ "hochlauf" hochlauf_writes

theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    (d_hochlauf : hochlauf_meets_statement) :
    Contract ρ "hochlauf" hochlauf_requires hochlauf_post := by
  obtain ⟨r_hochlauf, fr_hochlauf⟩ := hp
  obtain ⟨c_autoritaet_melden, fr_autoritaet_melden, c_cap_tabellen, fr_cap_tabellen, c_ipc_tabellen, fr_ipc_tabellen, c_mmu_an, fr_mmu_an, c_root_task_starten, fr_root_task_starten, c_verifizierer_starten, fr_verifizierer_starten⟩ := ha
  have c_hochlauf : Contract ρ "hochlauf" hochlauf_requires hochlauf_post :=
    contract_of_duty ρ "hochlauf" hochlauf_body hochlauf_requires hochlauf_post r_hochlauf
      (fun t ht => d_hochlauf ρ t ht.1 ht.2 c_autoritaet_melden fr_autoritaet_melden c_cap_tabellen fr_cap_tabellen c_ipc_tabellen fr_ipc_tabellen c_mmu_an fr_mmu_an c_root_task_starten fr_root_task_starten c_verifizierer_starten fr_verifizierer_starten)
  exact c_hochlauf

end GabbroDuty.DutyF07
