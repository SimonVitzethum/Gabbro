/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  messung/fragmente/F06.gab  total 2  goals 2  refused 0
...
/-
  duty_1  S  unberuehrt :: loop invariant #1  --  carried by `unberuehrt_loop_1_keeps`
  duty_2  N  unberuehrt :: ensures #1  --  carried by `unberuehrt_meets`
-/
...
def unberuehrt_body : List Stmt :=
  [(.bindName "i" (.lit (.int 0))), (.bindName "#returned" (.lit (.bool false))), (.bindName "#ret" (.lit .absent)), (.bindName "#pass" (.lit (.int 0))), (.loop "unberuehrt#1" (.bin .and (.hasShape "i" (.intIn 0 65536)) (.bin .and (.hasShape "#returned" .bool) (.bin .and (.hasShape "#pass" .int) (.bin .or (.bin .and (.name "#returned") (.bin .and (.hasShape "#ret" (.intIn 0 18446744073709551615)) (.bin .le (.name "#ret") (.fieldOf "Stack" "len")))) (.bin .and (.un .not (.name "#returned")) (.bin .le (.name "i") (.name "#pass"))))))) [(.bindName "#pass" (.bin .add (.name "#pass") (.lit (.int 1)))), (.ite (.name "#returned") [.leave] []), (.ite (.bin .ne (.name "w") (.lit (.int 16045690984833335023))) [(.bindName "#ret" (.bin .mul (.name "i") (.lit (.int 8)))), (.bindName "#returned" (.lit (.bool true))), .leave] []), (.bindName "i" (.bin .add (.name "i") (.lit (.int 1))))]), (.ite (.name "#returned") [(.ret (some (.name "#ret")))] []), (.ret (some (.bin .mul (.name "i") (.lit (.int 8)))))]
...
def unberuehrt_loop_1_inv : Expr :=
  (.bin .and (.hasShape "i" (.intIn 0 65536)) (.bin .and (.hasShape "#returned" .bool) (.bin .and (.hasShape "#pass" .int) (.bin .or (.bin .and (.name "#returned") (.bin .and (.hasShape "#ret" (.intIn 0 18446744073709551615)) (.bin .le (.name "#ret") (.fieldOf "Stack" "len")))) (.bin .and (.un .not (.name "#returned")) (.bin .le (.name "i") (.name "#pass")))))))

def unberuehrt_loop_1_body : List Stmt :=
  [(.bindName "#pass" (.bin .add (.name "#pass") (.lit (.int 1)))), (.ite (.name "#returned") [.leave] []), (.ite (.bin .ne (.name "w") (.lit (.int 16045690984833335023))) [(.bindName "#ret" (.bin .mul (.name "i") (.lit (.int 8)))), (.bindName "#returned" (.lit (.bool true))), .leave] []), (.bindName "i" (.bin .add (.name "i") (.lit (.int 1))))]
...
theorem unberuehrt_loop_1_keeps : unberuehrt_loop_1_keeps_statement := by
  ...
  have hall := hinv
  gabbro_simp_at hall [unberuehrt_loop_1_inv, e_i, e__returned, e__pass]
  gabbro_auto [unberuehrt_loop_1_body, unberuehrt_loop_1_inv, wellFormed, e_i, e__returned, e__pass, hall, hpass] using shapeOf
...
def unberuehrt_meets_statement : Prop :=
  ∀ (ρ : Env) (s : State)
    ...
    (l_unberuehrt_loop_1 : LoopRuleP ρ "unberuehrt#1" wellFormed unberuehrt_loop_1_inv "#pass"),
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoopN ρ "unberuehrt#1" unberuehrt_loop_1_body "w" 0 18446744073709551616 8192
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    ...
    LoopRuleP ρ "unberuehrt#1" wellFormed unberuehrt_loop_1_inv "#pass"
  ...
  have l_unberuehrt_loop_1 : LoopRuleP ρ "unberuehrt#1" wellFormed unberuehrt_loop_1_inv "#pass" :=
    looprule_of_body_p ρ "unberuehrt#1" wellFormed unberuehrt_loop_1_inv unberuehrt_loop_1_body "w" "#pass" 0 18446744073709551616 8192 rl_unberuehrt_loop_1
      (fun t k i hlo hhi hplo hphi hpass hw hin => d_unberuehrt_loop_1 ρ t k i hlo hhi hplo hphi hpass hw hin)
