/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  beispiele/gift/435-slots-of-laufvariable.gab  total 0  goals 0  refused 0
...
namespace GabbroDuty.Duty435SlotsOfLaufvariable
...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Riesig" _ "wert" => some (.intIn 0 4294967295)
  | .slot "Winzig" _ "wert" => some (.intIn 0 4294967295)
  | _ => none
...
def f_body : List Stmt :=
  [(.bindName "summe" (.lit (.int 0))), (.loop "f#1" (.hasShape "summe" (.intIn 0 65535)) [(.loop "f#2" (.bin .and (.hasShape "summe" (.intIn 0 65535)) (.hasShape "t" .int)) [(.ite (.bin .lt (.name "summe") (.lit (.int 60000))) [(.bindName "summe" (.bin .add (.name "summe") (.lit (.int 1))))] [])])]), (.ret (some (.name "summe")))]
...
def f_loop_2_inv : Expr :=
  (.bin .and (.hasShape "summe" (.intIn 0 65535)) (.hasShape "t" .int))
...
def f_loop_2_keeps_statement : Prop :=
  ∀ (ρ : Env) (t : State) (k : Int)
    ...
    ∃ t', finalState (exec ρ f_loop_2_body { t with local' := bindLocal t.local' "i" (.int k) }) = some t'
...
theorem f_loop_2_keeps : f_loop_2_keeps_statement := by
  ...
  obtain ⟨w_t, e_t⟩ := shape_int t "t" (and_right _ _ _ hinv)
  have hall := hinv
  gabbro_simp_at hall [f_loop_2_inv, e_summe, e_t]
  gabbro_auto [f_loop_2_body, f_loop_2_inv, wellFormed, e_summe, e_t, hall] using shapeOf

/-- Loop `f#1` of `f`: its body and its invariant (with the shapes of the locals in scope). -/
def f_loop_1_inv : Expr :=
  (.hasShape "summe" (.intIn 0 65535))

def f_loop_1_body : List Stmt :=
  [(.loop "f#2" (.bin .and (.hasShape "summe" (.intIn 0 65535)) (.hasShape "t" .int)) [(.ite (.bin .lt (.name "summe") (.lit (.int 60000))) [(.bindName "summe" (.bin .add (.name "summe") (.lit (.int 1))))] [])])]
...
def f_loop_1_keeps_statement : Prop :=
  ∀ (ρ : Env) (t : State) (k : Int)
    ...
    ∃ t', finalState (exec ρ f_loop_1_body { t with local' := bindLocal t.local' "t" (.int k) }) = some t'
...
theorem f_loop_1_keeps : f_loop_1_keeps_statement := by
  ...
  intro ρ t k hlo hhi hwf hinv l_f_loop_2
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_summe, e_summe, lo_summe, hi_summe⟩ := shape_intIn t "summe" _ _ hinv
  have hall := hinv
  gabbro_simp_at hall [f_loop_1_inv, e_summe]
  gabbro_auto [f_loop_1_body, f_loop_1_inv, wellFormed, f_loop_2_inv, e_summe, hall] using shapeOf
...
theorem f_meets : f_meets_statement := by
  ...
  gabbro_auto [f_body, f_pre, f_post, wellFormed, f_loop_2_inv, f_loop_1_inv, hall] using shapeOf
...
def Program (ρ : Env) : Prop :=
  Runs ρ "f" f_body
  ∧ Frame ρ "f" f_writes
  ∧ RunsLoopIn ρ "f#2" f_loop_2_body "i" 0 64
  ∧ RunsLoopIn ρ "f#1" f_loop_1_body "t" 0 2
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
  ...
  have l_f_loop_2 : LoopRule ρ "f#2" wellFormed f_loop_2_inv :=
    looprule_of_body_in ρ "f#2" wellFormed f_loop_2_inv f_loop_2_body "i" 0 64 rl_f_loop_2
      (fun t k hlo hhi hw hi => d_f_loop_2 ρ t k hlo hhi hw hi)
  have l_f_loop_1 : LoopRule ρ "f#1" wellFormed f_loop_1_inv :=
    looprule_of_body_in ρ "f#1" wellFormed f_loop_1_inv f_loop_1_body "t" 0 2 rl_f_loop_1
      (fun t k hlo hhi hw hi => d_f_loop_1 ρ t k hlo hhi hw hi l_f_loop_2)
...
end GabbroDuty.Duty435SlotsOfLaufvariable
