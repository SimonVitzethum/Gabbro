...
-- REFUSED  poll_used  (carrier-not-a-table): the carrier of a place is not a declared `table`, record or `format`
...
-- REFUSED  publish  (carrier-not-a-table): the carrier of a place is not a declared `table`, record or `format`
--   Its contract stands below all the same, so that a caller's theorem can name it;
--   what is missing is the theorem that discharges it.
