...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Griffe" _ "zaehler" => some (.intIn 0 65534)
...
def aufraeumen_body : List Stmt :=
  [(.locked "GRIFFE" [(.bindCallElse "n" "platz_freigeben" ["g", "s"] [(.name "g"), (.name "s")] (.bin .and (.hasShape "s" (.intIn 0 255)) (.lit (.bool true))) "e" [(.onReason (.name "e") [("Unbelegt", [(.ret (some (.lit (.int 0))))]), ("Buchfuehrung", [(.call "buchfuehrung_kaputt" [] [] (.lit (.bool true)))])])]), (.ret (some (.name "n")))])]

/-- The precondition: the declared shapes and the `requires`. -/
def aufraeumen_pre : Expr :=
  (.hasShape "s" (.intIn 0 255))
...
def nur_unbelegte_zaehlen_body : List Stmt :=
  [(.locked "GRIFFE" [(.bindCallElse "n" "platz_freigeben" ["g", "s"] [(.name "g"), (.name "s")] (.bin .and (.hasShape "s" (.intIn 0 255)) (.lit (.bool true))) "e" [(.ite (.bin .eq (.name "e") (.lit (.reason "Unbelegt"))) [(.ret (some (.lit (.int 1))))] []), (.ret (some (.lit (.int 0))))]), (.ret (some (.name "n")))])]
...
def platz_freigeben_pre : Expr :=
  (.bin .and (.hasShape "s" (.intIn 0 255)) (.lit (.bool true)))
...
theorem nur_unbelegte_zaehlen_meets : nur_unbelegte_zaehlen_meets_statement := by
  ...
  obtain ⟨w_s, e_s, lo_s, hi_s⟩ := shape_intIn s "s" _ _ hpre
  have hall := hpre
  gabbro_simp_at hall [nur_unbelegte_zaehlen_pre, e_s]
  gabbro_auto [nur_unbelegte_zaehlen_body, nur_unbelegte_zaehlen_pre, nur_unbelegte_zaehlen_post, wellFormed, platz_freigeben_pre, platz_freigeben_requires, platz_freigeben_post, platz_freigeben_writes, Frame_read _ _ _ fr_platz_freigeben, e_s, hall] using shapeOf
...
theorem platz_freigeben_meets : platz_freigeben_meets_statement := by
  ...
  obtain ⟨n_Griffe_zaehler_s, h_Griffe_zaehler_s, lo_Griffe_zaehler_s, hi_Griffe_zaehler_s⟩ := WF_intIn shapeOf s.world (.slot "Griffe" w_s "zaehler") _ _ hwf rfl
  have hall := hpre
  gabbro_simp_at hall [platz_freigeben_pre, e_s, h_Griffe_belegt_s, h_Griffe_zaehler_s]
  gabbro_auto [platz_freigeben_body, platz_freigeben_pre, platz_freigeben_post, wellFormed, e_s, h_Griffe_belegt_s, h_Griffe_zaehler_s, hall] using shapeOf
