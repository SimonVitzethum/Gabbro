/-  Written by `gabbro pflichten --lean`. Do not edit -- the source is the
    ...
    REFUSED by name. The line that has to add up:
        ...
        @duty 1  beispiele/50-verfeinerung.gab  total 1  goals 1  refused 0
...
namespace GabbroDuty.Duty50Verfeinerung
...
/-
  duty_1  R  freigeben :: refines ist_frei  --  carried by `freigeben_meets`
-/
...
def shapeOf : Typing := fun p =>
  match p with
  | .slot "Buch" _ "belegt" => some .bool
  | .slot "Buch" _ "wert" => some (.intIn 0 65535)
  | _ => none
...
def freigeben_body : List Stmt :=
  [(.assign "Buch" (.name "p") "belegt" (.lit (.bool false))), (.assign "Buch" (.name "p") "wert" (.lit (.int 0)))]

/-- The precondition: the declared shapes and the `requires`. -/
def freigeben_pre : Expr :=
  (.bin .and (.hasShape "p" (.intIn 0 255)) (.bin .and (.lit (.bool true)) (.place "Buch" (.name "p") "belegt")))

def freigeben_writes : List String := ["Buch"]
...
def freigeben_post (s s' : State) (r : Option Value) : Prop :=
  wellFormed s'
  ∧ -- refines ist_frei
  (eval { world := s'.world, local' := s.local' } (.un .not (.place "Buch" (.name "p") "belegt")) = some (.bool true))
...
theorem freigeben_meets : freigeben_meets_statement := by
  ...
  obtain ⟨n_Buch_belegt_p, h_Buch_belegt_p⟩ := WF_bool shapeOf s.world (.slot "Buch" w_p "belegt") hwf rfl
  have hall := hpre
  gabbro_simp_at hall [freigeben_pre, e_p, h_Buch_belegt_p]
  gabbro_auto [freigeben_body, freigeben_pre, freigeben_post, wellFormed, e_p, h_Buch_belegt_p, hall] using shapeOf
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
  ...
  obtain ⟨r_freigeben, fr_freigeben⟩ := hp
...
end GabbroDuty.Duty50Verfeinerung
