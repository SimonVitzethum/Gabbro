...
def zaehle_freie_body : List Stmt :=
  [(.bindName "n" (.lit (.int 0))), (.bindName "#pass" (.lit (.int 0))), (.loop "zaehle_freie#1" (.bin .and (.hasShape "n" (.intIn 0 4294967295)) (.bin .and (.hasShape "#pass" .int) (.bin .le (.name "n") (.name "#pass")))) [(.bindName "#pass" (.bin .add (.name "#pass") (.lit (.int 1)))), (.ite (.un .not (.place "Plaetze" (.name "i") "belegt")) [(.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))] [])]), (.ret (some (.name "n")))]
...
def zaehle_freie_loop_1_inv : Expr :=
  (.bin .and (.hasShape "n" (.intIn 0 4294967295)) (.bin .and (.hasShape "#pass" .int) (.bin .le (.name "n") (.name "#pass"))))

def zaehle_freie_loop_1_body : List Stmt :=
  [(.bindName "#pass" (.bin .add (.name "#pass") (.lit (.int 1)))), (.ite (.un .not (.place "Plaetze" (.name "i") "belegt")) [(.bindName "n" (.bin .add (.name "n") (.lit (.int 1))))] [])]
...
theorem zaehle_freie_loop_1_keeps : zaehle_freie_loop_1_keeps_statement := by
  ...
  have hall := hinv
  gabbro_simp_at hall [zaehle_freie_loop_1_inv, e_n, e__pass, h_Plaetze_belegt_i]
  gabbro_auto [zaehle_freie_loop_1_body, zaehle_freie_loop_1_inv, wellFormed, e_n, e__pass, h_Plaetze_belegt_i, hall, hpass] using shapeOf
...
def zaehle_freie_meets_statement : Prop :=
  ∀ (ρ : Env) (s : State)
    ...
    (l_zaehle_freie_loop_1 : LoopRuleP ρ "zaehle_freie#1" wellFormed zaehle_freie_loop_1_inv "#pass"),
...
def Program (ρ : Env) : Prop :=
  ...
  ∧ RunsLoopN ρ "zaehle_freie#1" zaehle_freie_loop_1_body "i" 0 8 8
...
theorem unit_closed (ρ : Env) (hp : Program ρ) (ha : Assumed ρ)
    ...
    ∧ LoopRuleP ρ "zaehle_freie#1" wellFormed zaehle_freie_loop_1_inv "#pass"
  ...
  have l_zaehle_freie_loop_1 : LoopRuleP ρ "zaehle_freie#1" wellFormed zaehle_freie_loop_1_inv "#pass" :=
    looprule_of_body_p ρ "zaehle_freie#1" wellFormed zaehle_freie_loop_1_inv zaehle_freie_loop_1_body "i" "#pass" 0 8 8 rl_zaehle_freie_loop_1
      (fun t k i hlo hhi hplo hphi hpass hw hin => d_zaehle_freie_loop_1 ρ t k i hlo hhi hplo hphi hpass hw hin)
