...
def shapeOf : Typing := fun p =>
  ...
  | .slot "Anfragen" _ "was" => some (.sum [("Leer", false), ("Kurz", true), ("Lang", true), ("Antwort", true)])
  | _ => none
...
def art_von_pre : Expr :=
  (.hasShape "m" (.sum [("Leer", false), ("Kurz", true), ("Lang", true), ("Antwort", true)]))
...
def gewicht_im_slot_pre : Expr :=
  (.bin .and (.hasShape "i" .int) (.bin .and (.bin .le (.lit (.int 0)) (.name "i")) (.bin .le (.name "i") (.lit (.int 7)))))
...
theorem gewicht_im_slot_meets : gewicht_im_slot_meets_statement := by
  ...
  intro ρ s hwf hpre
  simp only [wellFormed] at hwf ⊢
  obtain ⟨w_i, e_i, lo_i, hi_i⟩ := shape_int_in s "i" _ _ hpre
  ...
  gabbro_try 300 (gabbro_simp [gewicht_im_slot_body, gewicht_im_slot_pre, gewicht_im_slot_post, wellFormed, e_i, h_Anfragen_was_i, hall])
  gabbro_try 300 (all_goals (try gabbro_cases 4 [gewicht_im_slot_body, gewicht_im_slot_pre, gewicht_im_slot_post, wellFormed, e_i, h_Anfragen_was_i, hall]))
  gabbro_try 300 (all_goals (try gabbro_calls shapeOf [gewicht_im_slot_body, gewicht_im_slot_pre, gewicht_im_slot_post, wellFormed, e_i, h_Anfragen_was_i, hall]))
  gabbro_open_hyps
  trace_state
  all_goals sorry
